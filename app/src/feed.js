...
function renderPost(post, isReply) {
  const wrap = el('article', isReply ? 'feed-post feed-reply' : 'feed-post');
...
export function createFeedPanel() {
  ...
  async function poll() {
    ...
    if (!threads.length) {
      status.textContent = payload.live === false ? 'Quiet right now.' : 'Nobody has posted yet.';
    ...
    const key = threads.map((t) => `${t.id}:${t.posts.length}`).join('|');
