...
- `window.SF` is exposed for automated checks: `SF.rig.state` (`pivot`, `yaw`, `pitch`, `distance`),
  ...
  `SF.pick(nx, ny)`, `SF.boot`. Prefer real UI interaction for the recording and use `SF.rig.state`
  only as numeric corroboration.

## The boot curtain (`app/src/boot.js` + `boot.css`)

Every load opens behind full-screen fog with a chrome-capsule progress bar; when the bar fills, the
fog parts like two doors sliding apart to reveal the finished city, and the whole `#boot` subtree is
removed from the DOM. The markup is inline in `index.html` (not built by JS) so it paints before any
module parses.

- **Skip it in a test**: `SF.boot.reveal('qa')` lifts it immediately. `SF.boot.cleared` is `true` once
  the node is gone; `SF.boot.state` reports `phase`, `real`, `shown`, `streamDone`, `msSinceStart`
  and the policy constants. Anything that screenshots the city should wait for `SF.boot.cleared`.
- **The reveal gate** is whichever comes first: `REVEAL_CELLS` (340) cells streamed, or
  `REVEAL_GRACE_MS` (28 s) after the first rendered frame — plus 3 rendered frames and a `MIN_MS`
  (2.4 s) floor, with a hard `CAP_MS` (60 s) backstop. Measured: un-throttled the data gate wins at
  ~3.4 s with the city fully loaded; on Fast 4G the grace gate wins at ~37 s. It deliberately does
  NOT wait for all ~1590 cells — that is over two minutes on 4G.
- **A hidden tab never reveals on its own.** The reveal needs rendered frames, and the pause guard
  stops rAF when `document.hidden` (so the in-app preview pane, which runs the tab hidden, freezes
  the curtain mid-flight and its CSS transitions with it). Drive it with a real foregrounded Chrome
  (`--disable-backgrounding-occluded-windows --disable-renderer-backgrounding`) or call
  `SF.boot.reveal()`. Only the `CAP_MS` watchdog eventually fires while hidden.
- **The fog ships in tiers**, each a strict upgrade applied only once fully decoded (`boot.js`
  `upgradeFog`), signalled by a class on `#boot`: procedural SVG (0 bytes, always, paints with the
  stylesheet) → `has-doors` (two Cycles-rendered volumetric halves) → `has-wisps` (sparse foreground
  fragments for depth). Assets live in `app/public/boot/` (522 KB) and are regenerated by
  `artifacts/boot-fog/fog_render.py` (headless Blender, does NOT need the MCP addon).
- **Why the door plates are square.** They are stretched with `background-size: 100% 100%`, not
  `cover`. `cover` on a 16:9 plate blows up ~4x on a portrait phone: the billows crop away to a flat
  grey wash and the torn seam lands off screen, so the parting reads as two blank walls sliding.
  Square + stretch pins the seam at centre on every aspect for a distortion fog tolerates.
- **Two things that will silently break the parting** if they are reintroduced: driving the idle
  drift and the slide from the same element's `transform` (removing an animation and setting a
  transform in one recalculation does not fire a transition — the doors teleport), and fading rather
  than immediately clearing `#boot`'s background and `.boot-sky` (the gap then opens onto the
  curtain's own dark sky instead of the city). The doors travel 128%, not 104%: each is 104vw wide,
  its fog overhangs another 5%, and the swell grows both back toward centre.
- **Fallback drills** (rule 3) that must keep passing: delete `#boot` before boot → every method
  no-ops (`state.phase === 'absent'`) and the city loads; block `terrain.bin`/`manifest.json` → the
  curtain marks itself failed, lifts, and `main.js`'s "Failed to load the city" panel is readable;
  `prefers-reduced-motion: reduce` → all four SVG fog layers, the door drift, the wisps, the grid and
  the gloss report `animationName: none` (the doors still part, just faster and without the swell) (the per-layer `:nth-child` drift rules outrank a plain reset — the
  media query has to match their specificity); block `*/boot/karl-*` → no tier class is set, the SVG
  fog stays at full opacity, and the reveal is unaffected.
- **Regression check for "the fog did not fully clear"**: screenshot the last frame before teardown
  and the clean city after it, and diff. Desktop should be ~0% different; any standing percentage of
  changed pixels along a screen edge is leftover door.
- After the reveal the curtain contributes nothing: no GL objects, no timers,
  `document.querySelectorAll('[class^=boot-]').length === 0`.
