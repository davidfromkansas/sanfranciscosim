"""Fresh-scene contract validation for 164-south-park.glb.

    blender -b --python validate_164_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3949436, 37.7812107],
        "street_facet_headings_deg_true": [86.0, 91.1, 95.8, 100.6, 135.2],
        "long_axis_heading_deg_true": 315.1,
        "target_height_m": 5.4,
    ...
    results["checks"] = {
        # 37.2 x 36.5 x 5.4 m: the XY box is the ~45 deg rotation of a
        # 42 x 16 m wedge plus the canopy's 1.5 m projection, not a 37 m
        # building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            5.3 <= dims.z <= 5.5
            and 36.7 <= dims.x <= 37.7
            and 36.0 <= dims.y <= 37.0
        ),
        "crest_normalized_to_target": abs(mx.z - 5.4) <= 0.02,
