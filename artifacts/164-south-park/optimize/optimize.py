...
# coplanar ring bands. 164 South Park has two — the parapet and the coping both
# follow the whole 105 m footprint perimeter, so their top and bottom faces are
# perfect annuli. Even a strictly-coplanar dissolve merges each into one ngon,
# and re-triangulating an annulus emits slivers up to tens of metres long with
# ~0.2 mm width. Those pass an area-based degeneracy test, survive Blender's
# import (which recomputes loop normals), and then surface as
# invalid_or_nonunit_loop_normal_count in the gltfpack output — after the
# shipping swap. This asset is 3,366 triangles; the dissolve was worth a few
# dozen on 350-brannan and was reverted there for exactly this reason.
stats["limited_dissolve"] = "skipped: coplanar parapet + coping ring bands (prompt 3.3)"
