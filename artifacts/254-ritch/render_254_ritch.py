"""Controlled review renders of the exported 252-254 Ritch Street GLB.

    blender -b --python render_254_ritch.py -- [--glb FILE] [--out DIR]
                                            [--prefix 254-ritch] [--night]
...
opposite faces. Each view keeps the nearest compass filename:
    ...
    -north.png  RITCH STREET front, faces 45.05 deg - the public elevation: the
                two-storey canted bay, the twin-door entry recess, the stoop
    -east.png   the EXPOSED flank, faces 135.05 deg - the parking-lot side, and
                the reason three of this building's four sides are public
    -south.png  rear, faces 225.05 deg - the yard side, seen only from the air
    -west.png   party wall toward 248-250 Ritch, faces 315.05 deg. Blind.

Plus -facade.png, a square-on long-lens look at the 45.05 deg street front. None
of the four cardinal elevations shows the public face properly, because the
...
BG = (0.60, 0.56, 0.50, 1.0)  # neutral tabletop, kept a clear step DARKER
                              # than Toy_stone so the pale roof - which is the
                              # surface the app actually looks at - separates
                              # from the background instead of merging with it
...
VIEWS = [
    ("north", FRONT_AZ),          # Ritch Street front, the public face
    ("south", FRONT_AZ + 180.0),  # rear, toward the yard
    ("east", 135.05),             # the exposed flank toward the parking lot
    ("west", 315.05),             # party wall toward 248-250, blind
]
...
def fade_glow():
    ...
    for mat, bsdf in glow_materials():
        ...
        # Alpha alone is not enough in Cycles: make_material leaves Emission
        # Strength at 1.0 on every _Glow material, and an emissive surface keeps
        # emitting whatever its alpha says. The app's day state has no emission
        # at all - the glow layer is unlit and simply translucent - so the day
        # pass has to zero it or the lit sashes render cream in daylight.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "254-ritch.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "254-ritch")
    night = "--night" in argv
    ...
    # maps to world (-sin rz, cos rz); setting that equal to the front bearing
    # gives rz = LONG_AXIS + 90. Taking rz = LONG_AXIS - 90 instead renders the
    # plan upside down - the cornice, the bay and the stoop come out at the
    # bottom of the frame - which is the classic top-view roll error.
