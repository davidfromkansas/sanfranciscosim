...
# ring bands. This one has two - `curb_a` and `curb_b`, the light-well curbs,
# built by ring_band() as closed rectangular annuli. Their top and bottom faces
# are perfectly coplanar annuli, so even a strictly-coplanar dissolve merges each
# into one ngon, and re-triangulating an annulus emits slivers that pass every
...
# 350-brannan, 13 Aug 2026). The three cornice steps and the two bay bands are
# open arc_band() sweeps rather than rings, but the curbs alone are enough.
# At 3,140 triangles this step was worth single digits anyway.
stats["limited_dissolve"] = "skipped - closed ring bands (curb_a, curb_b)"
