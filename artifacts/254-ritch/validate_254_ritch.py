"""Fresh-scene contract validation for 254-ritch.glb.

    blender -b --python validate_254_ritch.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    for obj in meshes:
        ...
        # bay cheeks stand at 45 degrees to the front, so a strip on a cheek can
        # point well away from the model centre while being correctly outward. Casting a ray in along
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3956316, 37.7801280],
        "front_heading_deg_true": 45.05,
        "flank_heading_deg_true": 135.05,
    ...
    results["checks"] = {
        # ~16.2 x 16.2 x 8.8 m: the XY box is the 45.05 deg rotation of a
        # 7.60 x 14.20 m building plus its bay, cornice and stoop, not a 16 m
        # building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            8.7 <= dims.z <= 8.9
            and 15.0 <= dims.x <= 17.5
            and 15.0 <= dims.y <= 17.5
        ),
        "crest_normalized_to_target": abs(mx.z - 8.8) <= 0.02,
