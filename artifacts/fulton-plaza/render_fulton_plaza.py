"""Controlled review renders of the exported Civic Center Plaza GLB.
    ...
    blender -b --python render_fulton_plaza.py -- [--glb FILE] [--out DIR]
                                                  [--prefix fulton-plaza]
                                                  [--night]
...
runs 81.15 deg toward Hyde Street.
...
* the TOP view is the primary review image for this asset, not a supporting one
  - the plaza is 120 x 49 m and 12.8 m tall, so the plan IS the design. It is
  rendered larger than the elevations and must resolve into five things: two
  koi, the pale monument apron with the monument on it, the dark planted band
  on the museum side and the pale terrace on the library side.
* -axis.png is a low three-quarter looking WEST along the plaza, the real
  place's signature composition (City Hall would fill the far end).
...
night read to check for is two glowing koi circling a lit monument on a dark
field. If the koi do not read, the glow design is wrong.
"""
...
RES = (1900, 560)    # extreme letterbox: 128 m across and only 12.8 m tall
AER_RES = (1800, 1150)
TOP_RES = (2200, 1250)   # the primary review image; landscape, the plaza is E-W
...
def add_lights(height, floor, night=False):
    ...
    # A ground catcher gives the contact shadow that sells the miniature. It
    # has to sit at the model's own FLOOR, not at z=0: this asset is terrain
    # draped, so z=0 is the anchor's ground and min_z is -1.5 m. A catcher at
    # z=0 slices through the deck's skirt and renders as a cream slab lying
    # across the plaza — which is what the first aerial showed.
    # Sized from the model's PLAN extent, not its height: this asset is 128 m
    # across and 12.8 m tall, so the civic-center-plaza rig's `height * 5`
    # produced a 64 m table that was narrower than the plaza in one direction
    # and wider in the other, and rendered as a cream slab lying across the
    # south terrace in the first aerial.
    bpy.ops.mesh.primitive_plane_add(size=height * 3.0, location=(0, 0, floor - 0.02))
...
def main():
    ...
    out = arg("--out", here)
    prefix = arg("--prefix", "fulton-plaza")
    night = "--night" in argv
    ...
    add_lights(max(width, 40.0), mn.z, night=night)
    ...
    # Two koi, one pale round apron with the monument on it, one dark planted
    # band north, one pale terrace south. If the koi do not separate from the
    # asphalt at this size they will not separate in the app either.
    ...
    pitch = math.radians(42)  # 30-50 deg downward; steeper than a building
                              # needs, because on this asset the plan is the
                              # subject and a shallow angle hides it
    # NOT down the long axis: at 81 deg the camera looks straight along the
    # plaza and the two koi foreshorten into slivers. 152 deg puts it off the
    # south-south-east, over the library terrace, so the axis, both koi, the
    # apron and both flanks read at once.
    az = math.radians(152)
    r = span * 2.15
    aer.location = Vector(
        (
    ...
    # --- the signature composition: low three-quarter standing at the Hyde
    # (east) end and looking WEST along the plaza's own axis, past the monument.
    # City Hall would fill the far end; it is not part of this asset. This is
    # also the composition the registry camera preset (yaw 99) reproduces.
    ...
    axis.location = Vector(
        (
            center.x + r2 * math.cos(apitch) * math.sin(math.radians(81.15)),
            center.y + r2 * math.cos(apitch) * math.cos(math.radians(81.15)),
