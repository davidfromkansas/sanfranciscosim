...
# Phase B's steps are independently switchable so the four-variant table
# (weld x join, each packed with the repo-standard gltfpack) can be measured
# rather than assumed. On a flat-shaded asset the "coincident vertex pairs" the
# census counts ARE the shading topology, and welding them makes the exporter
# re-split into a worse arrangement; on a bevel-heavy one the weld can pay.
# There is no shortcut heuristic — run the table.
NO_WELD = "--no-weld" in argv
NO_JOIN = "--no-join" in argv
NO_DISSOLVE = "--no-dissolve" in argv
...
for o in mesh_objs():
    ...
    bm.free()
    if not NO_WELD:
        # remove_doubles fuses the split vertices that carry flat shading, so the
        # mesh silently falls back to smooth. G1/G2/G3/G5 all pass; only G4 sees
        # it. Guarded: calling shade_flat() when nothing was welded also destroys
        # the imported custom split normals and costs +8,600 exported vertices.
        o.data.shade_flat()
stats["weld"] = not NO_WELD
snap("weld+degenerate" if not NO_WELD else "degenerate-only(no-weld)")
...
for o in ([] if NO_DISSOLVE else mesh_objs()):
...
stats["dissolve"] = not NO_DISSOLVE
snap("limited-dissolve" if not NO_DISSOLVE else "limited-dissolve(skipped)")
