"""Controlled review renders of the exported Columbus Tower GLB.

    blender -b --python render_columbus_tower.py -- [--glb FILE] [--out DIR]
                                                    [--prefix columbus-tower]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the apex faces NNW.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "columbus-tower.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "columbus-tower")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(330)  # from the NNW: the apex is the identity
        r = height * 3.4
        aer.location = Vector(
            (
    ...
    # --- top view: the wedge plan, cornice, turret and roof plant ----------
    top = make_camera("cam_top")
    top.data.type = "ORTHO"
    top.data.ortho_scale = width * 1.35
    ...
    az = math.radians(330)  # from the NNW — looking straight at the apex
    r = height * 4.0
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
