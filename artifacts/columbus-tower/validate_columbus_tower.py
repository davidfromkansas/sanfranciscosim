"""Fresh-scene contract validation for columbus-tower.glb.

    blender -b --python validate_columbus_tower.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "normal_orientation_method": (
            ...
            "reimported loop normals must be finite/unit; 31,500 deterministic "
            "visibility rays test the first visible face from nine interior targets."
        ),
        ...
        "anchor_lonlat": [-122.4050266, 37.7965554],
        "apex_heading_deg_true": 330.4,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        "meters_and_plausible_dimensions": 28.0 <= dims.z <= 30.0
        and 15.0 <= dims.x <= 22.0
        and 14.0 <= dims.y <= 21.0,
