"""Controlled review renders of the exported 155 - 157 South Park Street GLB.

    blender -b --python render_155_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 155-south-park]
                                                    [--night]
...
Street front faces NNW (327.2 deg).
...
RES = (1500, 1000)   # landscape: the lot is 31 m long and only 10 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "155-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "155-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(337)  # from the NNW: the shopfront glow is the identity
        r = span * 4.3
        aer.location = Vector(
            (
    ...
    pitch = math.radians(25)  # low end of the app band: the lot is 31 m long
    az = math.radians(337)  # from the NNW, 10 deg off square so a flank reads too
    r = span * 4.3
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
