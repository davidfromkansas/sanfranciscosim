...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "asian-art-museum.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "asian-art-museum")
    only = arg("--only", "")
    ...
    az = math.radians(238)  # SW, looking across the Fulton arcade to the Larkin front
    r = span * 3.6
    aer.location = Vector(
        (
    ...
    az_n = math.radians(268)  # standing on Civic Center Plaza, facing the Larkin front
    rn = span * 2.05
