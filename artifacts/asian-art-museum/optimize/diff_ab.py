...
# Reads renders/A_*.png and renders/B_*.png, writes renders/diff_*.png
...
out = {}
for tag in PAIRS:
    a = Image.open(f"renders/A_{tag}.png").convert("RGBA")
    b = Image.open(f"renders/B_{tag}.png").convert("RGBA")
    pa, pb = a.load(), b.load()
    w, h = a.size
    total = n = 0
    maxd = 0
...
cell = Image.open("renders/A_elev_n.png")
cw, ch = cell.size
cw2, ch2 = cw // 2, ch // 2
sheet = Image.new("RGB", (cw2 * 4, ch2 * 3 + 24), (24, 24, 28))
d = ImageDraw.Draw(sheet)
d.text((8, 4), "Chase Center A/B — rows: input / optimized / diff x8 — cols: N E S W",
       fill=(220, 220, 220))
for r, pref in enumerate(["renders/A_", "renders/B_", "renders/diff_"]):
