"""Fresh-scene contract validation for asian-art-museum.glb.

    blender -b --python validate_asian_art_museum.py -- [--glb FILE] [--out FILE]
                                                    [--closed-solids]
...
(`optimize/input/asian-art-museum.glb`) to additionally assert that every object is
...
def main():
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": (
            # The 106.60 x 54.71 m surveyed envelope rotated 8.32 deg onto the
            # Civic Center grid, plus the cornice and colonnade projections.
            110 <= dims.x <= 120 and 60 <= dims.y <= 70 and abs(dims.z - TARGET_HEIGHT) <= 0.05
        ),
