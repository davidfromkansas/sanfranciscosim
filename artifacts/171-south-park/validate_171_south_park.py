"""Fresh-scene contract validation for 171-south-park.glb.

    blender -b --python validate_171_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3945219, 37.7809],
        "front_heading_deg_true": 343.5,
        "target_height_m": 12.6,
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 12.4 <= dims.z <= 12.8
        and 18.0 <= dims.x <= 20.5
        and 17.5 <= dims.y <= 19.5,
        "crest_normalized_to_target": abs(mx.z - 12.6) <= 0.02,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        # The origin is the footprint's AREA CENTROID, which is the point the
        # loader places at the manifest anchor. On a wedge that is not the
        # bounding-box centre: the mass is concentrated at the broad park front,
        # so the bbox centre sits ~1.4 m south of the origin. That offset is
        # correct and must not be "fixed" by recentring on the bbox, which would
        # push the building off its own lot. Budget: 2 m.
        "origin_at_footprint_area_centroid": abs(center.x) <= 2.0
        and abs(center.y) <= 2.0,
