"""Controlled review renders of the exported 171 South Park Street GLB.

    blender -b --python render_171_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 171-south-park]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the park front
faces NNW (343.5 deg), so the 'north' elevation is the hero.
...
RES = (1400, 1150)   # this building is nearly as tall as it is wide
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    ...
    for mat, bsdf in glow_materials():
        if bsdf:
            # 6.0 clipped every lit window to pure white, which is the exact
            # failure the plans README warns about — the app's night layer is an
            # unlit overlay at the material's own baked colour, so the preview
            # has to keep that colour rather than blow past it.
...
def render_to(path, cam, res):
    # Cycles on CPU takes minutes a frame, and a rig that cannot be resumed
    # means one interrupted run throws away everything it had already produced.
    if SKIP_EXISTING and os.path.exists(path):
        print(f"[render] skip (exists) {path}")
        return
    scene = bpy.context.scene
    scene.camera = cam
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "171-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "171-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(343.5)  # from the NNW: square onto the park front
        r = span * 4.2
        aer.location = Vector(
            (
    ...
    # --- top view: THE acceptance render — the wedge plan, cornice ring,
    # skylight row, mechanical box and rear deck ---------------------------
    ...
    az = math.radians(343.5)  # from the NNW — square onto the park front
    r = span * 4.2  # 3.1 cropped this subject: tall for its plan
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
            center.y + r * math.cos(pitch) * math.cos(az),
