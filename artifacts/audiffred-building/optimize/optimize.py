...
# GLB-OPTIMIZE-PROMPT v2 s.3 step 3 says to skip the dissolve entirely on assets
# with large coplanar ring bands that follow the footprint all the way round.
# This one has FOUR: the entablature (5.35-6.15 m), the corbel table
# (10.55-10.95 m), the mansard crown moulding (15.10-15.55 m) and the party-wall
# firewall. Their top and bottom faces are perfectly coplanar annuli, so even a
# strictly-coplanar dissolve merges each ring into a single annulus ngon, and
# re-triangulating an annulus emits slivers - measured on 350-brannan as
# triangles up to 24.35 m long and ~0.24 mm wide. Those slivers pass an
# area-based degeneracy test and only surface after the shipping swap, as
# invalid_or_nonunit_loop_normal_count in the stage-2 contract validator, and
# only in the PACKED file (Blender recomputes loop normals on import; gltfpack
# re-emits the stored ones). On 350-brannan the dissolve was worth 0.4%; the
# rings here are a bigger share of the model than they were there.
