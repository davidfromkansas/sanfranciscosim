"""Fresh-scene contract validation for audiffred-building.glb.
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3927766, 37.7933230],
        "front_heading_deg_true": 315.2,
        "end_heading_deg_true": 45.2,
        "party_wall_heading_deg_true": 135.2,
        "target_height_m": 17.5,
    ...
    results["checks"] = {
        # ~40.4 x 40.3 x 17.5 m: the XY box is the 45.2 deg rotation of a
        # 41.82 x 14.00 m SLAB, so it comes out nearly SQUARE. That is expected
        # here and is the single most likely thing to be misread as a scale
        # error - check the footprint along the building's own axes instead. See
        # REPORT.md.
        "meters_and_plausible_dimensions": (
            17.4 <= dims.z <= 17.6
            and 39.5 <= dims.x <= 41.5
            and 39.5 <= dims.y <= 41.5
        ),
        "crest_normalized_to_target": abs(mx.z - 17.5) <= 0.02,
