"""Fresh-scene contract validation for towers-at-rincon.glb.

    blender -b --python validate_towers_at_rincon.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3924907, 37.7919910],
        "front_heading_deg_true": 135.0,
        "flank_heading_deg_true": 45.0,
        "long_axis_heading_deg_true": 315.0,
        "target_height_m": 89.0,
    ...
    results["checks"] = {
        # ~108.7 x 108.7 x 89.0 m: the XY box is the axis-aligned box of a
        # diamond block whose sides are 73-89 m, not a 109 m building. See
        # REPORT.md.
        "meters_and_plausible_dimensions": (
            88.9 <= dims.z <= 89.1
            and 104.0 <= dims.x <= 113.0
            and 104.0 <= dims.y <= 113.0
        ),
        "crest_normalized_to_target": abs(mx.z - 89.0) <= 0.02,
