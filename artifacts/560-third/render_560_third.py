"""Controlled review renders of the exported 560 Third Street GLB.

    blender -b --python render_560_third.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 560-third]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Third Street front faces NE (44.1 deg).
...
RES = (1500, 640)    # landscape: 24 m long and only 7.2 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    ...
    The glTF round-trip drops the authored emission colour (the asset exports
    with strength 0, so emissiveFactor is black and the importer leaves the
    socket white). Driving strength alone therefore emits pure WHITE and throws
    away the whole point of a warm band against cool skylights — so re-seed the
    emission colour from each material's own base colour first.
    """
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "560-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "560-third")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(45)   # from the NE: the Third Street front is the identity
        r = span * 3.6
        aer.location = Vector(
            (
        ...
        aim(aer, Vector((center.x, center.y, center.z)))
    ...
    # --- top view: the hero image — parapet ring, two skylights, plant cluster ----
    ...
    az = math.radians(45)   # from the NE — square onto the Third Street front
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
