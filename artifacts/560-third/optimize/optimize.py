...
#   3. (SKIPPED) limited dissolve — coplanar parapet/coping ring bands, prompt s.3.3
#   4. (skipped) curve retess — the only curves are two 8-segment vent cylinders
...
# large coplanar ring bands". 560 Third has TWO — the parapet ring and the
# steel coping ring, both closed annuli following the whole footprint, plus a
# third annulus where the membrane field is inset from the parapet. Even a
# strictly-coplanar dissolve merges each into one ngon whose retriangulation
# emits slivers that pass every area-based degeneracy test and only surface in
# the packed file as invalid_or_nonunit_loop_normal_count. The measured saving
# on 350-brannan was 0.4%; here the geometry is smaller still. Not worth it.
stats["steps"].append({"step": "limited-dissolve", "skipped":
                       "coplanar parapet/coping/membrane ring bands (prompt s.3.3)"})
