"""Controlled review renders of the exported Salesforce Tower GLB.
    ...
    blender -b --python render_coit_tower.py -- [--glb FILE] [--out DIR]
                                                      [--prefix coit-tower]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "coit-tower.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "coit-tower")
    ...
    az = math.radians(330)  # camera NNW of the tower, looking back at the entrance quadrant
    r = height * 4.2
    aer.location = Vector(
        (
