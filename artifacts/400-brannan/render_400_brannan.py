"""Controlled review renders of the exported 400 Brannan Street GLB.
...
(north = Blender +Y), which is how the asset is authored — the Third front faces NE (45.2 deg) and the Brannan front SE (135.2 deg).
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "400-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "400-brannan")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(90)   # from the east: square onto the CORNER, which is the identity
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, skylights, HVAC pair and stair penthouse ----------
    ...
    az = math.radians(90)   # from the east — both street elevations at once
    r = span * 3.1
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
