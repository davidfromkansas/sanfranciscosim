"""Controlled review renders of the exported Asian Art Museum GLB.

    blender -b --python render_earl_warren_building.py -- [--glb FILE] [--out DIR]
                                                  [--prefix earl-warren-building]
                                                  [--only aerial]
...
Asian Art Museum is a wide, low building, so the rig is framed on the plan extent
rather than the height, and the aerial sits WNW to look down the entry axis.
"""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "earl-warren-building.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "earl-warren-building")
    only = arg("--only", "")
    ...
    # WNW of the arena, looking down the entry axis at the sail crest.
    ...
    az = math.radians(203)  # SSW, across the McAllister arcade with the roof open
    r = span * 3.1
    aer.location = Vector(
        (
    ...
    az_n = math.radians(186)  # standing on Civic Center Plaza, facing the entrance arches
    rn = span * 1.35
    ...
    render_to(os.path.join(out, f"{prefix}-night-entrance.png"), ncam, (1500, 900))
