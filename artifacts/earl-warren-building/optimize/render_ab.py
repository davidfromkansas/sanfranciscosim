...
# 24 denoised samples, applied identically to BOTH sides of the A/B, so the
# comparison stays valid. The asset is flat-shaded Lambert with no glossy or
# transmissive materials; denoising keeps the noise floor below the G4 delta
# gates. 64 samples was not completing on a machine shared with ~17 other
# Blender jobs.
scene.cycles.samples = 24
# Denoising ON (the generic script has it off for a pure pixel comparison at 64
# samples). At 24 samples an undenoised noise floor would swamp the G4 deltas;
# OIDN is deterministic and both sides share a sampling seed, so identical
# geometry still denoises identically.
