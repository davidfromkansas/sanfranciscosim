"""Fresh-scene contract validation for earl-warren-building.glb.

    blender -b --python validate_earl_warren_building.py -- [--glb FILE] [--out FILE]
                                                    [--closed-solids]
...
(`optimize/input/earl-warren-building.glb`) to additionally assert that every object is
...
def main():
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": (
            ...
            114 <= dims.x <= 124 and 45 <= dims.y <= 55 and abs(dims.z - TARGET_HEIGHT) <= 0.05
        ),
