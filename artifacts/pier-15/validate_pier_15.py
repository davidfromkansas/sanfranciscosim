"""Fresh-scene contract validation for pier-15.glb.

    blender -b --python validate_pier_15.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3974662, 37.8016046],
        "front_heading_deg_true": 234.9,
        "pier_axis_deg_true": 54.9,
        "target_height_m": 16.4,
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 16.2 <= dims.z <= 16.6
        and 246.0 <= dims.x <= 252.0
        and 218.0 <= dims.y <= 224.0,
        "crest_normalized_to_target": abs(mx.z - 16.4) <= 0.02,
        ...
        # real-world anchor. It is not the AABB centre: the deck flares to the
        # full historic bulkhead-wharf width at the seawall and carries the wide
        # south apron, so the two differ by ~5 m over a 249 m asset (2%). Moving
        # the origin to the AABB centre would place the model 5 m off its
        # surveyed position to satisfy a tolerance written for compact buildings.
        "centered_xy": abs(center.x) <= 5.5 and abs(center.y) <= 3.5,
