...
# --- 3. limited dissolve: SKIPPED for pier-15 ---
# GLB-OPTIMIZE-PROMPT v2 §3.3: skip this step on assets with large coplanar ring
# bands. Pier 15 has eight bands following a ~510 m perimeter (deck slab, deck
# surface, fender curb, railing ribbon, bulkhead cornice, parapet, parapet cap,
# office parapet). Their top and bottom faces are coplanar annuli, so even a
# strictly-coplanar dissolve merges each into one ngon, and re-triangulating an
# annulus emits slivers that pass every area-based degeneracy test and only
# surface after the shipping swap as nonunit STORED normals in the packed file.
# The step is the cheapest in Phase B and the only one that can manufacture new
# degenerate geometry, so it is not worth the risk here.
stats["limited_dissolve"] = "skipped (coplanar ring bands, prompt 3.3)"
