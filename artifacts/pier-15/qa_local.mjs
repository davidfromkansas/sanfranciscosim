// Stage-5 local QA for pier-15, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/pier-15/qa_local.mjs [--drill]
//
...
const LON = -122.3974662;
const LAT = 37.8016046;
const SLUG = 'pier-15';
const CAMERA = { distance: 460, yaw: 215, pitch: 20 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qaPier15-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('pier-15') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasP15: window.SF.assets.placed.has('pier-15'),
    placeP15: window.SF.assets.placed.get('pier-15') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('pier-15').log, scale: window.SF.assets.placed.get('pier-15').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedP15:', placed.hasP15, JSON.stringify(placed.placeP15));
  ...
  // Pier 15 is the first over-water landmark that is not a bridge. placeGeneric
  ...
  if (!DRILL) {
    const seat = await evaluate(`(() => {
      const p = window.SF.assets.placed.get('pier-15');
  ...
  if (DRILL) {
    ...
    // single-shot warn(): expect `sf-assets: pier-15 failed to load (...)` with no
