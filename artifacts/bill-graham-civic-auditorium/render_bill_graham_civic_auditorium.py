"""Controlled review renders of the exported Bill Graham Civic Auditorium GLB.

    blender -b --python render_war_memorial_opera_house.py -- [--glb FILE] [--out DIR]
                                                     [--prefix bill-graham-civic-auditorium]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "bill-graham-civic-auditorium.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "bill-graham-civic-auditorium")
    ...
    top.rotation_euler = (0, 0, math.radians(-80.69) + math.pi / 2)
    ...
    aer.data.type = "PERSP"
    aer.data.lens = 78.0               # long lens, restrained perspective (s.18)
    pitch = math.radians(38)           # 30-50 deg downward
    az = math.radians(23)              # from the NNE: Grove arcade + the dome
    r = span * 3.15
    aer.location = Vector(
        (
        ...
        if mat.name.endswith("_Glow") and mat.use_nodes:
            ...
            if bsdf:
                ...
                bsdf.inputs["Emission Strength"].default_value = {
                    "Toy_white_Glow": 3.6,      # the marquee bulb band
                }.get(mat.name, 2.4)            # the three lit arch panes
    render_to(os.path.join(out, f"{prefix}-night.png"), aer, AER_RES)

    # night closeup: the three lit arches and the marquee band from Grove St
    ...
    aim(ncam, Vector((center.x, center.y, mn.z + height * 0.30)))
    render_to(os.path.join(out, f"{prefix}-night-front.png"), ncam, (1500, 1000))
