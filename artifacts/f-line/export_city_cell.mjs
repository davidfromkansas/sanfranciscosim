...
//
//   node export_city_cell.mjs [--cell 22_11] [--out city-cell.json]
//
...
//
// Default cell 22_9 is the EMBARCADERO around Green Street - F-line track on
// the Fisherman's Wharf leg, flat, 456 real baked buildings and NOT ONE of them
// over 60 m.
//
// Lower Market (22_11) was the first choice and had to be abandoned: it is the
// right piece of city, but 79 of its 142 buildings clear 60 m, and at the app's
// 42-degree camera any azimuth more than about 8 degrees off the street axis
// puts the camera INSIDE a tower - the render came back as a flat grey wall
// twice. The Embarcadero gives the same F-line track with a sightline.
...
const cell = arg('--cell', '22_9');
const bearing = Number(arg('--bearing', '10'));   // the street the car stands on
...
async function findPlacement(key) {
  ...
  for (let s = 0; s < d.count; s++) {
    ...
    for (let k = 0; k + 1 < d.ptCount[s]; k++) {
      ...
      // The F line runs on flat arterials - Market and the Embarcadero - so
      // unlike the cable car's exporter this one wants the LONGEST NEAR-LEVEL
      // segment near the cell centre, not a chosen grade. Length matters as
      // much as flatness: a 23.6 m vehicle standing on a 10 m stub straddles
      // two intersections and the scale test reads as a modelling error.
      // The line's own street is identified by BEARING: every cross street in
      // the waterfront grid is 90 degrees off it. Score = off-bearing, then
      // flatness, then distance from the cell centre. Segment LENGTH is not
      // scored - the baked centrelines are chopped into ~9 m pieces, so it
      // says nothing about the street.
      const grade = Math.abs((rise / run) * 100);
      let off = Math.abs(((Math.abs(h) * 180) / Math.PI) - bearing);
      off = Math.min(off, Math.abs(180 - off));
      const score = off * 2 + grade * 3 + dist / 20;
      const row = {
        score,
        gradeAbs: grade,
        dist,
