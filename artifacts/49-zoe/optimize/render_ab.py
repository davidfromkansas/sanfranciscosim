...
# EEVEE, not Cycles: this machine runs a dozen concurrent landmark sessions and
# was at load 250 during this pass, where a single CPU-Cycles frame takes minutes.
# The A/B gate compares two renders of ONE rig against each other, so the engine
# only has to match on both sides — and EEVEE is deterministic and noise-free,
# which makes any non-zero pixel delta a real difference rather than sampling
# noise. In Blender 5.2 the enum is BLENDER_EEVEE, not BLENDER_EEVEE_NEXT.
