...
#   4. (skipped) curve retess — the six 10-segment vent cans could halve to 5
#      (chord error 0.0098 m against a 0.035 m one-pixel budget at the 51.9 m
#      near distance) but that is ~350 tris of 7,688 and the penthouse vent at
#      r=0.75 is already at the budget. Not worth the silhouette risk.
...
# coplanar ring bands following the footprint. 49 Zoe has two — `parapet`
# (33.81 x 33.90 x 0.68 m) and `coping` (34.05 x 34.14 x 0.12 m), both closed
# annuli built by ring_band() around the whole 28.24 x 19.78 m plan. Their top
# and bottom faces are perfectly coplanar annuli, so even a strictly-coplanar
# 0.05-deg dissolve merges each into one ngon, and re-triangulating an annulus
# emits slivers up to the full 34 m diagonal with ~0.2 mm width. Those pass an
...
# sliver's shared vertex normal averages to ~0. Measured on 350-brannan
# (13 Aug 2026) at a cost of 30 triangles saved. Not worth it here either: this
# asset's win is node overhead, not triangles.
