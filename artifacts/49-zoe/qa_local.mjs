// Stage-5 local QA for 49-zoe, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/49-zoe/qa_local.mjs [--drill]
//
...
const LON = -122.3960338;
const LAT = 37.7800764;
const SLUG = '49-zoe';
const CAMERA = { distance: 180, yaw: 338, pitch: 28 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa49zoe-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('49Zoe') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    has340: window.SF.assets.placed.has('49Zoe'),
    place340: window.SF.assets.placed.get('49Zoe') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('49Zoe').log, scale: window.SF.assets.placed.get('49Zoe').scale })) : null,
  }))()`);
