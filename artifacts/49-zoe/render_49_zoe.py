"""Controlled review renders of the exported 49 Zoe Street GLB.

    blender -b --python render_49_zoe.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 49-zoe]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Zoe Street front faces SW (225.4 deg).
...
def setup_world(night=False, samples=64):
    scene = bpy.context.scene
    # EEVEE, not Cycles: David's Mac routinely runs a dozen concurrent landmark
    # sessions and at load ~200 a single 1200x1000 CPU-Cycles frame takes over
    # two minutes and often never finishes. Nothing gate 2/3 judges — silhouette,
    # massing, the stripe rhythm, which surfaces glow — needs path tracing, and
    # the stage-4 A/B gate compares two renders of ONE rig, so the engine only
    # has to match on both sides. In Blender 5.2 the enum is BLENDER_EEVEE.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "49-zoe.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "49-zoe")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    setup_world(night=night, samples=samples)
    ...
    if night:
        ...
        az = math.radians(182)  # from the S: catches BOTH finished faces at once
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, monitor spine, skylights, penthouse, deck ----
...
def render_aerial(center, span, out, prefix):
    ...
    az = math.radians(182)  # from the S — the Zoe elevation AND the parking-lot
    # flank in one frame, which is what the app's camera gets. This building's
    # two exposed faces meet at its SOUTH corner, so the hero aerial comes from
    # the south; anything past ~215 deg puts the blind party wall in shot.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
