"""Controlled review renders of the exported Pier 3 GLB.

    blender -b --python render_pier_3.py -- [--glb FILE] [--out DIR]
                                                    [--prefix pier-3]
                                                    [--night]
...
RES = (1800, 700)    # very wide: a 196 m pier only 18.5 m tall
AER_RES = (1600, 1100)
TOP_RES = (1500, 1250)
...
def main():
    ...
    for name, az in VIEWS:
        ...
        cam.data.type = "ORTHO"
        cam.data.ortho_scale = 78.0 if name == "frontage" else ortho_scale
        a = math.radians(az)
        cam.location = Vector(
            (center.x + dist * math.sin(a), center.y + dist * math.cos(a), center.z)
        )
        if name == "frontage":
            focus = Vector((0.0, 0.0, 9.0))
            a2 = math.radians(233.92)
    ...
    if water:
        ...
        az = math.radians(126)   # from the southeast slip, three-quarter to the bulkhead
        r = span * 1.55
        low.location = Vector(
            (
