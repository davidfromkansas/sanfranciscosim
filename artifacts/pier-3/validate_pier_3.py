"""Fresh-scene contract validation for pier-3.glb.

    blender -b --python validate_pier_3.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "pier-3.glb"))
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3947017, 37.7982322],
        "front_heading_deg_true": 233.92,
        "pier_axis_deg_true": 53.92,
        "target_height_m": 18.5,
        "datum": "Z=0 is the waterline (over-bay asset; placeGeneric seats it at max(0, sampleElevation))",
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 18.3 <= dims.z <= 18.7
        and 193.0 <= dims.x <= 199.0
        and 158.0 <= dims.y <= 164.0,
        "crest_normalized_to_target": abs(mx.z - 18.5) <= 0.02,
        # Z=0 is the waterline and the pile feet reach it exactly, so this is a
        # tighter test here than on a land asset, not a looser one.
        "base_at_z_zero": abs(mn.z) <= 0.05,
        # The origin is the pier polygon's AREA centroid, which is the honest
        # real-world anchor. It is not the AABB centre: the pier flares from 40 m
        # across at the head to 53.5 m at the bulkhead, so the two differ by
        # ~2.7 m over a 196 m asset (1.4%). Moving the origin to the AABB centre
        # would place the model 2.7 m off its surveyed position to satisfy a
        # tolerance written for compact buildings.
        "centered_xy": abs(center.x) <= 3.5 and abs(center.y) <= 3.5,
