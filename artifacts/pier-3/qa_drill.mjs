// Stage-5 local QA for pier-3, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/pier-3/qa_local.mjs [--drill]
//
...
const LON = -122.3947017;
const LAT = 37.7982322;
const SLUG = 'pier-3';
const CAMERA = { distance: 430, yaw: 215, pitch: 20 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qaPier3-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('pier-3') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasPier3: window.SF.assets.placed.has('pier-3'),
    placePier3: window.SF.assets.placed.get('pier-3') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('pier-3').log, scale: window.SF.assets.placed.get('pier-3').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedPier3:', placed.hasPier3, JSON.stringify(placed.placePier3));
  ...
  const draws = -1;  // drill-only: no GPU work
  ...
  if (DRILL) {
    ...
    // single-shot warn(): expect `sf-assets: pier-3 failed to load (...)` with no
  ...
  await writeFile(path.join(SHOTS, DRILL ? 'drill.json' : 'drill.json'),
