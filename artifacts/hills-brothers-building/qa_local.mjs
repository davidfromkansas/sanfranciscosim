// Stage-5 local QA for hills-brothers-building, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/hills-brothers-building/qa_local.mjs [--drill]
//
...
const LON = -122.3892854;
const LAT = 37.7894167;
const SLUG = 'hills-brothers-building';
const CAMERA = { distance: 480, yaw: 45, pitch: 24 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qahills-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('hillsBrothersBuilding') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasHills: window.SF.assets.placed.has('hillsBrothersBuilding'),
    placeHills: window.SF.assets.placed.get('hillsBrothersBuilding') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('hillsBrothersBuilding').log, scale: window.SF.assets.placed.get('hillsBrothersBuilding').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedHills:', placed.hasHills, JSON.stringify(placed.placeHills));
  console.log('  placedIds:', placed.placedIds.join(','));
  console.log('  console lines:', console_.length);
  for (const l of console_) if (/hills|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
