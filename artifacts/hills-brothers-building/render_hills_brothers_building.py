"""Controlled review renders of the exported 375 Alabama Street GLB.

    blender -b --python render_hills_brothers_building.py -- [--glb FILE] [--out DIR]
                                                    [--prefix hills-brothers-building]
                                                    [--night]
...
front (the sign, the arched base) faces south-east, 135 deg; the campanile
projects north-west toward the plaza.
...
RES = (1600, 1000)   # the quad is 85 m wide, crest 53.2 m
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    """Preview the app's night pass: the app's night layer is unlit geometry
    drawn at the material's own baked colour, and the glTF round-trip writes
    emissiveFactor 0 (which re-imports as default WHITE emission) — so copy
    Base Color into Emission Color before raising the strength, or every glow
    renders as a white slab (docs/asset-plans/README.md, night renders)."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "hills-brothers-building.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "hills-brothers-building")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(135)  # from the SE over the bay: facade, sign and tower
        r = span * 3.35
        aer.location = Vector(
            (
    ...
    # --- top view: sawtooth field, flat north membrane, parapet ring, tower ---
    ...
    az = math.radians(135)  # from the SE over the bay — the Embarcadero facade,
    # the sign, the roofscape and the campanile at once (plan Part 1)
    r = span * 3.35
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
