"""Controlled review renders of the exported 45-49 South Park GLB.

    blender -b --python render_49_south_park.py -- [--glb FILE] [--out DIR]
                                               [--prefix 49-south-park] [--night]
...
FRONT_AZ = 315.8         # the South Park elevation faces north-west
LONG_AXIS = 135.8        # front -> rear bearing

# Azimuths are the bearings each elevation FACES, so the camera stands on that
# side looking back at the building.
VIEWS = [
    ("north", FRONT_AZ),          # South Park front, the public face
    ("south", FRONT_AZ + 180.0),  # rear, toward the Masonic Temple
    ("west", 225.8),              # Jack London Alley flank - the second hero
    ("east", 45.8),               # party wall toward 41-43, blind
]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "49-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "49-south-park")
    night = "--night" in argv
    ...
    top.data.type = "ORTHO"
    top.data.ortho_scale = 24.0   # portrait frame: this is the VERTICAL extent
