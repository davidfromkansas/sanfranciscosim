...
#
# The generic script runs a 0.05 deg limited dissolve here. This asset is
# exactly the case GLB-OPTIMIZE-PROMPT s.3 step 3 tells you to skip it on, and
# it is a worse case than the 350 Brannan one that produced that rule.
#
# 45-49 South Park's cornice is THREE `rim` solids stacked in z, each following
# the full bay outline all the way round the building - 60+ vertices apiece,
# with top and bottom faces that are large, perfectly coplanar annuli. A
# strictly-coplanar dissolve merges each of those into a single annulus ngon,
# and re-triangulating an annulus emits slivers: invisible, area-test-clean, and
# fatal two steps later, because a sliver's shared vertex sits between opposing
# normals, its averaged vertex normal collapses to ~0, and gltfpack re-emits the
# STORED normals so the stage-2 validator fails on
# `invalid_or_nonunit_loop_normal_count`.
#
# The bracket shelf under every bay and the water-table rim are three more ring
# bands on the same footprint, so there are six of them here against 350
# Brannan's two.
#
# Measured cost of skipping: see phaseb_stats.json. The step was worth 30
# triangles on 350 Brannan; there is no reason to expect more here, and no
# reason to manufacture degenerate geometry for it.
