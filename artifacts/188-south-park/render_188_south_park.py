"""Controlled review renders of the exported 188 South Park GLB.

    blender -b --python render_188_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 188-south-park]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "188-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "188-south-park")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(315)  # three-quarter from the SE (park side)
        r = span * 4.4
        aer.location = Vector(
            (
    ...
    # 315 deg looks from the SE (park side) — three-quarter to the building's
    # 135 deg front axis: the park front, the penthouse and the SW flank all
