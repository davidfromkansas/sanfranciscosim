"""Fresh-scene contract validation for 188-south-park.glb.

    blender -b --python validate_188_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3950794, 37.7810118],
        "front_heading_deg_true": 134.8,
        "target_height_m": 15.93,
    ...
    results["checks"] = {
        # The building is 23.7 x 16.1 m at a 45 deg heading, so its
        # axis-aligned box is ~28 x 28 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 15.7 <= dims.z <= 16.1
        and 27.0 <= dims.x <= 30.0
        and 27.0 <= dims.y <= 30.0,
        "crest_normalized_to_target": abs(mx.z - 15.93) <= 0.02,
