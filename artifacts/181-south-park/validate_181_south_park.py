"""Fresh-scene contract validation for 181-south-park.glb.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "181-south-park.glb"))
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3945113, 37.7807582],
        "front_heading_deg_true": 315.2,
        "target_height_m": 16.5,
    ...
    results["checks"] = {
        # The building is 43.2 x 13.8 m at a 135.2 deg heading, so its
        # axis-aligned box is ~40.8 x 40.5 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 16.3 <= dims.z <= 16.7
        and 39.0 <= dims.x <= 42.5
        and 39.0 <= dims.y <= 42.5,
        "ridge_normalized_to_target": abs(mx.z - 16.5) <= 0.02,
