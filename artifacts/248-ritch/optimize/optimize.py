...
# coplanar ring bands, and 248-ritch is made of them: the cornice dentil and
# crown run the whole street front and both returns, three parapet upstands ring
# the deck, and the water table, the entry hood, its dentil and the two bay caps
# are all long coplanar strips. Their top and bottom faces are perfectly
# coplanar annuli, so even a strictly-coplanar dissolve merges each into one
# ngon, and re-triangulating an annulus emits slivers up to tens of metres long
# with sub-millimetre widths.
#
# Those slivers pass an area-based degeneracy test, so nothing in Phase B or E
# catches them. What catches them is the stage-2 contract validator, two steps
# later and after the shipping swap: a sliver's shared vertex sits between faces
...
# normals, so the failure surfaces only in the packed file.
#
# On 350-brannan this step was worth 30 triangles out of 6,770 (0.4%) and was
# reverted. This asset is 3,572 triangles with proportionally more band geometry,
# so the upside is smaller still. Not worth the risk; not run.
stats["limited_dissolve"] = "skipped: coplanar ring bands (cornice, parapets, water table, hood, bay caps)"
