...
# 32, not the generic 64. This machine is running four concurrent CPU Cycles
# jobs from sibling pipeline sessions at load 150-260, and the A/B pass is 16
# frames. Denoising stays OFF - it is the pair's DIFFERENCE that is measured,
# and a denoiser would smooth away exactly the small shading changes the gate
# exists to catch. The cost is a slightly higher noise floor in the mean delta,
# which is recorded in the report and is well inside the 2%/4% gates.
