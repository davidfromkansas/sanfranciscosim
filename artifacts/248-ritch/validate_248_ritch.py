"""Fresh-scene contract validation for 248-ritch.glb.

    blender -b --python validate_248_ritch.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    for obj in meshes:
        ...
        #
        # NOT a dot product against the model centre. Two of the six lit panes
        # sit on the bay's 45 deg returns and two more sit inside the entry
        # recess, facing out of a pocket; a centre-dot test mis-scores both.
        # Casting a ray in along each face's own normal and requiring that face
        # to be the first thing hit is both stricter and correct.
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3956749, 37.7801751],
        "front_heading_deg_true": 45.05,
        "exposed_flank_heading_deg_true": 315.05,
        "long_axis_heading_deg_true": 225.05,
        "target_height_m": 8.6,
    ...
    results["checks"] = {
        # ~15.8 x 15.8 x 8.6 m: the XY box is the 45.05 deg rotation of a
        # 7.60 x 13.90 m building plus the two stoops that project 1.20 m onto
        # the pavement, not a 15.8 m building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            8.5 <= dims.z <= 8.7
            and 15.0 <= dims.x <= 16.5
            and 15.0 <= dims.y <= 16.5
        ),
        "crest_normalized_to_target": abs(mx.z - 8.6) <= 0.02,
