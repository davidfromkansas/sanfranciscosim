"""Controlled review renders of the exported 226 Ritch Street GLB.

    blender -b --python render_226_ritch.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 226-ritch]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
(north = Blender +Y), which is how the asset is authored — the Ritch Street
front faces NE (45.6 deg).
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "226-ritch.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "226-ritch")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(70)  # same three-quarter as the day aerial: the lit
        # loft windows are all on the Ritch Street front, so any other azimuth
        # renders a dark box and proves nothing.
        r = span * 4.4
        aer.location = Vector(
            (
    ...
    # 70 deg puts the camera east-north-east, weighted toward the bisector of the Ritch Street front
    # (normal 45.6 deg) and the SE flank (135.6 deg): the designed elevation, the
    # fire escape, the roof deck and the bulkhead all read at once, which is what
    # the app's camera actually gets on an alley building.
    az = math.radians(70)
    r = span * 4.4
    aer.location = Vector(
        (
