...
RES = (960, 720)
FOV_DEG = 40.0
AZIMUTH = math.radians(45.0)   # fixed 3/4 view from the NORTH-EAST: this
                               # building presents its one designed elevation to
                               # Ritch Street at bearing 45.6 deg, so G4 has to
                               # look at it (off = cos(az) east, sin(az) north)
...
# ENGINE DEVIATION (documented per GLB-OPTIMIZE-PROMPT §10): the generic script
# renders Cycles/64 on CPU. This machine was running at load average 265 with
# several parallel sessions on it, and a single Cycles night frame did not
# finish in 90 s. EEVEE at 128 taa samples renders the same scene in ~7 s, does
# emission (so the night half of G4 still means something), and — because the
# A/B comparison is differential, same rig on both sides — costs the gate
# nothing. It is also the engine the asset's own stage-2 review rig uses.
