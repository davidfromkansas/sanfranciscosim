"""Fresh-scene contract validation for 226-ritch.glb.
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3960899, 37.7804376],
        "front_heading_deg_true": 45.6,
        "target_height_m": 18.10,
    ...
    results["checks"] = {
        # The building is 12.13 x 22.80 m at a 45.6 deg heading, so its
        # axis-aligned box is ~25 x 25 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 17.9 <= dims.z <= 18.3
        and 24.0 <= dims.x <= 26.5
        and 24.0 <= dims.y <= 26.5,
        "crest_normalized_to_target": abs(mx.z - 18.10) <= 0.02,
