"""Fresh-scene contract validation for 76-south-park.glb.

    blender -b --python validate_76_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3940170, 37.7820261],
        "front_heading_deg_true": 135.0,
        "long_axis_heading_deg_true": 315.0,
        "target_height_m": 16.28,
    ...
    results["checks"] = {
        # 26.6 x 26.0 x 16.28 m: the XY box is the exact 45 deg rotation of a
        # 6.90 x 29.70 m sliver, not a 26 m building. The two XY figures differ
        # by 0.6 m because the canted bay projects on the street end only. See
        # REPORT.md.
        "meters_and_plausible_dimensions": (
            16.2 <= dims.z <= 16.35
            and 25.6 <= dims.x <= 27.0
            and 25.6 <= dims.y <= 27.0
        ),
        "crest_normalized_to_target": abs(mx.z - 16.28) <= 0.02,
