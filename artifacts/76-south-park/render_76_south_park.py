"""Controlled review renders of the exported 76-82 South Park GLB.

    blender -b --python render_76_south_park.py -- [--glb FILE] [--out DIR]
                                               [--prefix 76-south-park] [--night]
...
opposite faces. Each view keeps the nearest compass filename:
    ...
    -north.png  REAR elevation, faces 315 deg (the rear yard; the rear stair)
    -west.png   south-west party flank, faces 225 deg (toward 84 South Park; the
                exposed band above the neighbour's 11.36 m roof)
    -east.png   north-east party flank, faces 45 deg (toward 70 South Park;
                blind — 70 is only 0.21 m lower, so this face is never visible
                in the app and is rendered only for completeness)

-top.png doubles as the required look straight down the roof: it is the frame
that has to show the deck, the railing and the penthouse, because that is what
the app's camera sees most of the time.
...
FRONT_AZ = 135.0         # the street elevation faces south-east
LONG_AXIS = 315.0        # front -> rear bearing (toward Taber Place)
...
VIEWS = [
    ...
    ("west", 225.0),              # flank toward 84, with the exposed band
    ("east", 45.0),               # flank toward 70, blind
]

# A three-quarter swung round from the facade normal toward the south-west, so
# one frame reads all four things that matter: the street elevation with its bay
# and arch, the 29.7 m depth running away, the exposed band on the south-west
# flank, and the roof deck. Square to the facade (135) would hide the last three,
# which are the recognition cues from the air.
AERIAL_AZ = 172.0
AERIAL_PITCH = 34.0
AERIAL_R = 3.40
...
def light_glow():
    ...
    for mat, bsdf in glow_materials():
        if bsdf:
            # 3.2 carried over from 104-106. This facade has only three lit
            # openings on a 6.9 m front plus a railing line, so it does not need
            # more; re-check on the night render before changing it.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "76-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "76-south-park")
    night = "--night" in argv
