"""Fresh-scene contract validation for 169-steuart.glb.

    blender -b --python validate_169_steuart.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3919821, 37.7926993],
        "front_heading_deg_true": 45.1,
        "target_height_m": 46.64,
    ...
    results["checks"] = {
        # The 45.1 deg heading turns a 42.35 x 41.84 m building into a ~60 m
        # axis-aligned bounding box. That is expected, not a scale error.
        "meters_and_plausible_dimensions": 46.4 <= dims.z <= 46.9
        and 58.0 <= dims.x <= 62.0
        and 58.0 <= dims.y <= 62.0,
        "crest_normalized_to_target": abs(mx.z - 46.64) <= 0.02,
