"""Controlled review renders of the exported 169 Steuart Street (Army & Navy YMCA) GLB.

    blender -b --python render_169_steuart.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 169-steuart]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Embarcadero entry front faces NE (45.1 deg) and the Steuart Street
elevation faces SW (225.1 deg).
...
RES = (1400, 1300)   # a 60 m AABB over a 46.6 m building: nearly square
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "169-steuart.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "169-steuart")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(62.0)  # from the ENE: the Embarcadero entry front
        #   (45.1 deg) three-quarter, with the SE flank and the tile roof
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: tile-roof hips, arcade band, crest parapets, light court, podium step ----
    ...
    aer.data.type = "PERSP"
    # 70 mm at a long radius: restrained perspective, and the 60 m AABB of a
    # 45-deg-heading building needs the extra room.
    aer.data.lens = 70.0
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(62.0)  # from the ENE — the Embarcadero entry front
    #   (45.1 deg) seen three-quarter with the SE flank, which is the only angle
    #   that puts the hallmark elevation and the tile hip roof in one frame
    r = span * 3.4
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
