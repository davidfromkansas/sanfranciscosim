...
#   3. (SKIPPED) limited dissolve — this asset has coplanar parapet+coping ring
#      bands, the documented sliver trap; see the note at the step itself
#   4. (n/a) curve retess — 501 Second has no curved geometry at all: every
#      surface is a planar panel, box or prism, so there is nothing to retessellate
...
# follows the footprint all the way round." 169 Steuart has THREE such rings
# (the cast-stone base 0-9.60 m, the podium roof deck at 13.70-13.95 m and the
# wing parapet 27.84-28.14 m), each following the full footprint perimeter
# — plus the stone-base ring is 168 m round. Their top and bottom faces are perfectly coplanar
# annuli; even a strictly-coplanar dissolve merges each into one annulus ngon,
# and re-triangulating an annulus emits ~0.24 mm slivers up to 30 m long. Those
# pass an area-based degeneracy test, survive Phase B and E, and then surface
# only in the PACKED file as invalid_or_nonunit_loop_normal_count, because
# gltfpack re-emits stored normals while Blender recomputes them on import.
# Measured on 350-brannan, 13 Aug 2026. The step was worth 0.4% there. Not run.
stats["limited_dissolve"] = "skipped: coplanar parapet+coping ring bands (prompt s.3 step 3)"
