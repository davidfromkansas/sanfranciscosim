"""Fresh-scene contract validation for hyatt-regency.glb.

    blender -b --python validate_hyatt_regency.py -- [--glb FILE] [--out FILE]
                                                    [--closed-solids]
...
Normals: Hyatt Regency is a union of closed solids, so per-object signed volume
...
(`optimize/input/hyatt-regency.glb`) to additionally assert that every object is
...
def main():
    ...
    results = {
        ...
        "xy_origin_note": (
            "The origin is the surveyed footprint centroid (the manifest anchor), "
            "which is what placeGeneric positions. The bbox centre sits west of it "
            "because the entry canopy projects onto the plaza; Palace of Fine Arts "
            "ships with the same asymmetry."
        ),
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": (
            110 <= dims.x <= 130 and 90 <= dims.y <= 108 and abs(dims.z - TARGET_HEIGHT) <= 0.05
        ),
