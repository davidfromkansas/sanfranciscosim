"""Controlled review renders of the exported Hyatt Regency GLB.

    blender -b --python render_chase_center.py -- [--glb FILE] [--out DIR]
                                                  [--prefix hyatt-regency]
                                                  [--only aerial]
...
The Hyatt Regency is a stepped wedge, so the hero aerial sits NNW - the only
quarter from which the terraces, the Drumm-Street staircase and the Equinox
pavilion are all visible at once, and the heading the app's camera preset uses.
"""
...
def setup_world():
    ...
    if QUICK:
        # Workbench: seconds instead of minutes when the machine is shared with
        # other agent sessions (see memory sf3d-render-contention).
    ...
    scene.cycles.samples = int(os.environ.get("SF_SAMPLES", "48"))
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "hyatt-regency.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "hyatt-regency")
    only = arg("--only", "")
    global QUICK
    QUICK = "--quick" in argv
    ...
    # NNW of the hotel, looking down the wedge across the terrace field.
    aer = make_camera("cam_aerial")
    aer.data.type = "PERSP"
    aer.data.lens = 80.0  # long lens, restrained perspective (style bible s.18)
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(338)  # NNW: the view that shows the wedge falling
    r = span * 4.6
    aer.location = Vector(
        (
    ...
    # --- top view: the terrace field, the wing roof and the Equinox frame ---
    ...
    ncam = make_camera("cam_night_entry")
    ncam.data.type = "PERSP"
    ncam.data.lens = 55.0
    pitch_n = math.radians(14)
    az_n = math.radians(120)  # standing on Market Street
    rn = span * 2.6
    ...
    render_to(os.path.join(out, f"{prefix}-night-entry.png"), ncam, (1500, 900))
