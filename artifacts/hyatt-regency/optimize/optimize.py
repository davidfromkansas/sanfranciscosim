...
# bands. This one is nothing but ring bands — fifteen slab lips (ring_prism),
# the wing parapet ring, the podium eave and the two Equinox frames all follow
# the footprint the whole way round, so their top and bottom faces are perfect
# coplanar annuli. Re-triangulating an annulus ngon emits sub-millimetre
# slivers whose averaged vertex normals collapse to ~0; gltfpack then re-emits
# the stored normals and the stage-2 validator fails on
# invalid_or_nonunit_loop_normal_count, two steps after the shipping swap.
# The measured win on 350-brannan was 0.4% of triangles. Not worth it.
stats["limited_dissolve"] = "skipped (coplanar ring bands — see prompt s.3.3)"
