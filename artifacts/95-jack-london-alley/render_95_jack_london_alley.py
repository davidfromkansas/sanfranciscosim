"""Controlled review renders of the exported 135 South Park GLB.

    blender -b --python render_95_jack_london_alley.py -- [--glb FILE] [--out DIR]
                                                      [--prefix 95-jack-london-alley]
                                                      [--night]
...
(north = Blender +Y), which is how the asset is authored — the alley facade faces SW (225.9 deg).
...
RES = (1400, 1100)   # this building is a near-cube: 16 m across, 8.4 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def setup_world(night=False):
    ...
    # CPU deliberately. Metal was tried on this machine and the headless
    # `-b --factory-startup` process blocked at 0% CPU before the first tile,
    # so the rig stays on the path that is known to produce these images.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "95-jack-london-alley.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "95-jack-london-alley")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(225)  # from the SW: square onto the alley facade, which
        # is the only elevation with anything on it, lit or unlit
        r = span * 4.6
        aer.location = Vector(
            (
    ...
    # --- top view: coping ring, the facade parapet step at the alley end,
    # --- the dentil band down the north-west flank, three roof boxes -------
    ...
    az = math.radians(225)  # from the SW — square onto the alley facade
    r = span * 4.8
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
    ...
    # --- square-on at the 225.9 deg alley facade ---------------------------
    # None of the four cardinal elevations shows this building's public face:
    # it stands at 45.9 deg, so every one of them catches it obliquely.
    fac = make_camera("cam_facade")
    fac.data.type = "ORTHO"
    fac.data.ortho_scale = 11.5
    a = math.radians(225.9)
    fac.location = Vector(
        (center.x + dist * math.sin(a), center.y + dist * math.cos(a), center.z)
    )
    aim(fac, center)
    render_to(os.path.join(out, f"{prefix}-facade.png"), fac, (1100, 1250))

    # --- CLOSE three-quarter of the entrance ensemble ----------------------
    # The entrance is where this asset succeeds or fails, and a 16 m-wide
    # elevation render will not show you whether it does.
    ent = make_camera("cam_entrance")
    ent.data.type = "PERSP"
    ent.data.lens = 85.0
    ea = math.radians(203.0)
    ep = math.radians(13.0)
    er = 9.5
    # the entrance sits on the alley (south-west) face, 1.7 m above grade
    fx = center.x + 6.85 * math.sin(math.radians(225.9))
    fy = center.y + 6.85 * math.cos(math.radians(225.9))
    focus = Vector((fx, fy, 1.75))
    ent.location = Vector(
        (
            focus.x + er * math.cos(ep) * math.sin(ea),
            focus.y + er * math.cos(ep) * math.cos(ea),
            focus.z + er * math.sin(ep),
        )
    )
    aim(ent, focus)
    render_to(os.path.join(out, f"{prefix}-entrance.png"), ent, (1100, 1250))
