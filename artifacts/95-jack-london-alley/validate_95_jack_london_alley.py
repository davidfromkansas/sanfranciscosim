"""Fresh-scene contract validation for 95-jack-london-alley.glb.

    blender -b --python validate_95_jack_london_alley.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3934430, 37.7813460],
        "front_heading_deg_true": 225.9,
        "target_height_m": 8.4,
    ...
    results["checks"] = {
        # Plausibility window for THIS asset. An 8.60 x 13.70 m box at a 45.9 deg
        # heading has an axis-aligned bbox of ~15.8 x 15.7 m — near-square, and
        # only 0.1 m wider in X than in Y. The window is tight on purpose: a
        # model built on OSM's (rejected) 9.37 x 20.35 m footprint would land at
        # ~21 x 21 m and must not pass.
        "meters_and_plausible_dimensions": 8.3 <= dims.z <= 8.5
        and 15.6 <= dims.x <= 16.6
        and 15.5 <= dims.y <= 16.5,
        "crest_normalized_to_target": abs(mx.z - 8.4) <= 0.02,
