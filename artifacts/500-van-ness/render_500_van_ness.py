"""Controlled review renders of the exported 500 Van Ness Avenue GLB.

    blender -b --python render_500_van_ness.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 500-van-ness]
                                                    [--night] [--fast] [--gpu]
...
(north = Blender +Y), which is how the asset is authored. The building's two
show faces are west (Van Ness) and south (McAllister), so the aerial looks from
the south-west (azimuth 232 deg) — square onto the corner the city sees.
...
AZ_HERO = 232.0               # from the SW: square onto the Van Ness/McAllister corner
...
def enable_gpu():
    """Opt-in Metal/CUDA compute for the review renders (--gpu).

    Same Cycles integrator and the same sample count, so the images are the
    ones the repo rig produces; this only moves the work off a CPU that may be
    shared with other Blender sessions. Falls back to CPU silently if no
    compute device is available.
    """
    ...
    for kind in ("METAL", "OPTIX", "CUDA", "HIP", "ONEAPI"):
        ...
        for d in cprefs.devices:
            d.use = d.type != "CPU"
        print(f"[render] GPU compute: {kind} ({', '.join(d.name for d in devices)})")
        return True
    return False


def setup_world(night=False, fast=False, gpu=False):
    scene = bpy.context.scene
    scene.render.engine = "CYCLES"
    scene.cycles.device = "GPU" if (gpu and enable_gpu()) else "CPU"
    scene.cycles.samples = 20 if fast else 64
...
def light_glow():
    """Preview the app's night pass. glTF writes emissiveFactor 0 for an asset
    authored with strength 0, so a re-imported _Glow material carries a DEFAULT
    WHITE emission colour — copy Base Color across before raising strength, or
    every glow surface renders as a white slab (docs/asset-plans/README.md)."""
...
def hero_camera(name, center, span, lens, pitch_deg):
    cam = make_camera(name)
    cam.data.type = "PERSP"
    cam.data.lens = lens
    pitch = math.radians(pitch_deg)
    az = math.radians(AZ_HERO)
    r = span * 3.1
    cam.location = Vector(
        (
    ...
    aim(cam, Vector((center.x, center.y, center.z * 0.92)))
    return cam
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "500-van-ness.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "500-van-ness")
    night = "--night" in argv
    fast = "--fast" in argv   # aerial only, 20 samples — for design iteration
    ...
    setup_world(night=night, fast=fast, gpu="--gpu" in argv)
    add_lights(height, night=night)

    if night:
        light_glow()
        cam = hero_camera("cam_night", center, span, 90.0, 35.0)
        render_to(os.path.join(out, f"{prefix}-aerial-night.png"), cam, AER_RES)
        return

    fade_glow()

    if fast:
        cam = hero_camera("cam_fast", center, span, 105.0, 38.0)
        render_to(os.path.join(out, f"{prefix}-aerial.png"), cam, (1000, 850))
        return
    ...
    # --- top view: the two light wells, penthouse, skylight, vents ----------
    ...
    cam = hero_camera("cam_aerial", center, span, 105.0, 38.0)  # style bible s.18
    render_to(os.path.join(out, f"{prefix}-aerial.png"), cam, AER_RES)
