"""Fresh-scene contract validation for 500-van-ness.glb.

    blender -b --python validate_500_van_ness.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    # enclose positive volume. The ray test above is the secondary check, and its
    ...
    results = {
        ...
        "anchor_lonlat": [-122.4199220, 37.7804082],
        "front_heading_deg_true": 261.6,
    ...
    results["centering_note"] = (
        "Origin is the measured footprint ring bbox centre, so the building "
        "stands on its true coordinates (AGENTS rule 5). The full bbox is "
        "~2.4 m wider than the 40.0 x 41.8 m footprint on each axis because the "
        "cornice projects 1.05 m past the wall all the way round; that overhang "
        "is symmetric, so the bbox centre still lands on the footprint centre."
    )
    results["checks"] = {
        "meters_and_plausible_dimensions": 16.8 <= dims.z <= 17.2
        and 41.0 <= dims.x <= 44.0
        and 43.0 <= dims.y <= 46.0,
        "crest_normalized_to_target": abs(mx.z - 17.0) <= 0.02,
