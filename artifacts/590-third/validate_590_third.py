"""Fresh-scene contract validation for 590-third.glb.

    blender -b --python validate_599_third.py -- [--glb FILE] [--out FILE]
...
Normals are checked two ways, because this asset is a union of ~90 closed
solids rather than one shell — and one of them, the body, is genus 1: the light
well is a real hole cut through it:
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(9.5 / dims.z, 6),
        ...
        "anchor_lonlat": [-122.3946749, 37.7800837],
        "brannan_front_normal_deg_true": 135.1,
        "third_street_front_normal_deg_true": 45.2,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        "meters_and_plausible_dimensions": 9.4 <= dims.z <= 9.6
        and 31.0 <= dims.x <= 33.0
        and 30.8 <= dims.y <= 32.5,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 9.5) <= 0.01,
