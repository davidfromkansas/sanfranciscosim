...
#
# Same reason as 350 Brannan, and the hazard is worse here. GLB-OPTIMIZE-PROMPT
# s.3 step 3 says to skip the 0.05 deg limited dissolve on assets with large
# coplanar ring bands: their top and bottom faces are perfectly coplanar annuli,
# a strictly-coplanar dissolve merges each ring into one annulus ngon, and
# re-triangulating an annulus emits slivers whose averaged vertex normals
# collapse to ~0. Blender hides that on import; gltfpack re-emits the STORED
# normals, so the stage-2 contract validator fails on
# `invalid_or_nonunit_loop_normal_count` two steps later, after the shipping
# swap.
#
# 590 Third has FOUR such bands, not two: `parapet` and `parapet_cap` follow the
# footprint the whole way round, and `body` and `roof_field` are genus-1 solids
# whose top and bottom caps are themselves annuli — bridged corner to corner
# around the light well. Those caps are exactly the shape that produces the
# sliver, and the light well is the asset's most distinctive feature, so a
# defect there would be a defect in the thing worth shipping.
#
# Skipped without measuring the win. On 350 Brannan the same step was worth 30
# triangles out of 6,770 (0.4%); there is no reason to expect more here.
