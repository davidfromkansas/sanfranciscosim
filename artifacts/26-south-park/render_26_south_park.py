"""Controlled review renders of the exported 26-28 South Park GLB.

    blender -b --python render_26_south_park.py -- [--glb FILE] [--out DIR]
                                               [--prefix 26-south-park] [--night]
...
opposite faces. Each view keeps the nearest compass filename:
    ...
    -west.png   south-west party flank, faces 225 deg (toward 44-46 South Park; the
                exposed deck rail and the setback read here)
    -east.png   north-east party flank, faces 45 deg (toward the Hotel Madrid at 22-24; blind, and
...
RES = (1200, 1000)
AER_RES = (1600, 1200)
TOP_RES = (1000, 1500)   # portrait: the building is a 30.1 m sliver
...
VIEWS = [
    ...
    ("west", 225.0),              # flank toward 108-110, with the boarded strip
    ("east", 45.0),               # flank toward 102, blind
]

# A three-quarter swung round from the facade normal toward the south-west, so
# one frame reads all three things that matter: the street elevation, the 30.1 m
# depth running away, and — the point of this asset — the OPEN DECK, which is
# only legible from above the rail. Square to the facade (135) would hide the
# depth and flatten the deck into a line.
AERIAL_AZ = 168.0
AERIAL_PITCH = 34.0
AERIAL_R = 3.40
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "26-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "26-south-park")
    night = "--night" in argv
    ...
    top.data.type = "ORTHO"
    top.data.ortho_scale = 34.5   # portrait frame: this is the VERTICAL extent
