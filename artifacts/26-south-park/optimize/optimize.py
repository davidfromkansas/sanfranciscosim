...
# and the light-well curb), each an annulus following a 30 m footprint. Re-
# triangulating an annulus ngon emits slivers up to the full ring length; they
# pass every area-based degeneracy test, collapse their shared vertex normals to
# ~0, and only surface after the shipping swap as
# invalid_or_nonunit_loop_normal_count in the stage-2 contract validator (the
# 350-brannan failure of 13 Aug 2026, and the same call 106 South Park made).
# On a 2.5k-triangle asset with no curved shells the step is worth a handful of
# triangles at most, so it is not worth working around.
stats["limited_dissolve"] = "skipped: closed ring bands (parapet, well curb)"
