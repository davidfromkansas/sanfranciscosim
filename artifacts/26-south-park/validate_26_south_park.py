"""Fresh-scene contract validation for 26-south-park.glb.

    blender -b --python validate_26_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results["checks"] = {
        # 25.9 x 26.0 x 9.05 m: the XY box is the exact 45 deg rotation of a
        # 6.69 x 30.13 m sliver, not a 26 m building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            9.0 <= dims.z <= 9.10
            and 25.6 <= dims.x <= 26.4
            and 25.7 <= dims.y <= 26.5
        ),
        "crest_normalized_to_target": abs(mx.z - 9.05) <= 0.02,
