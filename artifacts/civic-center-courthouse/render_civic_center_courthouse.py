"""Controlled review renders of the exported Civic Center Courthouse GLB.
    ...
    blender -b --python render_war_memorial_opera_house.py -- [--glb FILE] [--out DIR]
                                                     [--prefix civic-center-courthouse]
...
(north = Blender +Y, how the asset is authored). The auditorium axis actually bears
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "civic-center-courthouse.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "civic-center-courthouse")
    ...
    # A near-white roof under the three-sun elevation rig blows out and the
    # lantern disappears into it, so this one camera rakes the key down to 34
    # deg and drops the fill: the penthouse, plant and lantern get real
    # shadows and the roof design reads. Restored immediately afterwards so
    # the aerial and night views use the same rig as the elevations.
    top = make_camera("cam_top")
    top.data.type = "ORTHO"
    top.data.ortho_scale = span * 1.18
    top.location = Vector((center.x, center.y, mx.z + span))
    top.rotation_euler = (0, 0, math.radians(-81.22) + math.pi / 2)
    ...
    az = math.radians(133)              # from the SE: the corner lantern + McAllister
    r = span * 3.15
    aer.location = Vector(
        (
        ...
        if mat.name.endswith("_Glow") and mat.use_nodes:
            ...
            if bsdf:
                ...
                bsdf.inputs["Emission Strength"].default_value = {
                    "Toy_white_Glow": 3.4,      # entrance canopy soffit strip
                }.get(mat.name, 2.4)            # lit arch panes and oculi
    render_to(os.path.join(out, f"{prefix}-night.png"), aer, AER_RES)

    # night closeup: the lit lantern and arcade from the McAllister/Polk corner
    ncam = make_camera("cam_night_corner")
    ncam.data.type = "PERSP"
    ncam.data.lens = 60.0
    pitch_n = math.radians(20)
    az_n = math.radians(133)
    rn = height * 2.6
    ...
    aim(ncam, Vector((center.x, center.y, mn.z + height * 0.62)))
    render_to(os.path.join(out, f"{prefix}-night-corner.png"), ncam, (1300, 1100))
