"""Fresh-scene contract validation for de-young.glb.

    blender -b --python validate_de_young.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    # Visibility-based outward-normal test. The museum is a low broad slab
    # with roof voids, so the deterministic ray sphere targets a 3 x 3 grid
    # across the plan at three heights; a visible outward face must oppose
    ...
    results = {
        ...
        "normal_orientation_method": "All source meshes run bmesh.ops.recalc_face_normals before export; reimported loop normals must be finite/unit; 22,500 deterministic visibility rays over 15 targets test the first visible face.",
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 150 <= dims.x <= 180 and 145 <= dims.y <= 180 and 42 <= dims.z <= 46,
