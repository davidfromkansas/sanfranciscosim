...
#
# Adapted for 2-south-park: skips limited dissolve (§3 step 3) because the
# asset has SIX ring bands (base_band, band_ground, band_spandrel, band_lintel,
# parapet, coping) whose coplanar annuli would merge into sliver ngons on
# re-triangulation — the densest ring-band asset in the set so far.
#
# Steps (per GLB-OPTIMIZE-PROMPT v2 §3):
#   1. weld coincident verts <=1mm within each object
#   2. delete degenerate faces; delete interior faces strictly buried inside
#      another closed solid (AABB-fill >= 95%) — provable-invisible only
#   3. SKIP limited dissolve — six ring bands present
...
# --- 3. SKIP limited dissolve ---
# This asset has six ring bands (base_band, band_ground, band_spandrel,
# band_lintel, parapet, coping) whose top and
# bottom faces are perfectly coplanar annuli. Even a strictly-coplanar dissolve
# merges each ring into one annulus ngon, and re-triangulating an annulus emits
# slivers (measured on 350-brannan: 7 triangles up to 24.35 m long, ~0.24 mm
# wide). These pass area-based degeneracy tests but fail the stage-2 contract
# validator's normal check. The dissolve savings (~30 tris, 0.4%) are not worth
# the risk. See GLB-OPTIMIZE-PROMPT §3 step 3.
stats["dissolve_skipped"] = (
    "six ring bands present: base_band, band_ground, band_spandrel, "
    "band_lintel, parapet, coping"
)
