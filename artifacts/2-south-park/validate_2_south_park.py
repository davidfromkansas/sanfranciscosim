"""Fresh-scene contract validation for 2-south-park.glb.

    blender -b --python validate_2_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3932364, 37.7824236],
        "front_heading_deg_true": 45.2,
        "target_height_m": 17.72,
    ...
    results["checks"] = {
        # The building is 29.8 x 20.9 m at a 45 deg heading, so its
        # axis-aligned box is ~36 x 36 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 17.5 <= dims.z <= 17.9
        and 35.0 <= dims.x <= 37.5
        and 35.0 <= dims.y <= 37.5,
        "crest_normalized_to_target": abs(mx.z - 17.72) <= 0.02,
