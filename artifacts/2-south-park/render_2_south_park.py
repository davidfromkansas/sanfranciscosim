"""Controlled review renders of the exported 2 South Park GLB.

    blender -b --python render_2_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 2-south-park]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
front faces NE (45.2 deg) and the South Park front SE (135.2 deg).
...
def render_top(center, mx, width, span, out, prefix):
    """Top view: the full roof, penthouse, skylight and mechanical layout."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "2-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "2-south-park")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(90)  # three-quarter from the east, over the corner
        r = span * 4.4
        aer.location = Vector(
            (
    ...
    # 90 deg looks from due east, square onto the E corner — the one place both
    # public street elevations (NE Second Street, SE South Park) and the roof
    # penthouse read at once. The corner IS the building (plan 2.5 cue 1).
    az = math.radians(90)
    r = span * 4.4
    aer.location = Vector(
        (
