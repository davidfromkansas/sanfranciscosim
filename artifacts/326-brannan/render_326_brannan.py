"""Controlled review renders of the exported 326 Brannan Street GLB.
...
(north = Blender +Y), which is how the asset is authored — the Brannan frontage faces SE (135.15 deg).
...
RES = (1400, 1050)
AER_RES = (1500, 1250)
# The top view is a HERO image for this asset, not a checkbox: two thirds of
# what the app's downward camera sees here is open court, so the court plan IS
# the elevation that matters. Rendered larger than the others on purpose.
TOP_RES = (1500, 1500)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "326-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "326-brannan")
    night = "--night" in argv
    ...
    if night:
        ...
        # Second night frame, lower and tighter: a lit garden seen through its
        # own gate is the reading this asset exists for.
        low = make_camera("cam_night_low")
        low.data.lens = 105.0
        pitch2 = math.radians(22)
        r2 = span * 2.5
        low.location = Vector(
            (
                center.x + r2 * math.cos(pitch2) * math.sin(az),
                center.y + r2 * math.cos(pitch2) * math.cos(az),
                center.z + r2 * math.sin(pitch2),
            )
        )
        aim(low, Vector((center.x, center.y, center.z * 0.75)))
        render_to(os.path.join(out, f"{prefix}-aerial-night.png"), low, AER_RES)
        return
    ...
    # --- top view (hero): the court plan — gate, planters, canopy, olive,
    # fire ring, then the shed roof at the back ----------------------------
