"""Fresh-scene contract validation for 326-brannan.glb.

    blender -b --python validate_380_brannan.py -- [--glb FILE] [--out FILE]
...
TRI_BUDGET = 12000
TARGET_HEIGHT = 5.9   # shed parapet crest; see REFERENCE.md s.3
...
def main():
    ...
    for obj in meshes:
        ...
        n_degen = sum(1 for tri in me.loop_triangles if tri.area < 1e-8)
        degenerate += n_degen
        if n_degen:
            degenerate_objects.append({"name": obj.name, "count": n_degen})
    ...
    results = {
        ...
        "degenerate_objects": sorted(degenerate_objects, key=lambda x: x["name"]),
        ...
        "anchor_lonlat": [-122.3928965, 37.7815080],
        "front_heading_deg_true": 135.15,
    ...
    results["checks"] = {
        # A 7.98 x 24.32 m lot on the 45-degree SoMa grid gives a ~22.3 m
        # axis-aligned XY box. That is correct, not a scale error.
        "meters_and_plausible_dimensions": 5.7 <= dims.z <= 6.1
        and 21.0 <= dims.x <= 23.5
        and 21.0 <= dims.y <= 23.5,
