...
(north = Blender +Y), which is how the asset is authored — the Brannan Street front faces SE (135.8 deg).
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "350-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "350-brannan")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(100)  # from the E: catches BOTH finished street faces
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, skylight row, HVAC pair and roof penthouse -------
...
def render_aerial(center, span, out, prefix):
    ...
    az = math.radians(100)  # from the E — the Brannan front AND the Jack
    # London Alley elevation in one frame, which is what the app's camera gets.
