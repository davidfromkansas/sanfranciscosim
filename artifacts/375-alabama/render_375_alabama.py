"""Controlled review renders of the exported 375 Alabama Street GLB.
    ...
    blender -b --python render_375_alabama.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 375-alabama]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Alabama Street
front (the address, the entrance, the tower) faces west, 265.7 deg.
...
RES = (1600, 900)    # landscape: a 67 m block only 22.5 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "375-alabama.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "375-alabama")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(225)  # from the SW: tower, entrance and both medallion walls
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    az = math.radians(225)  # from the SW — the only angle that shows the tower,
    # the arched entrance and both medallion elevations at once (plan Part 1)
    r = span * 3.1
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
