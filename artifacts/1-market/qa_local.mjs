// Stage-5 local QA for 1-market, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/1-market/qa_local.mjs [--drill]
//
...
const LON = -122.3948075;
const LAT = 37.7938412;
const SLUG = '1-market';
const CAMERA = { distance: 330, yaw: 200, pitch: 24 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa1market-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('1Market') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasThis: window.SF.assets.placed.has('1Market'),
    placeThis: window.SF.assets.placed.get('1Market') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('1Market').log, scale: window.SF.assets.placed.get('1Market').scale })) : null,
  }))()`);
  ...
  for (const l of console_) if (/1-market|1Market|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
