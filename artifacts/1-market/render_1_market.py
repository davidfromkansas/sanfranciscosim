"""Controlled review renders of the exported 1 Market Street GLB.

    blender -b --python render_1_market.py -- [--glb FILE] [--out DIR]
                                               [--prefix 1-market] [--night]
...
VIEWS = [
    ("north", 45.2),   # Steuart Street flank (NE)
    ("east", 135.2),   # the Mission Street returns and the open side of the court
    ("south", 225.2),  # Spear Street flank (SW)
    ("west", 315.2),   # Market Street front (NW) — the portal and the balcony
]
# A true three-quarter on the NORTH corner (Market x Steuart): the front, the
# Steuart flank and the crowning cornice returning round the corner in one frame,
# with the atrium apex showing over the Market wing (style bible s.18).
AERIAL_AZ = 0.0
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "1-market.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "1-market")
    night = "--night" in argv
    global FAST, GPU
    FAST = "--fast" in argv
    GPU = "--gpu" in argv
