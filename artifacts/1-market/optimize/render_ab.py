...
# This Mac runs many parallel Blender sessions; CPU Cycles could not finish the
# eight-frame set. Metal GPU renders the same image ~30x faster. Both sides of
# the A/B are rendered on the same device, so the comparison is unaffected.
_prefs = bpy.context.preferences.addons["cycles"].preferences
_prefs.compute_device_type = "METAL"
if hasattr(_prefs, "get_devices"):
    _prefs.get_devices()
