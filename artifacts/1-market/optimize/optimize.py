...
# 1 Market carries EIGHT coplanar ring bands that follow the footprint all the
# way round (plinth, base, belt, frieze, balustrade and the three cornice
# courses). GLB-OPTIMIZE-PROMPT s.3 step 3 says a strictly-coplanar dissolve
# turns each of those annuli into one ngon whose re-triangulation emits
# invisible slivers that only surface in the packed file. So the dissolve and
# the weld are both switchable and all four variants get measured, rather than
# assumed (sf3d-weld-heuristic-is-not-bevel).
...
stats = {"variant": {"weld": "--no-weld" not in argv, "dissolve": "--no-dissolve" not in argv},
...
for o in (mesh_objs() if DO_WELD else []):
    bm = bmesh.new()
    bm.from_mesh(o.data)
    ...
    bm.to_mesh(o.data)
    bm.free()
snap("weld+degenerate" if DO_WELD else "weld-skipped")
