"""Fresh-scene contract validation for conservatory-of-flowers.glb.

    blender -b --python validate_conservatory_of_flowers.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    # Visibility-based outward-normal test. The conservatory is an assembly of
    # open shells (vault glazing, dome, ribs), so signed-volume tests cannot
    # judge it; instead cast a deterministic sphere of rays toward targets
    # spread through the building - a visible outward face must oppose the
    ...
    targets = [
        ...
        for dx, dy in ((0.0, 0.0), (-0.30, 0.0), (0.30, 0.0), (0.0, -0.15))
        for fz in (0.15, 0.45, 0.80)
    ]
    ...
    results = {
        ...
        if invalid_normal_count == 0 and ray_hits > 0 and ray_flipped == 0
        else "FAIL",
        "normal_orientation_method": "All source meshes run bmesh.ops.recalc_face_normals before export; reimported loop normals must be finite/unit; 30,000 deterministic visibility rays test the first visible face of the open-shell model.",
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 70 <= dims.x <= 85
        and 24 <= dims.y <= 36
        and 17.8 <= dims.z <= 19.0,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "centered_xy": abs(center.x) <= 1.0 and abs(center.y) <= 1.5,
    ...
    with open(output, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print("[validate] overall:", results["overall"])
    for k, v in results["checks"].items():
        print(f"[validate] {k}: {'PASS' if v else 'FAIL'}")
