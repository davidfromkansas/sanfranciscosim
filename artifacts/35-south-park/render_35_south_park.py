"""Controlled review renders of the exported 35 South Park (Accel) GLB.

    blender -b --python render_168_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 35-south-park]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the South Park arcade faces NW (315.9 deg).
...
RES = (1400, 1050)   # the 45 deg heading makes every elevation ~42 m wide
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "35-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "35-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(315)  # from the NW: the South Park arcade is the identity
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: the hedge band, the penthouse and the roof lights --------
    ...
    aer.data.type = "PERSP"
    # 42 m of AABB on a 22.7 x 35.8 m building: the framing heuristic
    # underestimates it, so 65 mm at a longer radius keeps the restrained
    # perspective and still fits the whole thing.
    aer.data.lens = 65.0
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(315)  # from the NW — square onto the South Park arcade
    r = span * 3.4
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
    ...
    # --- square-on to the South Park arcade ---------------------------------
    ...
    fr.data.type = "ORTHO"
    fr.data.ortho_scale = 26.0
    a = math.radians(315)
    fr.location = Vector((center.x + dist * math.sin(a), center.y + dist * math.cos(a), center.z))
    aim(fr, center)
    render_to(os.path.join(out, f"{prefix}-front.png"), fr, (1400, 800))
