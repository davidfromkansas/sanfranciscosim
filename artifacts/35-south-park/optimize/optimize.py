...
#   3. (SKIPPED) limited dissolve — see the note below and GLB-OPTIMIZE-PROMPT §3.3
#   4. (SKIPPED) curve retess — 35 South Park's only curves are the five 10-segment
#      arch profiles, the four 14-segment roundels and one 10-segment vent. All three
#      are already at the floor of the style bible's 8-14 segment range and all three
#      are silhouette-defining on the hero elevation, so halving them is not available
...
# coplanar ring bands. This one has SIX that follow the whole footprint —
# water_table, architrave, frieze, cornice, parapet and coping — plus a body
# prism whose cap is a 791 m2 coplanar ngon. Their top and bottom faces are perfect
# coplanar annuli, so even a strictly-coplanar dissolve merges each ring into
# one annulus ngon and re-triangulating an annulus emits ~0.2 mm slivers up to
# 30 m long. Those pass every area-based degeneracy test in Phase B and E and
# only surface two steps later, in the packed file, as
# invalid_or_nonunit_loop_normal_count. Measured on 350-brannan, 13 Aug 2026.
# The step is worth well under 1% of triangles on an asset of this shape.
stats["steps"].append({"step": "curve-retess", "skipped":
                       "10-segment arches / 14-segment roundels are already minimal "
                       "and silhouette-defining on the hero elevation"})
stats["steps"].append({"step": "limited-dissolve", "skipped":
                       "six coplanar ring bands (water_table, architrave, frieze, "
                       "cornice, parapet, coping) plus the body cap"})
print("STEP limited-dissolve: SKIPPED (coplanar ring bands)")
