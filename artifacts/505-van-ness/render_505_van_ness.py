"""Controlled review renders of the exported 505 Van Ness Avenue GLB.

    blender -b --python render_505_van_ness.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 505-van-ness]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the entrance faces ESE (126.3 deg).
...
RES = (1600, 900)    # landscape: 124 m wide, 27 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    out = arg("--out", here)
    prefix = arg("--prefix", "505-van-ness")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(126)  # from the ESE: square onto the bowed drum front
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    span = max(width, height)
    if fast:
        aer = make_camera("cam_fast")
        aer.data.lens = 105.0
        pitch, az, r = math.radians(38), math.radians(126), span * 3.1
        aer.location = Vector((center.x + r * math.cos(pitch) * math.sin(az),
                               center.y + r * math.cos(pitch) * math.cos(az),
                               center.z + r * math.sin(pitch)))
        aim(aer, Vector((center.x, center.y, center.z * 0.92)))
        render_to(os.path.join(out, f"{prefix}-aerial.png"), aer, (1000, 850))
        return
    ...
    # --- top view: the open court, its glazed tower, skylights, penthouse ---------
