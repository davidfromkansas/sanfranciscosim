"""Fresh-scene contract validation for 505-van-ness.glb.

    blender -b --python validate_505_van_ness.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "505-van-ness.glb"))
    ...
    results = {
        ...
        "anchor_lonlat": [-122.4212915, 37.7804835],
        "front_heading_deg_true": 126.3,
        "target_height_m": 27.0,
    ...
    results["centering_note"] = (
        ...
        "stands on its true coordinates (AGENTS rule 5). The curved entrance "
        "stair projects ~12 m past the drum on the ESE only, which offsets the "
        "FULL bbox centre in +x; the footprint centre itself is at the origin."
    )
    results["checks"] = {
        "meters_and_plausible_dimensions": 26.8 <= dims.z <= 27.2
        and 120.0 <= dims.x <= 128.0
        and 92.0 <= dims.y <= 98.0,
        "crest_normalized_to_target": abs(mx.z - 27.0) <= 0.02,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "centered_xy": abs(center.x) <= 6.0 and abs(center.y) <= 1.0,
