"""Controlled review renders of the exported 234 Van Ness Avenue (The Kelsey Civic Center) GLB.

    blender -b --python render_234_van_ness.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 234-van-ness]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Van Ness Avenue
front faces west (261.8 deg) and the long face fronts Dr. Tom Waddell Place to
the south (170.7 deg).

The aerial is framed from the SOUTH-WEST and pitched steeply: this building's
identity is its open courtyard and its planted roof deck, and that is the only
angle from which both read.
...
RES = (1280, 900)    # landscape: 56 x 45 m footprint, 30 m tall
AER_RES = (1200, 1040)
TOP_RES = (1040, 1040)
...
# Flat, matte, untextured materials converge almost immediately under Cycles;
# 32 samples with denoising is visually indistinguishable from 64 here and
# halves a CPU-only render that has to share this machine with the other
# landmark sessions. Override with --samples for a slower reference pass.
SAMPLES = 32


# --workbench swaps Cycles for the rasterizer. It is the sanctioned headless
# fallback (sf-asset-check: "no GPU, so use Workbench or CPU Cycles") and it
# renders these flat, untextured, matte materials faithfully — what it gives up
# is Cycles' soft ambient occlusion and contact shadow, not colour or geometry.
WORKBENCH = False
...
def setup_world(night=False):
    ...
    if WORKBENCH:
        ...
        sd.show_cavity = True
        sd.cavity_type = "BOTH"
        sd.curvature_ridge_factor = 0.9
        sd.curvature_valley_factor = 1.6
        sd.shadow_intensity = 0.45 if not night else 0.6
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "234-van-ness.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "234-van-ness")
    night = "--night" in argv
    global SAMPLES, WORKBENCH
    ...
    if night:
        ...
        pitch = math.radians(46)
        az = math.radians(225)  # from the SW: courtyard + roof deck in one frame
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: the courtyard, the planted deck and the mech penthouse ---
    ...
    pitch = math.radians(46)  # steep end of the 30-50 deg band: the court is deep
    az = math.radians(225)  # from the SW — the only angle that shows the court
    r = span * 3.1
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
