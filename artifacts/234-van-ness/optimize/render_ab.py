...
# --workbench swaps Cycles for the rasterizer. For an A/B pixel-delta gate this
# is arguably the better instrument: it is deterministic and noise-free, so any
# non-zero delta is a real geometry or material difference rather than sampling
# noise. What it cannot do is emission, so the night pair does not exercise the
# _Glow layer under this flag — G1 (material-name set) and G3 (three.js
# round-trip) carry that proof instead. --samples N overrides Cycles sampling.
...
if WORKBENCH:
    ...
    _sd = scene.display.shading
    _sd.light = "STUDIO"
    _sd.color_type = "MATERIAL"
    _sd.show_shadows = True
    _sd.show_cavity = True
    _sd.cavity_type = "BOTH"
