"""Fresh-scene contract validation for 541-presidio.glb.

    blender -b --python validate_541_presidio.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.4518601, 37.7969312],
        "front_heading_deg_true": 120.68,
        "long_axis_heading_deg_true": 30.68,
    ...
    results["checks"] = {
        # 22.3 x 25.1 x 10.0 m: the XY box is the ~31 deg rotation of a
        # 19.77 x 11.65 m house plus a 1.75 m porch and 0.70 m eaves, not a
        # 22 m building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            9.9 <= dims.z <= 10.1
            and 21.7 <= dims.x <= 22.8
            and 24.5 <= dims.y <= 25.7
        ),
        "crest_normalized_to_target": abs(mx.z - 10.0) <= 0.02,
