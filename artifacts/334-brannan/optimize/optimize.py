...
#   3. limited dissolve — SKIPPED (coplanar parapet/coping ring bands, see below)
#   4. (n/a) curve retess — no curved shells here; the only non-box profile is
#      the 5-segment segmental arch over the entry portal, already minimal
...
#
# GLB-OPTIMIZE-PROMPT §3 step 3 says to skip this step entirely on assets with
# large coplanar ring bands, and 334 Brannan has two: the parapet ring and its
# coping, both ring_band solids whose top and bottom faces are perfectly
# coplanar annuli following the whole 21 x 21 m footprint. A strictly-coplanar
# dissolve merges each annulus into one ngon and re-triangulating it emits
# metre-long, sub-millimetre slivers whose averaged vertex normals collapse to
# ~0. Blender hides that on import; gltfpack re-emits the STORED normals and the
# stage-2 contract validator then fails on invalid_or_nonunit_loop_normal_count.
# Measured on the sibling 350-brannan (13 Aug 2026): 7 slivers up to 24.35 m
# long, bought for 30 triangles (0.4%). Not a trade. Skipped here by rule.
