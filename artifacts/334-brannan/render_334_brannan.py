"""Controlled review renders of the exported 334 Brannan Street GLB.

    blender -b --python render_334_brannan.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 334-brannan]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Brannan Street front faces SE (135.1 deg).
...
RES = (1400, 1100)   # this building is a 45-deg-on square: near-equal spans
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "334-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "334-brannan")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(92)  # from the E: catches the front AND the NE flank
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, gold frieze edge, roof deck and bulkhead --------
...
def render_aerial(center, span, out, prefix):
    ...
    az = math.radians(92)  # from the E — the Brannan front AND the exposed
    # northeast flank in one frame, which is what the app's camera gets.
    # Square-on from the SE would put the blind party wall in shot instead.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
