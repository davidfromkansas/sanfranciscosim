"""Fresh-scene contract validation for pier-9.glb.

    blender -b --python validate_pier_9.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3967994, 37.8006708],
        "front_heading_deg_true": 234.59,
        "flank_heading_deg_true": 324.59,
        "long_axis_heading_deg_true": 54.59,
        "target_height_m": 17.6,
        ...
        "origin_convention": (
            ...
            "anchor and the app's DEM carries this pier as a ~2.5 m ridge, so a "
            "model sitting on z = 0 would float 2.6 m above its own deck. The "
            "deck fascia and pile stubs therefore run to z = -2.6 m and "
            "min_z_m is NEGATIVE BY DESIGN. See docs/asset-plans/pier-9.md 2.3."
        ),
    ...
    results["checks"] = {
        # ~235 x 188 x 17.6 m: the XY box is the 54.59 deg rotation of a
        # 254 x 49 m pier, not a 235 m wide object. See REPORT.md.
        "meters_and_plausible_dimensions": (
            17.5 <= dims.z <= 17.7
            and 230.0 <= dims.x <= 240.0
            and 183.0 <= dims.y <= 193.0
        ),
        "vertical_extent_normalized_to_target": abs(dims.z - 17.6) <= 0.02,
        ...
        "sub_deck_depth_expected": -2.65 <= mn.z <= -2.55,
