// Stage-5 local QA for pier-9, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/pier-9/qa_local.mjs [--drill]
//
...
const LON = -122.3967994;
const LAT = 37.8006708;
const SLUG = 'pier-9';
const CAMERA = { distance: 450, yaw: 235, pitch: 20 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qaPier9-'));
  const chrome = spawn(findChrome(), [
    ...
    // GPU-backed headless (ANGLE/Metal): under parallel-session load ~300 the
    // SwiftShader software rasterizer cannot produce a first frame inside the
    // boot gate at all; the real GPU renders the diorama in seconds and keeps
    // the QA off the contended CPU.
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('pier-9') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasPier9: window.SF.assets.placed.has('pier-9'),
    placePier9: window.SF.assets.placed.get('pier-9') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('pier-9').log, scale: window.SF.assets.placed.get('pier-9').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedPier9:', placed.hasPier9, JSON.stringify(placed.placePier9));
  ...
  // Pier 9 uses the pier-1 deck-top origin: the app's DEM carries the pier as
  // a ~2.5 m centreline ridge and placeGeneric seats the origin on that sample
  // at the anchor. Expect y ~ 2.4-2.6, NOT 0 — a seat of 0 means the anchor
  // drifted off the ridge into open water, and the fix is to move it back onto
  // the measured bbox centre, never to shim the model.
  if (!DRILL) {
    const seat = await evaluate(`(() => {
      const p = window.SF.assets.placed.get('pier-9');
    ...
    check('seated on the DEM deck ridge (y ~ 2.5)', seat && seat.y > 2.2 && seat.y < 2.8,
  ...
  if (DRILL) {
    ...
    // single-shot warn(): expect `sf-assets: pier-9 failed to load (...)` with no
