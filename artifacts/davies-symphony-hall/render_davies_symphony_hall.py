"""Controlled review renders of the exported Davies Symphony Hall GLB.

    blender -b --python render_davies_symphony_hall.py -- [--glb FILE] [--out DIR]
                                                          [--prefix davies-symphony-hall]
                                                          [--only aerial]
...
The aerial is taken from the NORTH-EAST because that is the only angle from
which the front arc reads as an arc — the whole point of the building.
"""
...
RES = (1700, 700)          # elevations: wide, low subject (125 x 35 m)
AER_RES = (1400, 1050)
TOP_RES = (1500, 1400)
...
def day_glow_alpha(alpha=0.12):
    """The app draws the *_Glow set in a separate layer at ~12% alpha by day.
    The GLB itself is fully opaque (the contract forbids transparency); this
    only mirrors the runtime compositing so the daylight reviews show the
    glazing behind the glow shells instead of a wall of amber."""
    for mat in bpy.data.materials:
        if not mat.use_nodes or not ("_Glow" in mat.name):
            continue
...
def go_night():
    ...
    """The app's night pass makes *_Glow emissive; preview that in deep dusk."""
    for mat in bpy.data.materials:
        ...
        if not bsdf:
            continue
        if mat.name.endswith("_Glow") or "_Glow." in mat.name:
            bsdf.inputs["Emission Color"].default_value = bsdf.inputs["Base Color"].default_value
            bsdf.inputs["Emission Strength"].default_value = 6.0
    world = bpy.context.scene.world
    bg = world.node_tree.nodes["Background"]
    bg.inputs[0].default_value = (0.035, 0.05, 0.10, 1.0)
...
def main():
    ...
    out = arg("--out", here)
    prefix = arg("--prefix", "davies-symphony-hall")
    only = arg("--only", "")
    ...
    az = math.radians(48)            # north-east: square onto the arc
    r = width * 2.9
    aer.location = Vector((
    ...
    aim(aer, Vector((center.x, center.y, 8.0)))

    if only in ("", "aerial", "quick"):
        render_to(os.path.join(out, f"{prefix}-aerial.png"), aer, AER_RES)
    if only == "quick":
        top = make_camera("cam_top_quick")
        top.data.type = "ORTHO"
        top.data.ortho_scale = width * 1.04
        top.location = Vector((center.x, center.y, mx.z + 400))
    ...
    # --- top view: the shell roof is the surface the app camera lives on ----
