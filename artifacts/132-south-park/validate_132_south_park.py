"""Fresh-scene contract validation for 132-south-park.glb.

    blender -b --python validate_132_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3946173, 37.7815393],
        "front_heading_deg_true": 135.1,
        "target_height_m": 12.07,
    ...
    results["checks"] = {
        # The lot is 6.7 x 30.0 m at a 135.1 deg heading, so its axis-aligned
        # box is ~26.7 x 26.7 m. That is the expected consequence of the
        # real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 11.9 <= dims.z <= 12.2
        and 26.0 <= dims.x <= 27.5
        and 26.0 <= dims.y <= 27.5,
        "crest_normalized_to_target": abs(mx.z - 12.07) <= 0.02,
