// Stage-5 local QA for pier-17, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/pier-17/qa_local.mjs [--drill]
//
...
const LON = -122.3981018;
const LAT = 37.8022149;
const SLUG = 'pier-17';
const CAMERA = { distance: 420, yaw: -55, pitch: 30 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qapier17-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('pier-17') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasPier17: window.SF.assets.placed.has('pier-17'),
    placePier17: window.SF.assets.placed.get('pier-17') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('pier-17').log, scale: window.SF.assets.placed.get('pier-17').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedPier17:', placed.hasPier17, JSON.stringify(placed.placePier17));
  console.log('  placedIds:', placed.placedIds.join(','));
  console.log('  console lines:', console_.length);
  for (const l of console_) if (/pier-17|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
