"""Fresh-scene contract validation for pier-17.glb.

    blender -b --python validate_pier_17.py -- [--glb FILE] [--out FILE]

Factory-resets, imports only the final GLB, writes the machine-readable report.
"""
...
def main():
    ...
    # Visibility-ray normal test: rays inward toward interior targets; the
    # first face each ray meets must face outward. Targets stay inside the
    # SHED (|offset| small) rather than the full AABB, because the AABB of a
    # rotated pier is mostly empty water.
    ray_hits = 0
    ray_flipped = 0
    golden = math.pi * (3.0 - math.sqrt(5.0))
    targets = [
        Vector((center.x + dx, center.y + dy, mn.z + fz * dims.z))
        for dx, dy in ((0.0, 0.0), (30.0, 21.0), (-30.0, -21.0))
        for fz in (0.18, 0.45, 0.60)
    ]
    ...
    results = {
        ...
        "normal_ray_cast_flipped_fraction":
            round(ray_flipped / ray_hits, 6) if ray_hits else None,
        ...
        "normal_orientation_method": (
            ...
            "reimported loop normals must be finite/unit; per-object signed "
            "volume is authoritative for this union of solids; 31,500 "
            "deterministic visibility rays test the first visible face from "
            "nine interior targets, 0.15% residual allowed at coincident faces."
        ),
        ...
        "anchor_lonlat": [-122.3981018, 37.8022149],
        "front_heading_deg_true": 234.9,
    ...
    results["checks"] = {
        # deck ring 243 x 53-61 m at a 54.9 deg heading -> ~234 x 183 m AABB
        "meters_and_plausible_dimensions": 21.28 <= dims.z <= 21.32
        and 232.0 <= dims.x <= 236.0
        and 181.0 <= dims.y <= 185.0,
        "crest_normalized_to_target": abs(mx.z - TARGET_HEIGHT) <= 0.02,
    ...
    with open(output, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print(json.dumps({k: results[k] for k in ("overall", "checks",
                                              "triangle_count", "dimensions_m",
                                              "min_z_m", "xy_center_offset_m",
                                              "materials", "object_count")},
                     indent=2))
