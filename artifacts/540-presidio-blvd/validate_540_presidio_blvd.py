"""Fresh-scene contract validation for 540-presidio-blvd.glb.

    blender -b --python validate_540_presidio_blvd.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.4519224, 37.7966667],
        "front_heading_deg_true": 83.51,
        "target_height_m": 11.5,
    ...
    results["checks"] = {
        # 16.7 x 22.8 x 11.5 m: the XY box is the 6.49 deg rotation of a
        # 14.5 x 19.7 m footprint grown by the roof overhang, the porch, the
        # steps and the hedges — not a fat building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            11.4 <= dims.z <= 11.6
            and 16.2 <= dims.x <= 17.2
            and 22.3 <= dims.y <= 23.3
        ),
        "crest_normalized_to_target": abs(mx.z - 11.5) <= 0.02,
