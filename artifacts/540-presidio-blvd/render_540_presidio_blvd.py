"""Controlled review renders of the exported 540 Presidio Boulevard GLB.

    blender -b --python render_540_presidio_blvd.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 540-presidio-blvd]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the porch front
faces ESE (83.51 deg true), so the EAST elevation is the front of the house.
...
RES = (1300, 1100)   # a compact house: 17 x 23 m in plan, 11.5 m tall
AER_RES = (1500, 1150)
TOP_RES = (1200, 1300)
...
def light_glow():
    ...
    Kept close to 1.0 on purpose. assets.js draws the glow set as a flat unlit
    MeshBasicMaterial with no bloom, so a hot value would flatter the asset into
    something the scene will never show; 2.0 is the smallest value that survives
    the render's Standard view transform at this exposure."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "540-presidio-blvd.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "540-presidio-blvd")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(128)  # off the south-east front, matching the day aerial
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: the ridge, the four hips, the eave shadow, the chimneys ---
    ...
    pitch = math.radians(31)  # 30-50 deg downward
    # From the south-east: the porch front (83.5 deg true) reads three-quarter,
    # the south hip and the ridge read across it, and both chimneys stay clear
    # of each other. Straight down the front at 84 deg would flatten the hips.
    az = math.radians(128)
    r = span * 3.6
    aer.location = Vector(
        (
