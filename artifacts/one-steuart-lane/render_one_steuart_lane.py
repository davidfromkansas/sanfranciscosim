"""Controlled review renders of the exported One Steuart Lane GLB.

    blender -b --python render_one_steuart_lane.py -- [--glb FILE] [--out DIR]
                                               [--prefix one-steuart-lane] [--night]
...
The building stands at 45 deg to the compass, so a cardinal camera would show two
faces at once. These are TRUE elevations — one camera per measured face normal —
and the file names are a quarter-turn relabelling of them:

    -north  = the Steuart Lane / Bay elevation   (NE, 44.2 deg)
    -east   = the south-east flank               (SE, 134.8 deg)
    -south  = the south-west flank               (SW, 224.5 deg)
    -west   = the Howard Street elevation        (NW, 314.9 deg)

The subject of this building is the STACK, which reads best on a corner, so the
aerial camera looks down the east corner (the bisector of the Steuart and
south-east normals) and gets the zig-zag of both elevations in one frame. The top
view exists to check the roof composition and the penthouse crest.
...
RES = (1000, 1450)   # portrait: this subject is 67 m tall and 47 m wide
AER_RES = (1500, 1300)
TOP_RES = (1500, 1500)
...
VIEWS = [
    ("north", 44.2),   # Steuart Lane / Bay elevation (NE)
    ("east", 134.8),   # south-east flank
    ("south", 224.5),  # south-west flank
    ("west", 314.9),   # Howard Street elevation (NW)
]
# The east corner: the bisector of the Steuart (44.2) and south-east (134.8)
# normals, which is where the alternating setbacks read as a zig-zag.
AERIAL_AZ = 89.5
...
def light_glow():
    ...
    Emission Colour must be COPIED FROM BASE COLOUR, not merely turned up: glTF
    writes `emissiveFactor = 0` when the authored emission strength is 0, so a
    re-imported _Glow material carries a DEFAULT WHITE emission and every glow
    surface renders as a white slab. The app's night layer is an unlit overlay
    drawn at the material's own baked colour, so strength 1.0 on the base colour
    is also what the app actually does. See docs/asset-plans/README.md."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "one-steuart-lane.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "one-steuart-lane")
    night = "--night" in argv
    global FAST, GPU
    FAST = "--fast" in argv
    GPU = "--gpu" in argv
    ...
    if night:
        ...
        aim(aer, Vector((center.x, center.y, mn.z + (mx.z - mn.z) * 0.46)))
    ...
    if only in ("", "aerial"):
        ...
        aer.data.type = "PERSP"
        aer.data.lens = 92.0  # long lens, restrained perspective (style bible s.18)
    ...
    if only in ("", "elev"):
        # ortho_scale drives the LONGER sensor axis, which is vertical here
        ortho_scale = (mx.z - mn.z) * 1.12
        dist = tall * 2.2
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), center.z))
