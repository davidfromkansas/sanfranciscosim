...
# EEVEE, not Cycles, on this machine: it was carrying ~170 load average from
# other parallel Blender sessions and a single Cycles frame did not finish in
# two minutes. The A/B comparison only requires that BOTH files render through
# the SAME rig, which they do; EEVEE is emission- and alpha-capable, so the
# day glow-alpha pass and the night emission pass both still work. Recorded per
# GLB-OPTIMIZE-PROMPT s.10 (no substitutions without documenting them).
...
set_state("day")
# ortho_scale drives the LONGER sensor axis, which is HORIZONTAL at 960x720.
# long_axis * 1.15 therefore cropped 15% off the top and bottom of a 67 m tower;
# size it so the vertical coverage clears the height instead.
scale = long_axis * 1.15 * RES[0] / RES[1]
