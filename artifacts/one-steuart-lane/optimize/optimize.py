...
#   3. (skipped) limited dissolve — ring-band asset, see the note at step 3
#   4. (skipped) curve retess — only the two 12-seg cooling towers are curved,
#      and they read as circles on the roof from the aerial camera
...
# GLB-OPTIMIZE-PROMPT v2 s.3 step 3: skip the dissolve entirely on assets with
# large coplanar ring bands. This one is full of them — the roof parapet, the
# four terrace slab plates, their soffits and the four glow strips are all
# closed annuli that follow a volume plan the whole way round. Even a strictly
# coplanar dissolve merges each into one annulus ngon, and re-triangulating an
# annulus emits sub-millimetre slivers whose averaged vertex normals collapse to
# zero. Blender hides that on import; gltfpack re-emits the stored normals, so
# it surfaces only after the shipping swap, as invalid_or_nonunit_loop_normal_count.
# Measured worth on 350-brannan: 0.4% of triangles. Not worth the risk here.
stats["limited_dissolve"] = "skipped (ring-band asset, prompt s.3 step 3)"
