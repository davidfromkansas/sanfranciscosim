"""Fresh-scene contract validation for one-steuart-lane.glb.

    blender -b --python validate_one_steuart_lane.py -- [--glb FILE] [--out FILE]
...
Normals are checked two ways, because this asset is a union of ~1,000 closed
...
# The manifest target height: the mechanical penthouse cap, which must be the
# export bounding-box top so the loader's targetHeightM / measuredHeight is 1.0.
TARGET_HEIGHT_M = 67.06
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3916888, 37.7915643],
        "long_axis_heading_deg_true": 134.8,
        "front_normal_heading_deg_true": 44.2,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        # XY is the 45-degree-heading bounding box of a 40.5 x 47.0 m lot
        # (62.10 x 61.65 m) plus the 0.44 m travertine frame projection and the
        # 3.5 m entrance canopy that oversails the Steuart Lane sidewalk — not
        # any single elevation length.
        "meters_and_plausible_dimensions": 67.0 <= dims.z <= 67.1
        and 62.0 <= dims.x <= 63.5
        and 61.5 <= dims.y <= 63.0,
