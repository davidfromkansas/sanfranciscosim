"""Fresh-scene contract validation for 140-south-park.glb.

    blender -b --python validate_140_south_park.py -- [--glb FILE] [--out FILE]
...
TRI_BUDGET = 7000
TARGET_H = 10.68   # cornice crest, DataSF LiDAR hgt_maxcm (plan s.2.1)
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3947379, 37.7814643],
        "front_heading_deg_true": 135.0,
    ...
    results["checks"] = {
        # A 29.81 x 6.84 m bar standing at 45 deg to the world axes projects to
        # a near-SQUARE axis-aligned bbox of about 26 x 26 m. That is expected,
        # not a sign the footprint got squared up - the elevations and the top
        # view are where the 4.4:1 stick is checked.
        "meters_and_plausible_dimensions": TARGET_H - 0.2 <= dims.z <= TARGET_H + 0.2
        and 25.0 <= dims.x <= 28.0
        and 25.0 <= dims.y <= 28.0,
