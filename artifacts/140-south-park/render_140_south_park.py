"""Controlled review renders of the exported 140 South Park Street GLB.

    blender -b --python render_140_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 140-south-park]
                                                    [--night]
...
Street front faces SE (135.0 deg), so the "south" and "east" elevations each
show it at 45 deg and neither shows it square. That is correct and expected:
the building stands at 45 deg to the world axes.
...
RES = (1500, 1000)   # landscape: the lot is 29.8 m long and only 10.7 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "140-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "140-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(125)  # from the ESE: the shopfront glow is the identity
        r = span * 4.3
        aer.location = Vector(
            (
    ...
    pitch = math.radians(25)  # low end of the app band: the lot is 29.8 m long
    # The front faces 135 deg, so 125 swings the camera 10 deg toward the
    # NORTH-EAST flank - the open side passage, a real elevation. Swinging the
    # other way (145) shows the south-west party wall, which is deliberately
    # blank and reads as a featureless slab.
    az = math.radians(125)
    r = span * 4.3
    aer.location = Vector(
        (
