"""Controlled review renders of the exported 599 Third Street GLB.

    blender -b --python render_599_third.py -- [--glb FILE] [--out DIR]
                                               [--prefix 599-third] [--night]
...
front faces south-west and the Brannan front south-east.

The hero view is the aerial from due south, where the two designed street
elevations meet at the 3rd/Brannan corner; the top view carries the working
roof (skylights, condensers, deck pads, penthouse).
...
# Due south: the 3rd Street front (normal 224.8 deg) and the Brannan front
# (normal 135.2 deg) are symmetric about this azimuth, so both designed
# elevations and the corner between them read in one frame (style bible s.18).
AERIAL_AZ = 180.0
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "599-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "599-third")
    night = "--night" in argv
    only = arg("--only", "")
    ...
    if night:
        ...
        aim(aer, Vector((center.x, center.y, mn.z + 7.0)))
    ...
    if only in ("", "elev"):
        ...
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), mn.z + 9.0))
            aim(cam, Vector((center.x, center.y, mn.z + 9.0)))
