"""Fresh-scene contract validation for 599-third.glb.
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(18.3 / dims.z, 6),
        ...
        "anchor_lonlat": [-122.3942739, 37.7804504],
        "long_axis_heading_deg_true": 44.8,
        "front_normal_heading_deg_true": 224.8,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        "meters_and_plausible_dimensions": 18.2 <= dims.z <= 18.4
        and 42.5 <= dims.x <= 44.0
        and 42.2 <= dims.y <= 43.6,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 18.3) <= 0.01,
