"""Fresh-scene contract validation for pier-7.glb.

    blender -b --python validate_pier_7.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3955159, 37.7994429],
        "front_heading_deg_true": 234.65,
        "pier_axis_deg_true": 54.65,
        "target_height_m": 7.6,
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 7.55 <= dims.z <= 7.65
        and 218.0 <= dims.x <= 221.0
        and 164.5 <= dims.y <= 167.5,
        ...
        # The origin is the footprint's OBB centre (the manifest anchor). The
        # AABB centre differs by ~0.6 m because the mid bay and end platform
        # flare asymmetrically off the walkway line — real geometry, not error.
        "centered_xy": abs(center.x) <= 2.0 and abs(center.y) <= 2.0,
