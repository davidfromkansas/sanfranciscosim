"""Fresh-scene contract validation for 543-presidio-blvd.glb.

    blender -b --python validate_543_presidio_blvd.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.4515746, 37.7973711],
        "front_heading_deg_true": 100.7,
        "target_height_m": 9.55,
    ...
    results["checks"] = {
        # 17.20 x 17.73 x 9.55 m. The XY box is the ~11 deg rotation of a
        # 13.72 x 12.79 m house plus its 0.62 m eaves and its entry porch — NOT
        # a 17 m building. The bands below are +-0.4 m around the built values,
        # wide enough to survive a detail tweak and narrow enough that a real
        # scale error (a factor, a unit slip, a lost recentre) fails them.
        # See REPORT.md "Dimensions".
        "meters_and_plausible_dimensions": (
            9.4 <= dims.z <= 9.7
            and 16.8 <= dims.x <= 17.6
            and 17.3 <= dims.y <= 18.1
        ),
        "crest_normalized_to_target": abs(mx.z - 9.55) <= 0.02,
