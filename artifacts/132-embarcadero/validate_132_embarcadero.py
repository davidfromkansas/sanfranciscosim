"""Fresh-scene contract validation for 132-embarcadero.glb.

    blender -b --python validate_132_embarcadero.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3925476, 37.7931482],
        "front_heading_deg_true": 44.95,
        "target_height_m": 29.57,
    ...
    results["checks"] = {
        # The 44.95 deg heading turns a 13.75 x 42.95 m building into a ~40.5 m
        # axis-aligned bounding box. That is expected, not a scale error.
        "meters_and_plausible_dimensions": 29.4 <= dims.z <= 29.75
        and 39.0 <= dims.x <= 42.0
        and 39.0 <= dims.y <= 42.0,
        "crest_normalized_to_target": abs(mx.z - 29.57) <= 0.02,
