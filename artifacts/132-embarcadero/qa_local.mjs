// Stage-5 local QA for 132-embarcadero, INTEGRATION-PROMPT.md Steps 5 and 6.
//
// Adapted from artifacts/434-brannan/qa_local.mjs — same rig, this asset's
// anchor, camera preset and camelId. yaw 135 (not 45) because this building's
// public elevation is the Embarcadero front, outward normal 44.95 deg true, and
// camera.yaw = 180 - true bearing; yaw 45 stares at the Steuart entrance.
//
...
//
//   node artifacts/132-embarcadero/qa_local.mjs [--drill]
//
...
const SHOTS = path.join(HERE, 'integration', 'qa');
const DRILL = process.argv.includes('--drill');

const LON = -122.3925476;
const LAT = 37.7931482;
const SLUG = '132-embarcadero';
const CAMERA = { distance: 240, yaw: 135, pitch: 26 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa132emb-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('132Embarcadero') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasAsset: window.SF.assets.placed.has('132Embarcadero'),
    placeAsset: window.SF.assets.placed.get('132Embarcadero') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('132Embarcadero').log, scale: window.SF.assets.placed.get('132Embarcadero').scale })) : null,
  }))()`);
