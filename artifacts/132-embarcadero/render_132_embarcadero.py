"""Controlled review renders of the exported 132 The Embarcadero GLB.

    blender -b --python render_132_embarcadero.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 132-embarcadero]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Embarcadero front faces NE (44.95 deg).
...
RES = (1200, 1400)   # a 29.6 m slab on a 40 m AABB: portrait framing
AER_RES = (1400, 1250)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "132-embarcadero.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "132-embarcadero")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(15.0)  # NNE: the Embarcadero front (44.95 deg) plus
        #   the long exposed northwest flank — the three-quarter this slab needs
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, crown band, bulkhead and roof plant ----
    ...
    az = math.radians(15.0)  # NNE — square on to the Embarcadero front would
    #   flatten a 13.75 x 42.95 m slab into a single 13.75 m rectangle; 15 deg
    #   off shows the address elevation AND the 43 m depth that defines it
    r = span * 3.4
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
