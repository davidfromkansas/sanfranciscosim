"""Fresh-scene contract validation for 92-south-park.glb.

    blender -b --python validate_92_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3941549, 37.7819082],
        "front_heading_deg_true": 135.1,
        "target_height_m": 13.28,
    ...
    results["checks"] = {
        # The lot is 14.45 x 30.04 m at a 135.1 deg heading, so its axis-aligned
        # box is ~31.9 x 31.9 m. That is the expected consequence of the
        # real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 13.1 <= dims.z <= 13.4
        and 31.0 <= dims.x <= 32.6
        and 31.0 <= dims.y <= 32.6,
        "crest_normalized_to_target": abs(mx.z - 13.28) <= 0.02,
