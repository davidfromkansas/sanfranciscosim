"""Controlled review renders of the exported 92 South Park GLB.

    blender -b --python render_92_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 92-south-park]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
RES = (1200, 1200)   # square: a 45-deg lot whose AABB is 31.9 x 31.9 m
AER_RES = (1500, 1150)
TOP_RES = (1250, 1250)
...
def render_top(center, mx, width, span, out, prefix):
    """Top view: the whole lot — front block, open courtyard, rear cottage."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "92-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "92-south-park")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    # --- top view: front block, open courtyard, rear cottage ---------------
    ...
    # 135 deg is dead-on the South Park front and flattens the lot; 105 is a
    # three-quarter to the building's own 135 deg axis, so the bay fronts, the
    # NE flank, and — critically — the front block / courtyard / rear cottage
    # rhythm running away from the camera all read in one frame.
    az = math.radians(105)
    r = span * 4.4
    aer.location = Vector(
        (
