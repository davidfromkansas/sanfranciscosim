// Stage-5 local QA for 58-south-park, INTEGRATION-PROMPT.md Steps 5 and 6.
//
// Adapted from artifacts/340-brannan/qa_local.mjs — constants only (anchor,
...
//
//   node artifacts/58-south-park/qa_local.mjs [--drill]
//
...
const LON = -122.3938881;
const LAT = 37.7821223;
const SLUG = '58-south-park';
const CAMERA = { distance: 150, yaw: 45, pitch: 26 }; // the registry preset
const PIPELINE_ID = '58SouthPark'; // camelId(SLUG) — the key assets.placed uses
...
async function main() {
  ...
  const stats = DRILL
    ...
    : await until('58-south-park live',
        `(() => { const s = window.SF.assets.stats();
  ...
  const placed = await evaluate(`(() => ({
    ...
    placed58: window.SF.assets.placed.has('58SouthPark'),
  }))()`);
