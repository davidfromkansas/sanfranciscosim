"""Controlled review renders of the exported 54-58 South Park GLB.

    blender -b --python render_101_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 58-south-park]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the South Park front faces SE (135.2 deg).
...
RES = (1200, 1250)   # portrait: 9.7 m of frontage against a 16.9 m crest
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    ...
    for mat, bsdf in glow_materials():
        if bsdf:
            # This asset's only glow colour is the pale blue Toy_glass_Glow
            # (6f95b8), which is already a light value: 3.0 clipped every lit
            # opening to flat white and threw the colour away. 1.8 keeps the
            # glazed bay reading as lit GLASS rather than as a hole punched in
            # the facade.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "58-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "58-south-park")
    night = "--night" in argv
    only = arg("--only", "")
    ...
    if night:
        ...
        az = math.radians(105)  # three-quarter: front + the north-east flank
        r = span * 3.4
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, skylight field, mechanical cluster, penthouse -----
    if only != "aerial":
    ...
    pitch = math.radians(32)  # 30-50 deg downward
    # Square onto the front (135 deg) hides both 30 m flanks edge-on, which is
    # most of this building. 105 deg sits between the front normal (135) and the
    # north-east flank normal (45) and shows both — a true three-quarter.
    az = math.radians(105)
    r = span * 3.4
    aer.location = Vector(
        (
