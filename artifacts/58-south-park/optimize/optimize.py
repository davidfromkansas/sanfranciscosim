...
#   3. (skipped) limited dissolve — this asset has coplanar ring bands
#   4. (skipped) curve retess — this asset has no curved shells at all
...
# --- 3. limited dissolve: SKIPPED (asset-specific, 58-south-park) ---
# GLB-OPTIMIZE-PROMPT v2 s.3 step 3: "Skip this step entirely on assets with
# large coplanar ring bands". This asset has three — `parapet`, `coping` and
# `rear_low_cap` all follow the footprint the whole way round, and the main
# coping ring is 9.7 x 30.1 m. Their top and bottom faces are perfectly
# coplanar annuli, so even a strictly-coplanar dissolve merges each into one
# annulus ngon, and re-triangulating an annulus emits ~0.2 mm-wide slivers up
# to 30 m long. Those pass an area-based degeneracy test, survive Phase B and
# Phase E, and only surface two steps later as
# `invalid_or_nonunit_loop_normal_count` in the STORED normals of the packed
# file (measured on 350-brannan, 13 Aug 2026).
#
# The step is also the cheapest one in Phase B: on 350-brannan it was worth
# 0.4%. Not worth manufacturing degenerate geometry for.
stats["limited_dissolve"] = "skipped: coplanar ring bands (parapet, coping, rear_low_cap)"
...
# --- 6b. re-assert flat shading (carried over from 181-south-park) ---
# The step-1 weld merges coincident verts inside each object, and where two
# faces of different orientation shared those verts their loop normals get
# averaged. On this asset that put a smooth gradient across the roof slopes —
# 8x-amplified it was the only thing the A/B diff showed, and it softens exactly
# the faceted read the style bible asks for. The source model is authored
# shade_flat throughout, so re-asserting it after the weld is a restoration, not
# a change: it took the mean loop-normal-vs-face-normal deviation from 0.0146
# back to the input's 0.0003 and the day-near pixel delta from 1.50% to ~0.
for o in mesh_objs():
    o.data.shade_flat()
