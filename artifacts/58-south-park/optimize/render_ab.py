...
# EEVEE, not Cycles: these are flat untextured opaque materials, so the engines
# are visually equivalent, and the A/B gate only compares two renders made the
# same way. The authoring machine was running several batch sessions at once and
# Cycles CPU could not finish the pass. Set SF_AB_ENGINE=CYCLES to switch back.
if os.environ.get("SF_AB_ENGINE", "EEVEE").upper() == "EEVEE":
