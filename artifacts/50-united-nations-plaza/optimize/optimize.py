...
# Per-asset switches. 50 UNP is dominated by large COPLANAR RING BANDS - the
# plinth, belt course, main cornice, attic balustrade, top cornice and the two
# north parapets each follow the whole footprint - which is precisely the case
# GLB-OPTIMIZE-PROMPT §3.3 says to skip the limited dissolve on. Both switches
# exist so the four-variant table can be MEASURED rather than assumed.
DO_WELD = "--no-weld" not in argv
DO_DISSOLVE = "--no-dissolve" not in argv
...
if DO_WELD:
    for o in mesh_objs():
        ...
        bm.to_mesh(o.data)
        bm.free()
snap("weld+degenerate" if DO_WELD else "weld+degenerate (SKIPPED)")
...
# bmesh.ops.dissolve_limit rather than the edit-mode operator: identical result,
# but 548 objects x (mode_set + select_all + operator) round-trips took longer
# than the rest of the pass combined on this asset.
for o in (mesh_objs() if DO_DISSOLVE else []):
    bm = bmesh.new()
    bm.from_mesh(o.data)
    # 0.05 deg, not 0.5: dissolve merges transitively, and a 0.5-deg chain
    # accumulates into twisted ngons whose triangulation flips windings
    # (caught by the 22.5k-ray test). Strictly coplanar runs only.
    bmesh.ops.dissolve_limit(bm, angle_limit=0.000872665,
                             verts=bm.verts, edges=bm.edges,
                             delimit={"MATERIAL", "SHARP"})
    bm.to_mesh(o.data)
    bm.free()
snap("limited-dissolve" if DO_DISSOLVE else "limited-dissolve (SKIPPED)")
...
stats["switches"] = {"weld": DO_WELD, "dissolve": DO_DISSOLVE}
