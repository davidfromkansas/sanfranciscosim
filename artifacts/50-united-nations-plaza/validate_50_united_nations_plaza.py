"""Fresh-scene contract validation for 50-united-nations-plaza.glb.

    blender -b --python validate_50_united_nations_plaza.py --
        [--glb FILE] [--out FILE]
...
TRI_BUDGET = 24000  # this asset's task cap (repo landmark ceiling is 27k)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "50-united-nations-plaza.glb"))
    ...
    for obj in meshes:
        ...
        # Signed volume is the authoritative normals test for a union of
        # solids: every piece of this asset is a closed solid, so each one
        # must enclose a POSITIVE volume with outward winding.
        vol = 0.0
        for tri in me.loop_triangles:
            a, b, c = (obj.matrix_world @ me.vertices[i].co for i in tri.vertices)
            vol += a.dot(b.cross(c)) / 6.0
        if vol <= 0.0:
            negative_volume_objects.append(obj.name)
    ...
    # Visibility-based outward-normal test. This asset is a RING around an open
    # courtyard, so the deterministic ray sphere targets the four wings and the
    # courtyard at three heights; a visible outward face must oppose the
    # incoming ray direction. Signed volume above is the authoritative test for
    # a union of solids; this one catches an exposed inward face.
    ray_hits = 0
    ray_flipped = 0
    golden = math.pi * (3.0 - math.sqrt(5.0))
    targets = [
        Vector((center.x + dx * dims.x, center.y + dy * dims.y, mn.z + fz * dims.z))
        for dx, dy in ((0.0, 0.0), (-0.22, -0.22), (0.22, 0.22), (-0.22, 0.22), (0.22, -0.22))
        for fz in (0.1, 0.25, 0.8)
    ]
    for target in targets:
        for i in range(1500):
            y = 1.0 - 2.0 * (i + 0.5) / 1500
    ...
    results = {
        ...
        "negative_signed_volume_objects": negative_volume_objects,
        "normal_ray_cast_flipped_fraction": round(ray_flipped / ray_hits, 6)
        if ray_hits
        else None,
        "normal_orientation_status": "PASS"
        if invalid_normal_count == 0
        and not negative_volume_objects
        and ray_hits > 0
        and ray_flipped / ray_hits <= 0.0015
        else "FAIL",
        "normal_orientation_method": "All source meshes run bmesh.ops.recalc_face_normals before export. Per-object SIGNED VOLUME is authoritative for this union of solids and must be positive for every object; reimported loop normals must be finite/unit; 22,500 deterministic visibility rays over 15 targets test the first visible face, residual <= 0.15%.",
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 118 <= dims.x <= 128 and 80 <= dims.y <= 90 and 32.5 <= dims.z <= 33.5,
    ...
    with open(output, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print(json.dumps({k: results[k] for k in ("overall", "checks", "triangle_count", "dimensions_m", "materials")}, indent=2))
