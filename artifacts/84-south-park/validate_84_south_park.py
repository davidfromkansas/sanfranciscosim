"""Fresh-scene contract validation for 84-south-park.glb.

    blender -b --python validate_84_south_park.py -- [--glb FILE] [--out FILE]
...
# sf-asset-check's flat palette keys, for the informational off-palette report.
PALETTE_KEYS = {
    "Toy_cream", "Toy_sand", "Toy_trim", "Toy_teal", "Toy_coral", "Toy_mustard",
    "Toy_mint", "Toy_sky", "Toy_navy", "Toy_glass", "Toy_glassl", "Toy_ink",
    "Toy_roofd", "Toy_brick", "Toy_stone", "Toy_red", "Toy_steel", "Toy_rust",
    "Toy_gold", "Toy_ioorange", "Toy_verdigris", "Toy_white",
}
...
def main():
    ...
    results = {
        ...
        "off_palette_materials": sorted(
            m for m in (mat.name for mat in bpy.data.materials)
            if m.split("_Glow")[0] not in PALETTE_KEYS
        ),
        "off_palette_note": (
            "Toy_slate (#6d8188) is a deliberate off-palette body colour for the "
            "style bible's SF painted-facade exception. sf-asset-check scores an "
            "off-palette colour as a WARN, not a FAIL; it is reported here rather "
            "than gated. See REPORT.md correction 1."
        ),
        "glow_materials": sorted(m["name"] for m in mat_rows if m["glow"]),
        "anchor_lonlat": [-122.3940683, 37.7819798],
        "front_heading_deg_true": 135.18,
        "long_axis_heading_deg_true": 315.18,
        "target_height_m": 13.2,
    ...
    results["checks"] = {
        # 26.1 x 26.3 x 13.20 m: the XY box is the exact 45 deg rotation of a
        # 6.99 x 30.07 m sliver, not a 26 m building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            13.1 <= dims.z <= 13.3
            and 25.7 <= dims.x <= 26.6
            and 25.7 <= dims.y <= 26.6
        ),
        "crest_normalized_to_target": abs(mx.z - 13.20) <= 0.02,
