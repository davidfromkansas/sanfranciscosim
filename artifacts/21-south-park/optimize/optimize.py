...
#   3. (SKIPPED) limited dissolve — this asset has coplanar ring bands (parapet,
#      coping, two cornice runs); see the comment at the step for why
#   4. (skipped) curve retess — the segmental arches are already at 7 segments,
#      which is the silhouette-defining minimum for this asset
...
# large coplanar ring bands." This asset has four of them following the whole
# 33 m x 41 m footprint - `parapet`, `parapet_coping`, and the two cornice runs
# with their returns - plus the roof deck prism. Their top and bottom faces are
# perfectly coplanar annuli, so even a strictly-coplanar 0.05 deg dissolve
# merges each ring into one annulus ngon and re-triangulating an annulus emits
# slivers metres long and fractions of a millimetre wide. Those slivers pass an
# area-based degeneracy test, survive Phase B and E, and only surface after the
# shipping swap as `invalid_or_nonunit_loop_normal_count` in the stage-2
# contract validator, because gltfpack re-emits the STORED normals and a
# sliver's shared vertex normal has collapsed to ~0. This is the 350-brannan
# failure of 13 Aug 2026, and on that asset the whole step was worth 30
# triangles (0.4%). Not worth re-learning.
stats["steps"].append({"step": "limited-dissolve", "skipped":
                       "coplanar ring bands (parapet, coping, 2 cornice runs)"})
