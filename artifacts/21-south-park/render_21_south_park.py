"""Controlled review renders of the exported 21-29 South Park GLB.

    blender -b --python render_21_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 21-south-park]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
(north = Blender +Y), which is how the asset is authored — the main South Park
front faces NW (315.7 deg) and the angled front WNW (286.7 deg).
...
RES = (1500, 560)    # wide strip: a 51 m elevation of an 11.7 m building
AER_RES = (1500, 1150)
TOP_RES = (1200, 1200)
...
def render_top(center, mx, width, span, out, prefix):
    """Top view: the bend, the empty park-side apron and the equipment field."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "21-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "21-south-park")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(300)  # three-quarter, same as the day aerial
        r = span * 4.4
        aer.location = Vector(
            (
    ...
    # --- top view: the bend, the apron, the equipment field, the bulkhead ---
    ...
    # The two front planes face 315.7 and 286.7 deg and every other side is a
    # party wall, so the only informative eye is out over the park. Standing at
    # bearing 300 splits the two planes: the bend reads as a bend, both ranks of
    # openings stay open, and the roof's empty apron sits between them. Standing
    # square on either normal collapses the other plane and the bend with it.
    az = math.radians(300)
    r = span * 4.0
    aer.location = Vector(
        (
