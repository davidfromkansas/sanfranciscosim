"""Fresh-scene contract validation for 21-south-park.glb.

    blender -b --python validate_21_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3931063, 37.7817676],
        "front_heading_deg_true": [315.7, 286.7],
        "target_height_m": 11.73,
        "cornice_crest_m": 10.2,
        "roof_deck_m": 9.5,
    ...
    results["checks"] = {
        # The building is 32.75 x 40.68 m at a ~46 deg heading, so its
        # axis-aligned box is ~47.4 x 51.5 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 11.6 <= dims.z <= 11.9
        and 46.0 <= dims.x <= 48.5
        and 50.0 <= dims.y <= 52.5,
        "crest_normalized_to_target": abs(mx.z - 11.73) <= 0.02,
