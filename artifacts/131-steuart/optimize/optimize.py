...
# GLB-OPTIMIZE-PROMPT.md s.3 step 3: skip the dissolve entirely on assets with
# large coplanar ring bands. 131-steuart has four full-footprint annuli — the
# cornice, its cap, the gravel stop and the penthouse band — whose top and
# bottom faces are perfectly coplanar. A strictly-coplanar dissolve merges each
# into one annulus ngon, and re-triangulating an annulus emits slivers that pass
# an area-based degeneracy test but collapse the stored vertex normals in the
# packed file (measured on 350-brannan). The step is worth a few dozen triangles
# here and it is the only one that can manufacture degenerate geometry.
stats["limited_dissolve"] = "skipped (coplanar ring bands: cornice, cornice_cap, gravel_stop, pent_band)"
