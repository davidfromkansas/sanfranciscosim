// Stage-5 local QA for 131-steuart, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/131-steuart/qa_local.mjs [--drill]
//
...
const LON = -122.3924386;
const LAT = 37.7930568;
const SLUG = '131-steuart';
const CAMERA = { distance: 200, yaw: 135, pitch: 28 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa131st-'));
  ...
  // The app throttles asset placement to protect frame rate, so under machine
  // load the camera can sit on the anchor with the entry still `far`. Pump the
  // streaming scan by hand as well; it is a no-op when the real loop is running.
  await evaluate(`window.__pump = window.__pump || setInterval(() => { try { window.SF.assets.update(window.SF.camera.position, 0.4); } catch {} }, 250)`);
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('131Steuart') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasLm: window.SF.assets.placed.has('131Steuart'),
    placeLm: window.SF.assets.placed.get('131Steuart') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('131Steuart').log, scale: window.SF.assets.placed.get('131Steuart').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedLm:', placed.hasLm, JSON.stringify(placed.placeLm));
  console.log('  placedIds:', placed.placedIds.join(','));
  console.log('  console lines:', console_.length);
  for (const l of console_) if (/131-steuart|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
  ...
  // INTEGRATION-PROMPT Step 6 quotes the RESIDENT warning text ("keeping the
  // code-built landmark"). This landmark is STREAMED (loadRadius 2500), and a
  // streamed failure goes through scan(), which logs
  // `sf-assets: <id> failed to load (...)` with no "keeping" suffix. Match on
  // the id, not on the prompt's wording.
  const warnings = console_.filter((l) => /sf-assets:/.test(l) && /keeping the code-built|failed to load|error/i.test(l));
  if (DRILL) {
    ...
    check('the loader actually attempted the fetch', s.failed > 0, `failed: ${s.failed}`);
  } else {
