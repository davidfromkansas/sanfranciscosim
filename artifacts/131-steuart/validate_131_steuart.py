"""Fresh-scene contract validation for 131-steuart.glb.

    blender -b --python validate_131_steuart.py -- [--glb FILE] [--out FILE]
...
Normals are checked two ways, because this asset is a union of ~140 closed
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(27.7 / dims.z, 6),
        ...
        "anchor_lonlat": [-122.3924386, 37.7930568],
        "long_axis_heading_deg_true": 44.8,
        "front_normal_heading_deg_true": 224.9,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        # The building is 14.16 x 42.07 m but sits at ~45 deg to the world axes,
        # so its axis-aligned bbox is ~41.2 x 41.3 m. That is the expected
        # consequence of the real heading, not a scale error.
        "meters_and_plausible_dimensions": 27.6 <= dims.z <= 27.8
        and 40.5 <= dims.x <= 42.0
        and 40.5 <= dims.y <= 42.0,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 27.7) <= 0.01,
