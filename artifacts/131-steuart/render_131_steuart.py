"""Controlled review renders of the exported 131 Steuart Street GLB.

    blender -b --python render_131_steuart.py -- [--glb FILE] [--out DIR]
                                               [--prefix 131-steuart] [--night]
...
The aerial three-quarter looks in from the north-east, over the bay: that is the
view the app's camera gets of this building, and it is the only one that shows
the barrel-roofed penthouse crest against the brick cornice behind it. The top
view exists to check the roof composition, which on a 42 m slot is most of the
asset.
...
VIEWS = [
    ("north", 44.8),  # The Embarcadero front (NE) — the 1983 cast-stone face
    ("east", 135.0),  # SE party wall with 141 Steuart
    ("south", 224.9),  # Steuart Street front (SW) — the 1907 brick face
    ("west", 314.6),  # NW party wall with 121 Steuart
]
# A true three-quarter on the north corner: it shows the 3rd Street elevation,
# the Bryant Street elevation and the signed corner crown between them in one
# frame (style bible s.18).
AERIAL_AZ = 20.0
...
def sync_viewport_colors():
    """Workbench's MATERIAL colour mode reads material.diffuse_color, and the
    glTF importer leaves it at the 0.8 grey default — so a --fast review render
    shows the wrong palette unless it is copied across from the BSDF."""
    for mat in bpy.data.materials:
        if not mat.use_nodes:
            continue
        bsdf = mat.node_tree.nodes.get("Principled BSDF")
        if bsdf:
            mat.diffuse_color = tuple(bsdf.inputs["Base Color"].default_value)
            mat.roughness = bsdf.inputs["Roughness"].default_value
...
def light_glow():
    ...
    a re-imported _Glow material carries a DEFAULT WHITE emission colour and
    every glow surface renders as a white slab (see docs/asset-plans/README.md).
    Copy Base Color into Emission Color first — that is also exactly what the
    app does, since its night layer is an unlit overlay drawn at the material's
    own baked colour.
    """
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "131-steuart.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "131-steuart")
    night = "--night" in argv
    global FAST, GPU
    FAST = "--fast" in argv
    GPU = "--gpu" in argv
    ...
    if night:
        ...
        aim(aer, Vector((center.x, center.y, mn.z + 9.0)))
    ...
    if only in ("", "elev"):
        ...
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), mn.z + 13.0))
            aim(cam, Vector((center.x, center.y, mn.z + 13.0)))
