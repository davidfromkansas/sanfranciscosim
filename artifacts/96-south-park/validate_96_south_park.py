"""Fresh-scene contract validation for 96-south-park.glb.

    blender -b --python validate_96_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3943678, 37.7817707],
        "front_heading_deg_true": 135.4,
    ...
    results["checks"] = {
        # The building is 14.44 x 30.06 m at a 135.1 deg heading, so its
        # axis-aligned box is ~31.8 x 27.6 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 13.5 <= dims.z <= 13.9
        and 30.5 <= dims.x <= 33.0
        and 26.5 <= dims.y <= 29.0,
        "crest_normalized_to_target": abs(mx.z - 13.7) <= 0.02,
