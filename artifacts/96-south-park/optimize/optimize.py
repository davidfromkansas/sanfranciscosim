...
# GLB-OPTIMIZE-PROMPT s.3 step 3: skip entirely on assets with large coplanar
# ring bands. This one has EIGHT — `cylinder_cap` (512 tris), `brick_base` (360),
# `plinth` (360), `bronze_coping` (288), `coping_main` (192), `parapet_main`
# (192), `coping_upb` (128) and `parapet_upb` (128), 2,160 of the asset's 6,312
# triangles. Their top and bottom faces are perfectly coplanar
# annuli, so even a strictly-coplanar dissolve merges each ring into one annulus
# ngon and re-triangulating an annulus emits sub-millimetre slivers whose
# averaged vertex normals collapse to ~0. Blender hides that on import; gltfpack
# re-emits the stored normals, so the failure would surface only in the packed
# file, after the shipping swap (measured on 350-brannan, 13 Aug 2026).
# Set DISSOLVE = True to re-enable and measure it.
DISSOLVE = False
