"""Controlled review renders of the exported 86–96 South Park GLB.

    blender -b --python render_96_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 96-south-park]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
(north = Blender +Y), which is how the asset is authored — the South Park front faces SE (135.1 deg).
...
RES = (1200, 1100)   # near-square: a 30.1 m plan under a 13.7 m crest
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "96-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "96-south-park")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(180)  # three-quarter, same as the day aerial
        r = span * 4.0
        aer.location = Vector(
            (
    ...
    # --- top view: the cylinder, the gable, the pergola and the yard notch ----
    ...
    # The front normal is 135.1 deg (South Park) and the exposed SW flank's is
    # 225.2 deg (Jack London Alley), so a camera due SOUTH at 180 deg is the true
    # three-quarter to this corner building: both street elevations sit at 45 deg
    # to the lens and the rooftop cylinder reads against the sky.
    az = math.radians(180)
    r = span * 4.4
    aer.location = Vector(
        (
