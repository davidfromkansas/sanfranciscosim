"""Fresh-scene contract validation for cal-academy.glb.

    blender -b --python validate_cal_academy.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    # Visibility-based outward-normal test. The asset is an intentionally open
    # shell (glass walls under a separate eave plate, an open piazza bowl), so
    # signed-volume/manifold tests cannot judge it. Cast a deterministic sphere
    # of rays toward nine targets spread through the low, wide volume; a
    ...
    targets = [
        ...
        for dx, dy in ((0.0, 0.0), (-0.22, -0.22), (0.22, 0.22))
        for fz in (0.25, 0.5, 0.8)
    ]
    ...
    results["checks"] = {
        # yawed 161.3 x 102.5 m footprint + 8.5 m eave -> ~212.5 x 207.7 AABB;
        # hill crest at the architect's 19.3 m
        "meters_and_plausible_dimensions": 18.5 <= dims.z <= 20.0
        and 200 <= dims.x <= 220
        and 195 <= dims.y <= 215,
    ...
    with open(output, "w", encoding="utf-8") as f:
        ...
        f.write("\n")
    print(json.dumps({k: v for k, v in results.items() if k != "object_details"}, indent=2))
