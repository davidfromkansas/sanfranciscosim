"""Controlled review renders of the exported California Academy of Sciences GLB.

    blender -b --python render_cal_academy.py -- [--glb FILE] [--out DIR]
                                                 [--prefix cal-academy]
...
(north = Blender +Y), which is how the asset is authored. Because the building
is 19.3 m tall and ~212 m wide, the elevations are framed by plan width, not
height, in a wide landscape format.
"""
...
RES = (1800, 560)  # elevations: very low, very wide subject
AER_RES = (1400, 1050)
TOP_RES = (1500, 1500)
...
def add_lights(size):
    ...
    key.energy = 2.1
    key.angle = math.radians(14)  # soft enough to calm smooth-dome terminators
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "cal-academy.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "cal-academy")
    ...
    ortho_scale = width * 1.08  # framed by the plan, not the low height
    dist = width * 2.5
    ...
    # --- top view: the living roof is the hero ------------------------------
    ...
    az = math.radians(292)  # three-quarter onto the NW entrance front + SW end
    r = width * 2.6
    aer.location = Vector(
        (
    ...
    aim(aer, Vector((center.x, center.y, 4.0)))
    render_to(os.path.join(out, f"{prefix}-aerial.png"), aer, AER_RES)

    # --- night state: the app's night pass makes *_Glow emissive; preview ---
    # that here by lighting the same materials in a deep-dusk world
    ...
    bg = world.node_tree.nodes["Background"]
    bg.inputs[0].default_value = (0.035, 0.05, 0.10, 1.0)  # deep dusk blue
    bg.inputs[1].default_value = 0.35
    for light in bpy.data.lights:
        if light.name == "key":
            light.energy = 0.10
            light.color = (0.65, 0.75, 1.0)  # thin moonlight
        else:
            light.energy = 0.03
