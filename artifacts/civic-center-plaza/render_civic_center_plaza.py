...
RES = (1800, 620)    # extreme letterbox: 192 m across and only 30 m tall
AER_RES = (1700, 1150)
TOP_RES = (1700, 2100)   # the primary review image; portrait, the plaza is N-S
...
def main():
    ...
    out = arg("--out", here)
    prefix = arg("--prefix", "civic-center-plaza")
    night = "--night" in argv
    ...
    # Two green bosque slabs, four bright lawn rectangles, one pale gravel bar,
    # two colour playground pads. If any of those six merges into its
    # neighbour, the tonal separation is wrong (plan 2.9).
    ...
    # NOT down the long axis: at 117 deg the camera looks straight along the
    # ridge and the 55 m bar collapses to a roof plane. 158 deg puts the camera
    # off the southwest flank so the elevation, the ridge and the head's own hip
    # all read at once — the true three-quarter the style bible asks for.
    az = math.radians(158)
    r = span * 2.15
    aer.location = Vector(
        (
    ...
    # central court, the view every photograph of this plaza is taken from.
    # City Hall would fill the far end; it is not part of this asset.
    axis = make_camera("cam_axis")
    axis.data.type = "PERSP"
    axis.data.lens = 95.0
    apitch = math.radians(13)
    r2 = span * 1.30
    ...
    aim(axis, Vector((center.x, center.y, mn.z + 8.0)))
