...
def signed_volumes(objs):
    """Closed meshes only — an open shell's signed volume is arbitrary. The
    joined glass/glow/ribbon groups are single-sided sheets by design and are
    covered by the ray test below instead (prompt §3.7)."""
    out = {}
    for o in objs:
        if not is_closed(o):
            continue
...
report["open_shell_groups"] = [o.name for o in objs_out if not is_closed(o)]
...
report["gates"] = {
    ...
    "G2_ray_flip_ok": report["ray_test_output"]["flipped_fraction"] <= 0.0015,
