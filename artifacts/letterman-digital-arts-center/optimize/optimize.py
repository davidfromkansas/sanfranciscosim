...
# --- 7. normals audit ---
# Signed volume is only meaningful for CLOSED meshes (prompt §3.7). This asset
# joins two flat single-sided glow panes into grp_Toy_white_Glow and four
# ribbons/pane sheets into the glass and glow groups — open shells whose signed
# volume is arbitrary. They are covered by validate.py's visibility ray test
# instead; scoring them here only produces false positives.
inverted = [o.name for o in mesh_objs() if is_closed(o) and signed_volume(o) < 0]
stats["open_shell_groups_skipped_in_volume_audit"] = [
    o.name for o in mesh_objs() if not is_closed(o)
]
