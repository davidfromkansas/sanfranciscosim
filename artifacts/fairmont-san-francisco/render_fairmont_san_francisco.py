"""Controlled review renders of the exported Fairmont San Francisco GLB.

    blender -b --python render_fairmont_san_francisco.py -- [--glb FILE]
                                                            [--out DIR]
                                                            [--prefix NAME]
...
(north = Blender +Y), which is how the asset is authored — so the west view
shows the Mason Street entrance front.
"""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "fairmont-san-francisco.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "fairmont-san-francisco")
    night = "--night" in argv
    ...
    if night:
        ...
        elev.data.ortho_scale = extent * 1.28
        elev.location = Vector((center.x - extent * 3.0, center.y, center.z))
        aim(elev, center)
    ...
    # From the south-west so the Mason front, the flags, the courtyard and the
    # tower pair all read (style bible §18: 30-50 deg down, long lens).
