"""Fresh-scene contract validation for fairmont-san-francisco.glb.

    blender -b --python validate_fairmont_san_francisco.py -- [--glb FILE]
                                                              [--out FILE]
...
def main():
    ...
    normals_ok = invalid_normal_count == 0 and ray_hits > 0 and ray_flipped == 0
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 95 <= dims.z <= 105
        and 100 <= dims.x <= 140
        and 80 <= dims.y <= 115,
        ...
        # Night glow must stay on the declared surfaces: the entrance fascia
        # and doors, the lit cornice line, the colonnade wash, the tower crown
        # and Crown Room, and the lit-room panes (all named glow_*).
        "glow_limited_to_declared_night_surfaces": all(
            n.startswith(("pc_fascia", "twr_crown_fascia", "glow_"))
