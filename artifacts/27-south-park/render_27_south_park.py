"""Controlled review renders of the exported 27 South Park GLB.

    blender -b --python render_27_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 27-south-park]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the South Park
Street front faces NW (314.8 deg), so the "north" and "west" elevations each
show it at 45 deg and neither shows it square. That is correct and expected:
the building stands at 45 deg to the world axes, so a face-on render of the one
public elevation is produced separately as <prefix>-front.png.
...
RES = (1500, 1000)   # landscape: the lot is 33.5 m long and only 10.2 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    ...
    Emission COLOR has to be copied from Base Color first. glTF writes
    emissiveFactor = 0 when the authored emission strength is 0, so a
    re-imported _Glow material carries a default WHITE emission and raising the
    strength alone renders every glow surface as a white slab (asset-plans
    README, "Night renders: drive _Glow from Base Color"). Caught here exactly as
    it was caught on chase-center."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "27-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "27-south-park")
    night = "--night" in argv
    aerial_only = "--aerial-only" in argv
    ...
    if night:
        ...
        az = math.radians(335)  # from the NNW: the lit ground floor is the identity
        r = span * 4.3
        aer.location = Vector(
            (
        ...
        aim(aer, Vector((center.x, center.y, center.z * 0.62)))
    ...
    # --- top view: the 4.4:1 stick, the bare deck and the cornice overhang ---
    if not aerial_only:
    ...
    # --- face-on view of the one public elevation ---------------------------
    # The compass set never shows it square (it stands at 45 deg), and it is the
    # only elevation the city sees.
    if not aerial_only:
        front = make_camera("cam_front")
        front.data.type = "ORTHO"
        front.data.ortho_scale = span * 1.18
        a = math.radians(314.8)
        front.location = Vector(
            (center.x + dist * math.sin(a), center.y + dist * math.cos(a), center.z)
        )
        aim(front, center)
        render_to(os.path.join(out, f"{prefix}-front.png"), front, RES)
    ...
    pitch = math.radians(25)  # low end of the app band: the lot is 33.5 m long
    # The front faces 314.8 deg, so 335 swings the camera 20 deg toward the
    # NORTH-EAST flank. Both flanks are blind party walls, so the swing is
    # chosen for the roof rather than for a facade: from here the 33.5 m of
    # membrane and its plant cluster run away from the arcade in one read.
    az = math.radians(335)
    r = span * 4.3
    aer.location = Vector(
        (
