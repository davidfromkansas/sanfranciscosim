...
#   3. limited dissolve — SKIPPED here (three coplanar ring bands; see below)
#   4. (skipped) curve retess — the only curves are 6-segment arch heads and
#      8/10-segment roof fans, already at the floor of what reads
...
# --- 3. limited dissolve — DELIBERATELY SKIPPED ---
# GLB-OPTIMIZE-PROMPT v2 s.3 step 3: skip on assets with large coplanar ring
# bands. This asset has THREE that follow the whole footprint - `base`,
# `parapet` and `coping` - and their top and bottom faces are perfectly
# coplanar annuli. Even a strictly-coplanar 0.05 deg dissolve merges each into
# one annulus ngon, and re-triangulating an annulus emits slivers up to the
# building's full 33.55 m length at ~0.2 mm width. Those pass an area-based
# degeneracy test, survive Phase E, and surface only after the shipping swap as
# `invalid_or_nonunit_loop_normal_count` in the stage-2 contract validator,
# because gltfpack re-emits the stored (collapsed) vertex normals while Blender
# recomputes them on import and hides the problem. Measured on 350-brannan,
# 13 Aug 2026, where the step was worth 0.4% of triangles and was reverted.
#
# Confirmed empirically here before removal: an unpatched run of this script on
# the same input reported tris=4968 before and after the dissolve, so the step
# was worth exactly zero triangles on this asset and only carried the risk.
stats["limited_dissolve"] = "skipped: three coplanar ring bands (base/parapet/coping); measured worth 0 triangles"
