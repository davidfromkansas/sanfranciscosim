"""Fresh-scene contract validation for 27-south-park.glb.

    blender -b --python validate_27_south_park.py -- [--glb FILE] [--out FILE]
...
TRI_BUDGET = 7000
TARGET_H = 10.20   # parapet coping crest, Overture per-ring USGS LiDAR (plan s.2.1)
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3931439, 37.7817369],
        "front_heading_deg_true": 314.8,
    ...
    results["checks"] = {
        # A 33.55 x 12.19 m bar standing at ~45 deg to the world axes projects
        # to a near-SQUARE axis-aligned bbox of about 32.6 x 32.5 m. That is
        # expected, not a sign the footprint got squared up - the elevations and
        # the top view are where the 2.75:1 stick is checked.
        "meters_and_plausible_dimensions": TARGET_H - 0.2 <= dims.z <= TARGET_H + 0.2
        and 31.5 <= dims.x <= 33.6
        and 31.5 <= dims.y <= 33.6,
        "crest_normalized_to_target": abs(mx.z - TARGET_H) <= 0.02,
