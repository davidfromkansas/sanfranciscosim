...
#   3. (SKIPPED for this asset) limited dissolve — see the block below
#   4. (skipped) curve retess — the vent cowl is the only curve and it is 0.3 m
...
# large coplanar ring bands". This asset has exactly the described case — the
# `upstand` object is a 4-loop band following all 16 edges of the footprint
# right around, so its top and bottom faces are perfectly coplanar annuli.
# Even a strictly-coplanar dissolve merges each into one annulus ngon, and
# re-triangulating an annulus emits slivers: on 350-brannan that produced
# triangles up to 24.35 m long and ~0.24 mm wide, which pass an area-based
# degeneracy test, survive Phase B and E, and only surface after the shipping
# swap as `invalid_or_nonunit_loop_normal_count` in the packed file.
#
# The upside forgone is small and the downside is a silent contract failure, so
# it is skipped rather than worked around. Recorded in REPORT.md s.3.
stats["limited_dissolve"] = "skipped: coplanar ring band (upstand) — prompt s.3 step 3"
