"""Controlled review renders of the exported 126 South Park GLB.

    blender -b --python render_126_south_park.py -- [--glb FILE] [--out DIR]
                                                     [--prefix 126-south-park]
                                                     [--night] [--aerial-only]
...
(north = Blender +Y), which is how the asset is authored — the South Park front
faces SE (135.3 deg).
...
--aerial-only skips the elevation rig, for fast design iteration.
"""
...
RES = (1500, 1000)   # landscape: 26.7 m of plan at a 45 deg heading, only 7.6 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
BG = (0.86, 0.80, 0.69, 1.0)  # neutral warm tabletop background

# From the south-east: square onto the South Park front, and the angle that lays
# the 29.8 m depth and its pinched waist across the frame.
AERIAL_AZ = 135.0
...
def setup_world(night=False):
    ...
    # CPU deliberately: the same Metal hang 135 South Park documented applies on
    # this machine — the headless `-b --factory-startup` process blocks at 0%
    # CPU before the first tile.
...
def add_lights(height, night=False):
    ...
    bpy.ops.mesh.primitive_plane_add(size=max(height, 8.0) * 8, location=(0, 0, -0.02))
...
def fade_glow():
    ...
    are ~12% alpha and the opaque surfaces behind them read through. Rendering
...
def place_aerial(cam, center, span, az_deg, pitch_deg, radius_k):
    ...
    aim(cam, Vector((center.x, center.y, center.z * 0.9)))
...
def main():
    ...
    out = arg("--out", here)
    prefix = arg("--prefix", "126-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        aer.data.lens = 80.0
        place_aerial(aer, center, span, AERIAL_AZ, 40.0, 3.4)
    ...
    aer.data.type = "PERSP"
    aer.data.lens = 80.0  # long lens, restrained perspective (style bible §18)
    place_aerial(aer, center, span, AERIAL_AZ, 42.0, 3.4)
    ...
    # --- top view: THE important one here — the waist, both light wells, the
    # --- eave, the upstand ring and the sparse roof -------------------------
    top = make_camera("cam_top")
    top.data.type = "ORTHO"
    top.data.ortho_scale = width * 1.10
