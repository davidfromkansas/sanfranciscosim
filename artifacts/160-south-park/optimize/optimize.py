...
# GLB-OPTIMIZE-PROMPT v2 §3.3: skip the dissolve entirely on assets with large
# coplanar ring bands. This one has three that follow the whole 26 m footprint —
# `parapet` (432 tris), `base_bulkhead` (360) and `string_course` (192). Their
...
# annulus emits slivers up to the length of the building with ~0.2 mm width.
# Those slivers pass every area-based degeneracy test and only surface later, in
# the packed file, as invalid_or_nonunit_loop_normal_count — exactly the
# 350-brannan failure of 13 Aug 2026. The step was worth 30 triangles there; the
# ring geometry here is a larger share of the asset, so the trade is worse.
stats["limited_dissolve"] = "skipped: three full-footprint coplanar ring bands"
