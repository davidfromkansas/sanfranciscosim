...
# --- 3. limited dissolve: SKIPPED for this asset (prompt s.3 step 3) ---
# 414 Brannan carries three large coplanar ring bands that follow the footprint
# all the way round -- the main parapet and the two roof copings -- plus two
# full-length frieze panels. Their top and bottom faces are perfectly coplanar
# annuli, so even a strictly-coplanar dissolve merges each into one annulus ngon
# whose re-triangulation emits ~0.2 mm slivers (measured on 350-brannan, 13 Aug
# 2026). Those slivers pass an area-based degeneracy test and only surface later
# as invalid_or_nonunit_loop_normal_count in the packed file. The step was worth
# 0.4% there; it is not worth manufacturing degenerate geometry for.
stats["limited_dissolve"] = "skipped: large coplanar ring bands (parapet, 2 copings, 2 friezes)"
