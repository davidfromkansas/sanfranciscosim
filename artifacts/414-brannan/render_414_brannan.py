"""Controlled review renders of the exported 414 Brannan Street GLB.

    blender -b --python render_414_brannan.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 414-brannan]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Brannan front faces SE (135.2 deg) and the Ritch front SW (225.2 deg).
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "414-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "414-brannan")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(180)  # from the south: square onto the Brannan/Ritch CORNER
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: the tile line, the three roof levels and the monitor ----
    ...
    az = math.radians(180)  # from the south — both street elevations at once
    r = span * 3.1
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
