...
# coplanar ring bands. This building has two of them stacked - `parapet` and
# `coping` both follow the whole 55 m footprint perimeter, 360 tris each, and
# their top and bottom faces are perfectly coplanar annuli. Even a strictly
# coplanar dissolve merges each into one annulus ngon, and re-triangulating an
# annulus emits the ~0.24 mm slivers that 350-brannan documented: invisible,
# area-test-clean, and fatal at the stage-2 contract validator because a
# sliver's shared vertex sits between opposing normals and its averaged vertex
# normal collapses to ~0, which gltfpack then re-emits.
#
# On 155 South Park this step removed exactly 0 triangles. It is the cheapest
# step in Phase B and the only one that can manufacture degenerate geometry, so
# on a two-ring asset it is not worth running at all.
stats["limited_dissolve"] = "skipped (two coplanar ring bands: parapet + coping)"
