"""Controlled review renders of the exported 150 South Park GLB.

    blender -b --python render_150_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 150-south-park]
                                                    [--night]
...
Street front faces SE (133.1 deg).
...
RES = (1400, 1000)   # the lot is 18.7 m deep and only 8 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "150-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "150-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(143)  # from the SE: the shopfront glow is the identity
        r = span * 4.3
        aer.location = Vector(
            (
    ...
    pitch = math.radians(30)  # mid app band; the mass is compact
    az = math.radians(143)  # from the SE, 10 deg off square so the NE flank reads too
    r = span * 4.3
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
