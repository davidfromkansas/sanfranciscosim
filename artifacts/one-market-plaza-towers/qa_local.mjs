// Stage-5 local QA for one-market-plaza-towers, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/one-market-plaza-towers/qa_local.mjs [--drill]
//
...
const LON = -122.3941803;
const LAT = 37.7933169;
const SLUG = 'one-market-plaza-towers';
const CAMERA = { distance: 700, yaw: 58, pitch: 20 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qatowers-'));
  ...
  const stats = await until(
    ...
    DRILL
      ? `(() => { const s = window.SF.assets.stats(); return s.failed > 0 ? s : null; })()`
      : `(() => (window.SF.assets.placed.has('oneMarketPlazaTowers') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasThis: window.SF.assets.placed.has('oneMarketPlazaTowers'),
    placeThis: window.SF.assets.placed.get('oneMarketPlazaTowers') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('oneMarketPlazaTowers').log, scale: window.SF.assets.placed.get('oneMarketPlazaTowers').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placedThis:', placed.hasThis, JSON.stringify(placed.placeThis));
  console.log('  placedIds:', placed.placedIds.join(','));
  console.log('  console lines:', console_.length);
  for (const l of console_) if (/one-market|oneMarket|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
  ...
  if (DRILL) {
    ...
    // `failed: 0` is meaningless unless the loader actually reached for the
    // file: under load the camera can still be at its default position, the
    // streamed entry correctly reports `far`, and a rig that measured nothing
    // reads exactly like "the asset is missing and nothing broke". Refuse the
    // result rather than fabricate a pass.
    check('drill actually exercised the loader (failed > 0)', s.failed > 0,
          `failed=${s.failed} live=${s.live} far=${s.far} — INCONCLUSIVE if 0`);
