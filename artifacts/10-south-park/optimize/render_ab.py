...
# ENGINE SWAP, recorded: the machine this repo is authored on runs many Blender
# sessions at once and at load ~200 a single 1200x1000 Cycles/64 frame takes over
# two minutes. EEVEE at 64 TAA samples renders the same eight frames in seconds
# with shadows, flat materials and the glow layer intact — and the G4 gate
# compares two renders of ONE rig against each other, so the engine only has to
# match on both sides. Blender 5.2's enum is BLENDER_EEVEE, not BLENDER_EEVEE_NEXT.
