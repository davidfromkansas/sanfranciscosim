...
#   3. limited dissolve — SKIPPED on this asset (see below)
#   4. (skipped) curve retess — the bowed frontage facets and the pond/tree rings
#      are silhouette-defining at the app's camera
...
# --- 3. limited dissolve — SKIPPED, deliberately ---
# GLB-OPTIMIZE-PROMPT v2 section 3 step 3: skip this entirely on assets with
# large coplanar ring bands. This asset has FOUR of them — front_parapet,
# front_cap, rear_parapet, rear_cap — each an annulus following its block's
# whole footprint. A strictly-coplanar dissolve merges each into one ngon and
# re-triangulating an annulus emits metre-long, sub-millimetre slivers that
# pass an area-based degeneracy test, survive to gltfpack, and then fail the
# stage-2 contract validator on invalid_or_nonunit_loop_normal_count. On
# 350-brannan the same step was worth 0.4% of triangles. Not worth it.
stats["limited_dissolve"] = "skipped: four coplanar parapet ring bands"
