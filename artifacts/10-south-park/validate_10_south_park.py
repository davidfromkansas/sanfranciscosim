"""Fresh-scene contract validation for 10-south-park.glb.

Two disjoint solids (a front block and a rear block) plus a courtyard ground
plane, so the per-object SIGNED VOLUME test is the authoritative normals check
and the 31,500-ray visibility test is the cross-check; its residual is allowed up
to 0.15% because rays graze the coincident faces where the loggia fills, the
parapet rims and the applied bands sit against the body.

    blender -b --python validate_10_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "10-south-park.glb"))
    ...
    for target in targets:
        for i in range(3500):
            ...
            direction = -outward
            hit, _, normal, _, hobj, _ = bpy.context.scene.ray_cast(
            ...
            if hit:
                ...
                if normal.dot(direction) > 1e-5:
                    ray_flipped += 1
                    nm = hobj.name if hobj else "?"
                    flipped_by_object[nm] = flipped_by_object.get(nm, 0) + 1
    ...
    results = {
        ...
        "normal_ray_cast_flipped_by_object": dict(
            sorted(flipped_by_object.items(), key=lambda kv: -kv[1])
        ),
        ...
        "anchor_lonlat": [-122.3935162, 37.7823704],
        "front_heading_deg_true": [179.7, 135.2],
        "long_axis_heading_deg_true": 315.2,
        "target_height_m": 14.67,
    ...
    results["checks"] = {
        # ~39.9 x 36.0 x 14.67 m: the XY box is the axis-aligned bound of a
        # TRAPEZOID standing at 45 deg (party walls 42.34 and 36.28 m, Taber rear
        # 14.29 m, frontage 8.13 m of straight plus an 8.64 m arc), not a 40 m
        # building. See REPORT.md section 3.
        "meters_and_plausible_dimensions": (
            14.60 <= dims.z <= 14.75
            and 39.5 <= dims.x <= 40.4
            and 35.6 <= dims.y <= 36.4
        ),
        "crest_normalized_to_target": abs(mx.z - 14.67) <= 0.02,
