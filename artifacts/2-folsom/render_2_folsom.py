"""Controlled review renders of the exported 2 Folsom Street GLB.

    blender -b --python render_2_folsom.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 2-folsom]
                                                    [--night]
...
elevation faces NE (45.2 deg) and Folsom Street faces SE (135.2 deg).
...
RES = (1500, 1250)   # the 45 deg heading makes the AABB 112 m wide at 88 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    if night:
        ...
        az = math.radians(45.0)   # from the northeast: the harbour elevation
        #   (45.2 deg) with Folsom (135.2 deg) raking away, and the limestone
        #   tower standing over the atrium skylight
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    az = math.radians(45.0)   # from the northeast — square onto the Embarcadero
    #   elevation (45.2 deg), which is the one the whole composition is aimed
    #   at, with the Folsom front raking away to the right
    r = span * 3.4
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
