"""Fresh-scene contract validation for 2-folsom.glb.

    blender -b --python validate_2_folsom.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "2-folsom.glb"))
    ...
    results = {
        ...
        "anchor_lonlat": [-122.390975, 37.790787],
        "front_heading_deg_true": 45.2,
        "target_height_m": 88.0,
    ...
    results["checks"] = {
        # The 45.2 deg heading turns an 84.31 x 77.14 m building into a ~114 m
        # axis-aligned bounding box. That is expected, not a scale error.
        "meters_and_plausible_dimensions": 87.8 <= dims.z <= 88.2
        and 112.0 <= dims.x <= 116.0
        and 112.0 <= dims.y <= 116.0,
        "crest_normalized_to_target": abs(mx.z - 88.0) <= 0.02,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        # The AABB centre sits 2.04 m from the origin because the FOOTPRINT is
        # genuinely asymmetric: the two Embarcadero-side corners are square and
        # the two Spear-side corners step in 4.7 m. The ORIGIN is what matters
        # — placeGeneric() composes the placement matrix at the anchor and
        # applies no recentring — and the origin is the surveyed OBB centre.
        "centered_xy": abs(center.x) <= 2.5 and abs(center.y) <= 2.5,
