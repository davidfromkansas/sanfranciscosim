"""Controlled review renders of the exported Grace Cathedral GLB.

    blender -b --python render_grace_cathedral.py -- [--glb FILE] [--out DIR]
                                                     [--prefix grace-cathedral]
...
(north = Blender +Y, how the asset is authored). The nave axis actually bears
81 deg, so the east/west views face the entrance front and apse a few degrees
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "grace-cathedral.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "grace-cathedral")
    ...
    top.rotation_euler = (0, 0, math.radians(-81.03) + math.pi / 2)
    ...
    az = math.radians(112)             # from the ESE: entrance front + south flank
    r = span * 3.15
    aer.location = Vector(
        (
        ...
        if mat.name.endswith("_Glow") and mat.use_nodes:
            ...
            if bsdf:
                ...
                bsdf.inputs["Emission Strength"].default_value = {
                    "Toy_gold_Glow": 5.5,       # portal + fleche lantern beacons
                    "Toy_white_Glow": 3.2,      # the floodlit rose
                }.get(mat.name, 2.4)            # lit window panes
    render_to(os.path.join(out, f"{prefix}-night.png"), aer, AER_RES)

    # night entrance closeup: the lit rose + portal from the steps
