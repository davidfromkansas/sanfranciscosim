"""Fresh-scene contract validation for grace-cathedral.glb.

    blender -b --python validate_grace_cathedral.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "heading_deg_cw_from_north": 81.03,
        ...
        "normal_orientation_method": (
            "Every source mesh runs bmesh.ops.recalc_face_normals before export; "
            "reimported loop normals must be finite/unit; 22,500 deterministic "
            "visibility rays check that the first face each ray meets faces back "
            "along the ray."
        ),
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 100 <= dims.x <= 118 and 40 <= dims.y <= 56 and 74 <= dims.z <= 77,
