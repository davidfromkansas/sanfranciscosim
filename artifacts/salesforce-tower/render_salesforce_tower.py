"""Controlled review renders of the exported Salesforce Tower GLB.

    blender -b --python render_salesforce_tower.py -- [--glb FILE] [--out DIR]
                                                      [--prefix salesforce-tower]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "salesforce-tower.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "salesforce-tower")
    ...
    # --- top view: the roof and crown design -------------------------------
    top = make_camera("cam_top")
    top.data.type = "ORTHO"
    top.data.ortho_scale = width * 1.6
    ...
    az = math.radians(215)  # looking at the Mission Street corner
    r = height * 4.2
    aer.location = Vector(
        (
