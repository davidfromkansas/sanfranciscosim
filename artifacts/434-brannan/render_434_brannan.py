"""Controlled review renders of the exported 434 Brannan Street GLB.

    blender -b --python render_434_brannan.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 434-brannan]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Brannan Street front faces SE (134.8 deg).

--only aerial renders just the beauty aerial, and --samples lowers the sample
count: the pipeline requires reviewing the high three-quarter aerial FIRST and
iterating there, so the cheap preview pass must not cost six full-quality
images. The formal rig then runs at the defaults.
...
RES = (1500, 1000)   # landscape: this building is wide and low
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def setup_world(night=False, samples=64, engine="EEVEE"):
    """EEVEE by default, not Cycles. David runs many landmark sessions at once
    and the Mac sits at load 100-700; a 1400x1200 CPU-Cycles frame can take
    minutes or never finish, while the same rig on EEVEE renders in seconds with
    shadows, flat materials and the glow layer intact. Nothing gate 2/3 judges —
    silhouette, massing, the toothed roofline, which surfaces glow — needs path
    tracing. In Blender 5.2 the enum is BLENDER_EEVEE, not BLENDER_EEVEE_NEXT.
    Pass --engine cycles to force path tracing on an idle machine."""
...
def light_glow(strength=2.4):
    """Preview the app's night pass: emission up on every _Glow material.

    380 Brannan used 6.0; 2.4 is enough to read and spills less onto the
    ground plane. Note what this image can and cannot tell you: any strength
    above ~1.0 clips these colours to white under the Standard view transform,
    so the render judges WHICH surfaces glow and how restrained the scatter is
    — not the night palette. The app draws _Glow as an unlit overlay at the
    material's own baked colour (Toy_glassl blue windows, Toy_trim warm-white
    portals), so read the palette off PALETTE_HEX, not off this render.
    """
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "434-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "434-brannan")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    setup_world(night=night, samples=samples, engine=arg("--engine", "eevee"))
    ...
    if night:
        ...
        az = math.radians(170)  # from the S: catches BOTH finished street faces
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    span = max(width, height)
    if only == "aerial":
        render_aerial(center, span, out, prefix)
        return
    ...
    # --- top view: parapet ring, pilaster caps as teeth, penthouse, duct, skylight ----
...
def render_aerial(center, span, out, prefix):
    """The app's high three-quarter aerial — the view the style bible says to
    judge from, and the one the pipeline says to iterate on before the rig."""
    aer = make_camera("cam_aerial")
    aer.data.type = "PERSP"
    aer.data.lens = 85.0  # long lens, restrained perspective (style bible §18)
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(170)  # from the S — the Brannan front AND the Zoe Street
    # flank in one frame, which is what the app's camera gets. This building's
    # two finished faces meet at its SOUTH corner (Brannan x Zoe), so the hero
    # aerial comes from the south; anything east of ~140 deg puts the blind
    # northeast party flank in shot.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
