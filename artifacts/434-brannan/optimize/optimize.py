...
#   3. limited dissolve — DISABLED on this asset, see the block below
#   4. (skipped) curve retess — the only curve here is the 14-segment entry
#      disc, already at the miniature style's low-seg floor
...
# Per-asset switches so the four-variant weld/join measurement is one command
# each rather than five edits (GLB-OPTIMIZE-PROMPT s.11: measure, do not assume).
NO_WELD = "--no-weld" in argv
NO_JOIN = "--no-join" in argv
...
# The weld is NOT unconditionally good on a flat-shaded, box-heavy asset: the
# "coincident vertex pairs" a census counts there are the flat-shading topology,
# not waste, and welding them makes the exporter re-split into a worse layout
# (measured -50 KB the wrong way on 326-brannan). It DOES help on beveled solids,
# where the bevel leaves genuinely redundant verts (+27 KB on 300-brannan). This
# asset is both — beveled chunky solids plus ~120 flat applied panels — so the
# four-variant table in REPORT.md decides it rather than a rule of thumb.
for o in mesh_objs():
    bm = bmesh.new()
    bm.from_mesh(o.data)
    if not NO_WELD:
    ...
    bm.to_mesh(o.data)
    bm.free()
snap("no-weld+degenerate" if NO_WELD else "weld+degenerate")
...
#
# GLB-OPTIMIZE-PROMPT s.3 step 3 says to skip this entirely on assets with large
# coplanar ring bands, and 434 Brannan has two of them: the parapet ring_band and
# the coping ring_band, each following the whole 112 m footprint perimeter. Their
# top and bottom faces are perfectly coplanar annuli, so even a strictly-coplanar
# dissolve merges each ring into one annulus ngon, and re-triangulating an
# annulus emits hairline slivers up to the building's full length. Those pass an
# area-based degeneracy test, survive Phase B and Phase E, and then fail the
# stage-2 contract validator AFTER the shipping swap, because gltfpack re-emits
# the stored (near-zero) vertex normals that Blender was silently recomputing.
# First measured on 350 Brannan, repeated on 340 Brannan across the alley.
# It is worth ~0.4% of triangles. Not worth it.
stats["limited_dissolve"] = "skipped (coplanar parapet/coping ring bands)"
snap("limited-dissolve-skipped")

# --- 5. join per material (multi-material objects keep their own mesh) ---
groups = defaultdict(list)
for o in ([] if NO_JOIN else mesh_objs()):
