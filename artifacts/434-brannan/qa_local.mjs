// Stage-5 local QA for 434-brannan, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
const LON = -122.3954103;
const LAT = 37.7796003;
const SLUG = '434-brannan';
const CAMERA = { distance: 240, yaw: 45, pitch: 26 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa434-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('434Brannan') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    has434: window.SF.assets.placed.has('434Brannan'),
    place434: window.SF.assets.placed.get('434Brannan') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('434Brannan').log, scale: window.SF.assets.placed.get('434Brannan').scale })) : null,
  }))()`);
