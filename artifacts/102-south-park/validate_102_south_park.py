"""Fresh-scene contract validation for 102-south-park.glb.
...
def main():
    ...
    results["checks"] = {
        # The building is 7.78 x 29.76 m at a 135.4 deg heading, so its
        # axis-aligned box is ~27.3 x 27.1 m. That is the expected consequence of
        # the real-world orientation, not a scale error.
        "meters_and_plausible_dimensions": 13.8 <= dims.z <= 14.2
        and 26.0 <= dims.x <= 28.5
        and 26.0 <= dims.y <= 28.5,
        "crest_normalized_to_target": abs(mx.z - 14.0) <= 0.02,
