...
# Phase B's 1 mm weld is NOT unconditionally a win: on a flat-shaded box asset
# the "coincident vertex pairs" the census counts are the flat-shading topology,
# and welding them makes the exporter re-split worse (326-brannan: +50 KB raw).
# On a BEVELLED asset it wins (300-brannan: -27 KB). This asset is bevelled, so
# the answer is not obvious either way — run the four-variant table and decide
# from bytes. --no-weld / --no-dissolve make that one command each.
...
stats["weld_skipped"] = NO_WELD
stats["dissolve_skipped"] = NO_DISSOLVE
snap("interior-faces")

# --- 3. limited dissolve, coplanar only ---
# SKIPPED by default on this asset (--no-dissolve is the default here): it has
# THREE large coplanar ring bands following the footprint — the parapet, the
# dark coping and the roof-deck curb. GLB-OPTIMIZE-PROMPT s.3.3 says to skip
# the step on exactly that geometry, because re-triangulating a merged annulus
# ngon emits slivers whose averaged vertex normal collapses to ~0, and gltfpack
# re-emits the STORED normals so the failure only shows up in the packed file.
