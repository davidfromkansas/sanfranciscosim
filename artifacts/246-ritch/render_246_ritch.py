"""Controlled review renders of the exported 246 Ritch Street GLB.

    blender -b --python render_246_ritch.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 246-ritch]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Ritch Street front faces NE (45.0 deg).
...
RES = (1200, 1200)   # square: this building is 16.7 m wide and 18.8 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def setup_world(night=False, samples=64, workbench=False):
    scene = bpy.context.scene
    if workbench:
        # Cheap iteration pass. This Mac runs several asset sessions at once and
        # Cycles at load 200+ takes minutes per frame; Workbench is seconds.
        # It reads material.diffuse_color (which build_246_ritch.py sets), NOT
        # the node Base Color, and it has no shadows worth judging — use it for
        # composition and proportion, never for the formal rig or the palette.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "246-ritch.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "246-ritch")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    setup_world(night=night, samples=samples, workbench=workbench)
    ...
    if night:
        ...
        az = math.radians(45)  # from the NE: the Ritch Street front and the roof
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, penthouse, lightwell, mechanical cluster ------
...
def render_aerial(center, span, out, prefix):
    ...
    az = math.radians(45)  # from the NORTH-EAST — the Ritch Street front, its
    # balcony grid and the roof in one frame, which is what the app's camera
    # gets. This building has exactly one designed elevation and it faces 45 deg;
    # anything more than ~40 deg either side of that puts a party wall in shot.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
