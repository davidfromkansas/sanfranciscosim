"""Fresh-scene contract validation for 550-third.glb.

    blender -b --python validate_550_third.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(11.0 / dims.z, 6),
        ...
        "anchor_lonlat": [-122.3953409, 37.7804407],
        "long_axis_heading_deg_true": 45.3,
        "front_normal_heading_deg_true": 44.6,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        "meters_and_plausible_dimensions": 10.9 <= dims.z <= 11.1
        and 48.0 <= dims.x <= 50.5
        and 49.0 <= dims.y <= 51.5,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 11.0) <= 0.01,
