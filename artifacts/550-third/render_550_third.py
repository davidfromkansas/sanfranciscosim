"""Controlled review renders of the exported 550 Third Street GLB.

    blender -b --python render_550_third.py -- [--glb FILE] [--out DIR]
                                               [--prefix 550-third] [--night]
...
This building has no skyline silhouette: the top and aerial views are the ones
that matter, and both are framed on the roof composition.
...
# A true three-quarter: far enough round from the front normal (44.6 deg) to
# show the 3rd Street elevation and the SE party wall's property-line windows
# in the same frame (style bible s.18).
AERIAL_AZ = 80.0
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "550-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "550-third")
    night = "--night" in argv
    only = arg("--only", "")
    ...
    if only in ("", "elev"):
        ...
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), mn.z + 5.5))
            aim(cam, Vector((center.x, center.y, mn.z + 5.5)))
