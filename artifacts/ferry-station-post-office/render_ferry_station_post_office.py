"""Controlled review renders of the exported Ferry Station Post Office Building GLB.

    blender -b --python render_ferry_station_post_office.py -- [--glb FILE] [--out DIR]
                                               [--prefix ferry-station-post-office] [--night]
...
(north = Blender +Y), which is how the asset is authored — the Second Street
front faces north-east.

The subject of this building is its clay-tile hip roof and its Embarcadero
front, so the aerial camera looks straight down the frontage's outward normal
(234 deg) and gets the front elevation, the tiled hip and the stepped decks
behind it in one frame. The top view exists to check the roof composition: the
tile band and its hipped ends, the two flat decks, the light-well slot, the
three roof monitors and the SE wing.
...
# The building sits at 54 deg to the compass, so cardinal cameras would each
# show two faces at once. These are TRUE elevations: one camera per measured
# face normal, named for the direction that face looks.
VIEWS = [
    ("southwest", 234.0),  # the Embarcadero principal elevation — the subject
    ("northwest", 324.3),  # the flank facing Harry Bridges Plaza
    ("southeast", 144.3),  # the flank facing the ferry slips + the 1918 wing
    ("northeast", 54.0),   # the work-room rear, over the water
]
AERIAL_AZ = 250.0  # off the frontage normal, so the SE wing reads as well as the front
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "ferry-station-post-office.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "ferry-station-post-office")
    night = "--night" in argv
    global FAST, GPU
    FAST = "--fast" in argv
    GPU = "--gpu" in argv
    ...
    if night:
        ...
        aim(aer, Vector((center.x, center.y, mn.z + 4.0)))
    ...
    if only in ("", "elev"):
        ...
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), mn.z + 6.5))
            aim(cam, Vector((center.x, center.y, mn.z + 6.5)))
