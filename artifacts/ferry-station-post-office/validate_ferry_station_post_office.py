"""Fresh-scene contract validation for ferry-station-post-office.glb.

    blender -b --python validate_ferry_station_post_office.py -- [--glb FILE] [--out FILE]
...
Normals are checked two ways, because this asset is a union of ~285 closed
...
# The manifest target height: the clay-tile hip ridge, which must be the export
# bounding-box top so the loader's targetHeightM / measuredHeight is 1.0.
TARGET_HEIGHT_M = 12.65
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3921505, 37.7941368],
        "long_axis_heading_deg_true": 144.0,
        "front_normal_heading_deg_true": 234.0,
    ...
    results["checks"] = {
        ...
        "meters_and_plausible_dimensions": 12.6 <= dims.z <= 12.7
        and 68.5 <= dims.x <= 71.0
        and 64.0 <= dims.y <= 66.5,
