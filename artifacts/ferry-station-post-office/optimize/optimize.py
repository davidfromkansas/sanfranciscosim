...
# Per-asset adaptation (GLB-OPTIMIZE-PROMPT §3 and §11):
#   --no-weld     Phase B step 1 is measured, never assumed. This asset runs a
#                 2-segment Bevel over every chunky solid, which is the case
#                 where the weld wins — but the four-variant table in REPORT.md
#                 is what decides it, not this comment.
#   --no-dissolve Step 3 is OFF BY DEFAULT here. This asset carries seven large
#                 coplanar ring bands (granite plinth, work-room parapet and
#                 coping, the copper cornice, the mid-block parapet and coping,
#                 the SE wing cornice). A strictly-coplanar dissolve merges each
#                 into one annulus ngon and re-triangulates it into invisible
#                 slivers that only surface as invalid loop normals in the
#                 PACKED file, two steps later. --dissolve re-enables it.
NO_WELD = "--no-weld" in argv
DO_DISSOLVE = "--dissolve" in argv
...
for o in mesh_objs():
    ...
    bm.to_mesh(o.data)
    bm.free()
snap("degenerate" if NO_WELD else "weld+degenerate")
