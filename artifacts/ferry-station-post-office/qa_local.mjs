// Stage-5 local QA for ferry-station-post-office, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/ferry-station-post-office/qa_local.mjs [--drill]
//
...
const LON = -122.3921505;
const LAT = 37.7941368;
const SLUG = 'ferry-station-post-office';
const CAMERA = { distance: 260, yaw: 290, pitch: 22 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qafspo-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      // file: at high load the camera may never leave its default position and
      // an untouched entry reports far/live/failed exactly like a healthy miss.
      // Refuse to return until failed > 0.
      ? `(() => { const s = window.SF.assets.stats(); return s.failed > 0 ? s : null; })()`
      : `(() => (window.SF.assets.placed.has('ferryStationPostOffice') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasLm: window.SF.assets.placed.has('ferryStationPostOffice'),
    placeLm: window.SF.assets.placed.get('ferryStationPostOffice') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('ferryStationPostOffice').log, scale: window.SF.assets.placed.get('ferryStationPostOffice').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placed434:', placed.hasLm, JSON.stringify(placed.placeLm));
  console.log('  placedIds:', placed.placedIds.join(','));
  console.log('  console lines:', console_.length);
  for (const l of console_) if (/ferry-station-post-office|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
  ...
  await evaluate(`window.SF.setClock('2026-08-18T14:30:00-07:00')`);
  await new Promise((r) => setTimeout(r, 2500));
  await shot(DRILL ? 'drill-day' : 'day');

  // night: sweep past dusk
  await evaluate(`window.SF.setClock('2026-08-18T21:45:00-07:00')`);
  await new Promise((r) => setTimeout(r, 2500));
