"""Fresh-scene contract validation for 500-third.glb.

    blender -b --python validate_500_third.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(26.5 / dims.z, 6),
        ...
        "anchor_lonlat": [-122.3958224, 37.7808279],
        "long_axis_heading_deg_true": 134.9,
        "front_normal_heading_deg_true": 44.9,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        "meters_and_plausible_dimensions": 26.4 <= dims.z <= 26.6
        and 74.5 <= dims.x <= 76.0
        and 75.5 <= dims.y <= 77.0,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 26.5) <= 0.01,
