"""Controlled review renders of the exported 500 Third Street GLB.

    blender -b --python render_500_third.py -- [--glb FILE] [--out DIR]
                                               [--prefix 500-third] [--night]
...
This building has no skyline silhouette either, but unlike 550 Third its
identity is on its walls: the aerial three-quarter on the north corner is the
hero, and the top view exists to check the roof composition.
...
VIEWS = [
    ("north", 44.9),  # 3rd Street front (NE)
    ("east", 134.3),  # SE elevation over the parking lot
    ("south", 225.0),  # Ritch Street service rear (SW)
    ("west", 314.0),  # Bryant Street front (NW)
]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "500-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "500-third")
    night = "--night" in argv
    global FAST, GPU
    FAST = "--fast" in argv
    GPU = "--gpu" in argv
