"""Controlled review renders of the exported 521 Third Street GLB.

    blender -b --python render_521_third.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 521-third]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the 3rd Street front faces SW (225.1 deg) and the Taber Place flank NW (315.1 deg).
...
RES = (1300, 1100)   # near-square: a 45 deg heading puts both long faces in frame
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    ...
    Emission Color must be COPIED FROM Base Color, not left as imported. glTF
    writes `emissiveFactor = 0` whenever the authored emission strength is 0
    (which is how the shipping asset is authored), so a re-imported _Glow
    material carries Blender's default WHITE emission and every glow surface
    renders as a white slab. That is exactly what the app does not do: its
    night layer is an unlit overlay drawn at the material's own baked colour.
    See docs/asset-plans/README.md, "Night renders: drive _Glow from Base
    Color"; tools/glb-optimize/render_ab.py already does it this way.
    """
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "521-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "521-third")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(270)  # from the WEST: the hero corner, both designed fronts at once
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, five skylights, two hatches, the vent ------
    ...
    aer.data.type = "PERSP"
    aer.data.lens = 72.0  # long lens, restrained perspective (style bible §18).
    # 105 mm at span*3.1 crops this asset: a 32 m plan diagonal against an 8.2 m
    # height makes `span` the plan, not the elevation, so a tighter rig runs off
    # the sides of frame here.
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(270)  # from the WEST — straight at the hero corner, both
    # designed elevations in one frame. This is a CORNER building; a view square
    # onto either street throws the other away. 270 is the bisector of the 3rd
    # Street normal (225.1) and the Taber Place normal (315.1), and it is the
    # same angle the manifest camera preset uses.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
