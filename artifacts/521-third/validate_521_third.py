"""Fresh-scene contract validation for 521-third.glb.

    blender -b --python validate_521_third.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3952384, 37.7811509],
        "front_heading_deg_true": 225.1,
        "target_height_m": 11.4,
    ...
    results["checks"] = {
        # A 14.64 x 23.10 m footprint at a 45 deg heading gives a ~26.7 x 26.7 m
        # axis-aligned box, rising to ~27.3 with the cornice overhang. The
        # near-square XY is expected, not a scale error.
        "meters_and_plausible_dimensions": 11.2 <= dims.z <= 11.6
        and 26.5 <= dims.x <= 28.0
        and 26.5 <= dims.y <= 28.0,
        "crest_normalized_to_target": abs(mx.z - 11.4) <= 0.02,
