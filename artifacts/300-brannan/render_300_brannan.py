"""Controlled review renders of the exported 300 Brannan Street GLB.

    blender -b --python render_300_brannan.py -- [--glb FILE] [--out DIR]
                                               [--prefix 300-brannan] [--night]
...
The subject of this building is its canted east corner, so the aerial camera
looks straight down the cant's outward normal and gets both frontages and the
corner in one frame; the "east" elevation is the cant seen square on. The top
...
VIEWS = [
    ("north", 45.2),   # Second Street front (NE)
    ("east", 95.1),    # the canted corner — this building's subject
    ("south", 135.5),  # Brannan Street front (SE)
    ("west", 225.5),   # Stanford Street alley flank (SW)
    ("party", 315.1),  # the north-west lot-line wall
]
...
AERIAL_AZ = 95.1  # straight down the cant normal: both frontages plus the corner
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "300-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "300-brannan")
    night = "--night" in argv
    global FAST, GPU
    FAST = "--fast" in argv
    GPU = "--gpu" in argv
