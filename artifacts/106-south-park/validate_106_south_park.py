"""Fresh-scene contract validation for 106-south-park.glb.

    blender -b --python validate_106_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "106-south-park.glb"))
    ...
    results["checks"] = {
        # 26.4 x 26.4 x 11.58 m: the XY box is the exact 45 deg rotation of a
        # 7.32 x 29.72 m sliver, not a 26 m building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            11.5 <= dims.z <= 11.65
            and 26.0 <= dims.x <= 26.8
            and 26.0 <= dims.y <= 26.8
        ),
        "crest_normalized_to_target": abs(mx.z - 11.58) <= 0.02,
