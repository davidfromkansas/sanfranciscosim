...
# GLB-OPTIMIZE-PROMPT v2 s3 step 3 says to skip this entirely on assets with
# large coplanar ring bands. 156 South Park has three: shed_parapet, bar_parapet
# and bar_cap, each a band following the whole footprint, so their top and bottom
# faces are perfectly coplanar annuli. Even a strictly-coplanar dissolve merges
# each into one annulus ngon, and re-triangulating an annulus emits slivers up to
# the length of the building - which pass the area-based degeneracy test, survive
# to the packed file, and only surface as invalid_or_nonunit_loop_normal_count in
# the stage-2 contract validator AFTER the shipping swap. That is exactly what
# 350-brannan hit on 13 Aug 2026, where the step was worth 0.4% and was reverted.
# Same call here.
stats["limited_dissolve"] = "skipped: coplanar ring bands (parapets/cap)"
