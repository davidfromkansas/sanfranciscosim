"""Fresh-scene contract validation for 156-south-park.glb.

    blender -b --python validate_156_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3948748, 37.7813535],
        "front_heading_deg_true": 117.3,
        "centering_note": (
            "The origin is the footprint AREA centroid, which is the point the "
            "manifest anchor names and where the loader must put the building. "
            "On a lot that tapers from 5.9 m at the street to 9.8 m two thirds "
            "back, that centroid is ~1.6 m from the axis-aligned bbox centre. "
            "The tolerance below is set accordingly; it is not a placement error."
        ),
        "target_height_m": 8.7,
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 8.6 <= dims.z <= 8.8
        and 28.5 <= dims.x <= 31.5
        and 22.5 <= dims.y <= 25.0,
        "crest_normalized_to_target": abs(mx.z - 8.7) <= 0.02,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "centered_xy": abs(center.x) <= 1.7 and abs(center.y) <= 1.7,
