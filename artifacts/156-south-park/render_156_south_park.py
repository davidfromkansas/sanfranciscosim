"""Controlled review renders of the exported 156 South Park Street GLB.

    blender -b --python render_156_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 156-south-park]
                                                    [--night]
...
Street front faces ESE (117.3 deg).
...
RES = (1500, 1000)   # landscape: the lot is 32 m long and only 8.7 m tall
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "156-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "156-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(127)  # from the ESE: the two lit sash fields are the identity
        r = span * 4.3
        aer.location = Vector(
            (
    ...
    # --- top view: front parapet ring, the step down, rear deck and screen ---
    if not aerial_only:
    ...
    pitch = math.radians(27)  # the shed roof and its monitor run must read
    az = math.radians(127)  # from the ESE, 10 deg off square so a flank reads too
    r = span * 4.3
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
