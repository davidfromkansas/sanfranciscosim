"""Fresh-scene contract validation for 592-third.glb.

    blender -b --python validate_592_third.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3946805, 37.7800910],
    ...
    results["checks"] = {
        # A 21.67 x 23.07 m footprint at a 45 deg heading gives a ~31.2 x 32.2 m
        # axis-aligned box. The near-square XY is expected, not a scale error.
        "meters_and_plausible_dimensions": 8.0 <= dims.z <= 8.4
        and 30.0 <= dims.x <= 32.5
        and 31.0 <= dims.y <= 33.5,
        "crest_normalized_to_target": abs(mx.z - 8.2) <= 0.02,
