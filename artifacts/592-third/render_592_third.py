"""Controlled review renders of the exported 592 Third Street GLB.

    blender -b --python render_370_brannan.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 592-third]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the 3rd Street front faces NE (45.1 deg) and the Brannan front SE (135.2 deg).
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "592-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "592-third")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(90)  # from the east: the corner, both street fronts at once
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    az = math.radians(90)  # from the east — straight at the hero corner, both
    # designed elevations in one frame. This is a CORNER building; a view square
    # onto either street throws the other away.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
