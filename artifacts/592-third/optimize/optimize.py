...
#   3. (SKIPPED) limited dissolve — see the block below; this asset has a
#      footprint-following parapet ring, the exact case GLB-OPTIMIZE-PROMPT §3.3
#      says to skip
...
# coplanar ring bands. This building has one — `parapet`, a 0.30 m band
# following all four footprint edges from 7.82 to 8.20 m. Its top and bottom
# faces are perfectly coplanar annuli, so even a strictly-coplanar 0.05 deg
# dissolve merges each into a single ngon, and re-triangulating an annulus emits
# slivers tens of metres long and a fraction of a millimetre wide. Those pass an
# area-based degeneracy test and survive Phase B and E, then fail the stage-2
# contract validator AFTER the shipping swap, as
# `invalid_or_nonunit_loop_normal_count` in the packed file (measured on
# 350-brannan, 13 Aug 2026). At 3,352 triangles the dissolve would have been
# worth a fraction of a percent; it is not worth manufacturing that failure.
stats["limited_dissolve"] = "skipped: parapet ring band (prompt 3.3)"
