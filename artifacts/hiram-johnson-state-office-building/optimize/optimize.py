...
# GLB-OPTIMIZE-PROMPT v2 s.3 step 3: skip entirely on assets with large coplanar
# ring bands. This one is made of them - the parapet ring, the granite base, the
# sill/cap courses and the storey spandrels all follow the footprint the whole
# way round, so even a strictly-coplanar dissolve merges each into one annulus
# ngon and re-triangulating an annulus emits slivers. The build script already
# removes collinear runs from every generated ring (dedupe_ring), which is where
# this asset's dissolve savings would have come from, and the stage-2 validator
# fails on exactly the sliver signature this step manufactures
# (invalid_or_nonunit_loop_normal_count - it cost this asset one build cycle).
stats["limited_dissolve"] = "skipped (coplanar ring bands, prompt s.3 step 3)"
