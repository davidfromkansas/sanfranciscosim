// Stage-5 local QA for hiram-johnson-state-office-building, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/hiram-johnson-state-office-building/qa_local.mjs [--drill]
//
...
const LON = -122.4179151;
const LAT = 37.7810345;
const SLUG = 'hiram-johnson-state-office-building';
const CAMERA = { distance: 470, yaw: 150, pitch: 22 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qahiram-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('hiramJohnsonStateOfficeBuilding') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    hasAsset: window.SF.assets.placed.has('hiramJohnsonStateOfficeBuilding'),
    placeAsset: window.SF.assets.placed.get('hiramJohnsonStateOfficeBuilding') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('hiramJohnsonStateOfficeBuilding').log, scale: window.SF.assets.placed.get('hiramJohnsonStateOfficeBuilding').scale })) : null,
  }))()`);
  ...
  for (const l of console_) if (/hiram|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
