...
SKIP_DISSOLVE = "--skip-dissolve" in argv
...
interior_removed = 0
for o in mesh_objs():
    # _Glow shells are thin plates that intentionally hug solids; deleting
    # their "buried" backs turns them into open shells with negative joined
    # signed volume (caught by the stage-2 validator on this asset).
    if any(m and m.name.endswith("_Glow") for m in o.data.materials):
        continue
    bm = bmesh.new()
    bm.from_mesh(o.data)
    doomed = []
