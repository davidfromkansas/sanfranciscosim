// Stage-5 local QA for 345-spear, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/345-spear/qa_local.mjs [--drill]
//
...
const LON = -122.3901941;
const LAT = 37.7900769;
const SLUG = '345-spear';
const CAMERA = { distance: 450, yaw: 115, pitch: 26 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa345-'));
  const chrome = spawn(findChrome(), [
    ...
    // GPU headless: SwiftShader under sibling-session load takes minutes to
    // boot the diorama and reads as a broken asset (sf3d QA lesson, 17 Aug).
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('345Spear') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    has345: window.SF.assets.placed.has('345Spear'),
    place345: window.SF.assets.placed.get('345Spear') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('345Spear').log, scale: window.SF.assets.placed.get('345Spear').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placed345:', placed.has345, JSON.stringify(placed.place345));
  console.log('  placedIds:', placed.placedIds.join(','));
  console.log('  console lines:', console_.length);
  for (const l of console_) if (/345|warn|fail|error|keeping/i.test(l)) console.log('    >', l.slice(0, 200));
