"""Controlled review renders of the exported 501 Second Street GLB.

    blender -b --python render_345_spear.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 345-spear]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — Spear faces SW, the Embarcadero faces NE.
...
RES = (1500, 1100)   # the 45 deg heading makes every elevation ~23 m wide
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def light_glow():
    """Preview the app's night pass. The GLB round-trip writes emissiveFactor=0
    (authored strength is 0), so a re-imported _Glow material carries a DEFAULT
    WHITE emission — raising only the strength renders every glow white (the
    chase-center lesson, README "Night renders"). Copy Base Color into Emission
    Color, exactly what the app's unlit night layer does."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "345-spear.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "345-spear")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(90.0)  # from the bay: the Embarcadero arcade, terraces and tower
        #   (Second St at 225.4 deg and Bryant St at 315.4 deg)
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, cornice overhang, penthouse, light court and roof plant ----
    ...
    az = math.radians(90.0)  # from the bay — the identity view: arcade, terraces, tower
    #   (225.4 deg) and Bryant Street (315.4 deg) elevations, so the corner and
    #   both public fronts are in one frame
    r = span * 3.4
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
