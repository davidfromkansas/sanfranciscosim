"""Fresh-scene contract validation for 345-spear.glb.

    blender -b --python validate_345_spear.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3901862, 37.7900769],
        "front_heading_deg_true": 225.0,
        "target_height_m": 68.5,
    ...
    results["checks"] = {
        # The 45.6 deg heading turns a 20.92 x 29.63 m building into a ~36 m
        # axis-aligned bounding box. That is expected, not a scale error.
        # The 45.4 deg heading turns a 72.79 x 42.24 m building into a ~83 m
        # axis-aligned bounding box. That is expected, not a scale error.
        "meters_and_plausible_dimensions": 68.3 <= dims.z <= 68.7
        and 105.0 <= dims.x <= 110.0
        and 116.0 <= dims.y <= 122.0,
        "crest_normalized_to_target": abs(mx.z - 68.5) <= 0.02,
