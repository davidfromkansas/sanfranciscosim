"""Controlled review renders of the exported 165-167 South Park GLB.

    blender -b --python render_165_south_park.py -- [--glb FILE] [--out DIR]
                                               [--prefix 165-south-park] [--night]
...
RES = (1200, 1000)
AER_RES = (1600, 1200)
TOP_RES = (1100, 1400)   # portrait: the building is a 24 m sliver
BG = (0.86, 0.80, 0.69, 1.0)  # neutral warm tabletop background

FRONT_AZ = 349.73
LONG_AXIS = 135.3        # front -> rear bearing

# Azimuths are the bearings each elevation FACES, so the camera stands on that
# side looking back at the building.
VIEWS = [
    ("north", FRONT_AZ),          # street elevation, with the gate
    ("south", FRONT_AZ + 180.0),  # rear
    ("east", 62.8),               # flank toward 159
    ("west", 242.8),              # flank toward 171
]
...
def add_lights(height, night=False):
    ...
    if night:
        ...
        ob.rotation_euler = (math.radians(50), 0, math.radians(20))
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "165-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "165-south-park")
    night = "--night" in argv
    ...
    top.data.type = "ORTHO"
    top.data.ortho_scale = 27.0   # portrait frame: this is the VERTICAL extent
