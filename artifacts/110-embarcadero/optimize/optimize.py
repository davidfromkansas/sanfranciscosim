...
#
# GLB-OPTIMIZE-PROMPT s.3 step 3 says to skip this on assets with large coplanar
# ring bands, and this one is made of them: the main parapet is three slab runs
# plus two Toy_ink cap lines round the Embarcadero roof, and the Steuart front
# carries a cornice, a cornice cap, a frieze pair, a sill band and a plinth -
# every one a band that follows the full 13.9 m frontage, plus five deck joint
# bands and two trellis rails across the roof. A strictly-coplanar dissolve
# merges each of those into one annulus/strip ngon, and re-triangulating an
# annulus emits slivers: invisible, area-test-clean, and fatal two steps later
# because a sliver's shared vertex sits between opposing normals, its averaged
# vertex normal collapses toward zero, and gltfpack re-emits the STORED normals
# so the failure surfaces only in the packed file as
# invalid_or_nonunit_loop_normal_count. Measured worth on comparable assets:
