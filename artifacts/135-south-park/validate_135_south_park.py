"""Fresh-scene contract validation for 135-south-park.glb.
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3940203, 37.7811030],
        "front_heading_deg_true": 315.4,
        "target_height_m": 8.5,
    ...
    results["checks"] = {
        # Plausibility window for THIS asset. The 19.71 x 28.65 m L sits at a
        # ~45 deg heading, so its axis-aligned bbox is ~34.3 x 25.8 m — wider in
        # X than in Y, unlike 380 Brannan's near-square 31 x 31 m. The window is
        # asymmetric on purpose; a symmetric one would pass a rotated model.
        "meters_and_plausible_dimensions": 8.3 <= dims.z <= 8.7
        and 33.0 <= dims.x <= 36.0
        and 24.5 <= dims.y <= 27.5,
        "crest_normalized_to_target": abs(mx.z - 8.5) <= 0.02,
