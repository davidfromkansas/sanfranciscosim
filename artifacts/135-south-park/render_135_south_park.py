"""Controlled review renders of the exported 135 South Park GLB.
    ...
    blender -b --python render_135_south_park.py -- [--glb FILE] [--out DIR]
                                                      [--prefix 135-south-park]
                                                      [--night]
...
(north = Blender +Y), which is how the asset is authored — the South Park front faces NW (315.4 deg).
...
RES = (1500, 1000)   # landscape: this building is wide and very low (8.5 m)
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "135-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "135-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(315)  # from the NW: square onto the South Park front,
        # which is also the angle that puts the lit roof monitor across the frame
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: THE important one here — parapet ring, re-entrant rear yard,
    # --- the roof monitor, and the mechanical pair -------------------------
