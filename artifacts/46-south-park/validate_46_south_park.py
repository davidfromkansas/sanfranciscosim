"""Fresh-scene contract validation for 46-south-park.glb.

    blender -b --python validate_46_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results["checks"] = {
        # 27.9 x 27.7 x 16.15 m: the XY box is the exact 45 deg rotation of a
        # 9.47 x 29.43 m sliver, not a 28 m building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            16.05 <= dims.z <= 16.25
            and 27.4 <= dims.x <= 28.4
            and 27.3 <= dims.y <= 28.3
        ),
        "crest_normalized_to_target": abs(mx.z - 16.15) <= 0.02,
