...
report["gates"] = {
    ...
    # Gate on the DELTA, not an absolute threshold. The gate exists to catch the
    # optimizer flipping windings; an asset can carry a standing residual of its
    # own (here: applied facade panels whose back faces sit exactly in the wall
    # plane, so a ray can hit the inward-facing member of a coincident pair).
    # Judging the output alone would either fail a correct optimize or, on an
    # asset with a low residual, hide a regression inside the absolute budget.
    "G2_ray_flip_ok": (
