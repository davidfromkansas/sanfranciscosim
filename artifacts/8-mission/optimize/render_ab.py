...
# BLENDER_EEVEE, not Cycles: this Mac was at load average ~150 with ~16
# concurrent Blender processes and CPU Cycles makes no progress there. The A/B
# gate compares two renders of ONE rig against each other, so the engine only
# has to match on both sides - and EEVEE is deterministic and noise-free, which
# makes a non-zero pixel delta mean a real difference rather than sampling
# noise. (5.2 enum is BLENDER_EEVEE, not BLENDER_EEVEE_NEXT.)
