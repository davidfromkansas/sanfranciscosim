...
# GLB-OPTIMIZE-PROMPT v2 s.3.3: skip the dissolve entirely on assets with large
# coplanar ring bands. This one is nothing BUT ring bands - three parapets,
# three copings, three plinths and two sill courses, every one of them a closed
# annulus following the footprint the whole way round. Their top and bottom
# faces are perfectly coplanar annuli, so even a strictly-coplanar dissolve
# merges each ring into a single ngon, and re-triangulating an annulus emits
# hairline slivers. Those slivers pass an area-based degeneracy test and only
# surface two steps later, in the packed file, as
# invalid_or_nonunit_loop_normal_count - exactly the 350-brannan failure.
stats["limited_dissolve"] = "skipped (ring-band asset, prompt s.3.3)"
