"""Fresh-scene contract validation for 8-mission.glb.

    blender -b --python validate_49_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3932861, 37.7936872],
        "mission_heading_deg_true": 135.37,
        "embarcadero_heading_deg_true": 45.37,
        "steuart_heading_deg_true": 225.37,
        "don_chee_heading_deg_true": 315.37,
        "roof_plateaus_m": [25.10, 19.64, 14.18],
        "target_height_m": 28.66,
    ...
    results["checks"] = {
        # ~74.1 x 56.5 x 28.66 m: the XY box is the 45.37 deg rotation of an
        # L-shaped 64.08 x 42.07 m block, not a 74 m building. It is NOT square
        # (the notch takes a corner out of the plan), so the two XY figures
        # differ - see REPORT.md.
        "meters_and_plausible_dimensions": (
            28.5 <= dims.z <= 28.8
            and 72.0 <= dims.x <= 77.0
            and 54.0 <= dims.y <= 59.0
        ),
        "crest_normalized_to_target": abs(mx.z - 28.66) <= 0.02,
