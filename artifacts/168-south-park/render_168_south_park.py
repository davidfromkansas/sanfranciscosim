"""Controlled review renders of the exported 166-168 South Park GLB.

    blender -b --python render_168_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 168-south-park]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the South Park front faces SE (135 deg).
...
RES = (1400, 1050)   # the 45 deg heading makes every elevation ~26 m wide
AER_RES = (1400, 1200)
TOP_RES = (1200, 1200)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "168-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "168-south-park")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(135)  # from the SE: the South Park front is the identity
        r = span * 3.1
        aer.location = Vector(
            (
    ...
    # --- top view: the white membrane inside its parapet ring, and its vents ----
    ...
    aer.data.type = "PERSP"
    # 105 mm cropped this asset: it is 30 m long on the diagonal inside a 26 m
    # AABB, so the framing heuristic underestimates it. 70 mm at a longer
    # radius keeps the restrained perspective and fits the whole building.
    aer.data.lens = 70.0
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(135)  # from the SE — square onto the South Park front
    r = span * 3.4
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
    ...
    # --- square-on to the South Park front ----------------------------------
    # The four compass elevations each show two faces at 45 deg, which is
    # correct for a building on this heading but useless for judging the one
    # elevation that carries design. This view is orthographic straight at it.
    fr = make_camera("cam_front")
    fr.data.type = "ORTHO"
    fr.data.ortho_scale = height * 1.35
    a = math.radians(135)
    fr.location = Vector((center.x + dist * math.sin(a), center.y + dist * math.cos(a), center.z))
    aim(fr, center)
    render_to(os.path.join(out, f"{prefix}-front.png"), fr, (900, 1200))
