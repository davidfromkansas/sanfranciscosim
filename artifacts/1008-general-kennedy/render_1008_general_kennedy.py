...
(north = Blender +Y), which is how the asset is authored — the head block faces ESE (116.85 deg).
...
RES = (1800, 900)    # landscape, and extreme: 55 m long, 12 m across, 12 m tall
AER_RES = (1600, 1100)
TOP_RES = (1500, 1100)
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "1008-general-kennedy.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "1008-general-kennedy")
    night = "--night" in argv
    ...
    if night:
        ...
        az = math.radians(158)  # off the southwest flank, matching the day aerial
        r = span * 2.15
        aer.location = Vector(
            (
    ...
    # --- top view: the hip lines, the ridge, the chimneys, the head's own hip ---
