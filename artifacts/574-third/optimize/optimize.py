...
# --- 3. limited dissolve: SKIPPED (GLB-OPTIMIZE-PROMPT s.3 step 3) ---
# This asset is built from ring bands that follow the footprint all the way
# round — base band, floor-line course, parapet and coping. Their top and bottom
...
# metre-long, sub-millimetre-wide slivers whose averaged vertex normals collapse
# to ~0 — invisible to an area-based degeneracy test, fatal to the stage-2
# contract validator once gltfpack re-emits the stored normals. That is exactly
# the 350-brannan failure the prompt documents. The step was worth ~0.3% of
# triangles on this asset, so it is reverted under s.11 rather than worked around.
SKIP_DISSOLVE = True
for o in ([] if SKIP_DISSOLVE else mesh_objs()):
