"""Fresh-scene contract validation for st-marys-cathedral.glb.

    blender -b --python validate_st_marys_cathedral.py -- [--glb FILE] [--out FILE]
...
TRI_BUDGET = 18000  # the plan's cap for this asset (contract allows 27,000)
...
def main():
    ...
    # Visibility-based outward-normal test. The asset contains open sheets (the
    # crown cap and the glow ribbons), so signed volume cannot judge it: instead
    # fire a deterministic sphere of rays inward at nine targets and require
    # every first visible face to oppose its incoming ray.
    ray_hits = 0
    ray_flipped = 0
    ...
    for target in targets:
        for i in range(2500):
            ...
            hit, _, normal, _, _, _ = bpy.context.scene.ray_cast(
                dg, target + outward * 600.0, direction, distance=900.0
            )
            if hit:
                ray_hits += 1
    ...
    results = {
        ...
        "normal_orientation_status": (
            "PASS" if invalid_normal_count == 0 and ray_hits > 0 and ray_flipped == 0 else "FAIL"
        ),
        "normal_orientation_method": (
            "Closed solids run bmesh.ops.recalc_face_normals before export; the open "
            "sheets are wound by hand. Re-imported loop normals must be finite and unit, "
            "and 22,500 deterministic visibility rays must find no flipped first face."
        ),
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 70 <= dims.z <= 85 and 75 <= dims.x <= 100,
