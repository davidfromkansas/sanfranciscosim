"""Fresh-scene contract validation for sf-main-library.glb.

    blender -b --python validate_sf_main_library.py -- [--glb FILE] [--out FILE]
                                                    [--closed-solids]
...
Normals: the library is a union of closed solids, so per-object signed volume
...
(`optimize/input/sf-main-library.glb`) to additionally assert that every object is
...
def main():
    ...
    results = {
        ...
        "xy_origin_note": (
            "The origin is the XY centre of the model's own bounding box, which "
            "sits on the surveyed OBB centre - the manifest anchor and what "
            "placeGeneric positions. The OSM polygon centroid and OBB centre "
            "agree to 0.07 m here, so the two candidate anchors coincide."
        ),
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": (
            # The 106.42 x 56.88 m surveyed envelope rotated 9.06 deg onto the
            # Civic Center grid, plus the cornice and pavilion projections.
            112 <= dims.x <= 122 and 70 <= dims.y <= 80 and abs(dims.z - TARGET_HEIGHT) <= 0.05
        ),
