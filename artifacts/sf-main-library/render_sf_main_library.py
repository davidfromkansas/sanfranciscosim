"""Controlled review renders of the exported SF Main Public Library GLB.

    blender -b --python render_sf_main_library.py -- [--glb FILE] [--out DIR]
                                                     [--prefix sf-main-library]
                                                     [--only aerial|night]
                                                     [--fast]
...
This is a wide, low building whose roof is the design, so the rig is framed on
the plan extent rather than the height and the aerial sits SOUTH-WEST: that is
the one angle that shows the Larkin classical front and the Grove modern flank
in the same frame, which is the whole point of the building.
"""
...
def night_state():
    ...
    *_Glow is drawn as an UNLIT overlay whose opacity rises with the real San
    ...
    their own baked colour, self-luminous.
    """
    ...
    for mat in bpy.data.materials:
        if mat.name.endswith("_Glow") and mat.use_nodes:
            ...
            if bsdf:
                ...
                # material carries a default WHITE emission.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "sf-main-library.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "sf-main-library")
    only = arg("--only", "")
    if "--fast" in argv:
        SAMPLES = 16
    ...
    # SW of the block: the Larkin classical front on the left, the Grove modern
    # flank on the right, the whole roof composition in between.
    ...
    # --- top view: the oculus, the pyramid, the two sheds, the mech box -----
    ...
    ncam = make_camera("cam_night_larkin")
    ncam.data.type = "PERSP"
    ncam.data.lens = 62.0
    pitch_n = math.radians(15)
    az_n = math.radians(272)  # on Civic Center Plaza, facing the Larkin front
    rn = span * 1.95
    ...
    aim(ncam, Vector((ground.x, ground.y, mn.z + height * 0.45)))
    render_to(os.path.join(out, f"{prefix}-night-larkin.png"), ncam, (1500, 900))
