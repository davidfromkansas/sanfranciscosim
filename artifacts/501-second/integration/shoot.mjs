...
const DIST = Number(arg('--dist', '200'));
const YAW = Number(arg('--yaw', '180'));
const WIDE = Number(arg('--wide', '900'));
...
async function main() {
  ...
  for (const [tag, clock] of [
    ...
    await evalJs(`window.SF.goTo(${LON}, ${LAT}, ${DIST}, ${YAW}, 26)`);
    await sleep(14000);
  ...
  await evalJs(`window.SF.goTo(${LON}, ${LAT}, ${WIDE}, ${YAW + 20}, 30)`);
  await sleep(12000);
