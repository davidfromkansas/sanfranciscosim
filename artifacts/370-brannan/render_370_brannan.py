"""Controlled review renders of the exported 370 Brannan Street GLB.
...
(north = Blender +Y), which is how the asset is authored — the Brannan Street front faces SE (134.9 deg).
...
def main():
    ...
    if night:
        ...
        az = math.radians(135)  # from the SE: the Brannan front is the identity
        r = span * 3.6
        aer.location = Vector(
            (
    ...
    # --- top view: parapet ring, the two skylights, the roof light and the hatch ----------
    ...
    # 105 mm at span*3.1 crops this asset: its 23.8 m depth against a 7.6 m
    # height makes `span` the plan diagonal, not the elevation, so the same rig
    # that framed 380 Brannan runs off the top and bottom of frame here.
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(135)  # from the SE — square onto the Brannan front
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
