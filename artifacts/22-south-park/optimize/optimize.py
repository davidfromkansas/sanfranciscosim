...
# --- 3. limited dissolve — SKIPPED (prompt §3.3) ---
# This asset is built from closed prisms plus two closed RING BANDS — the parapet
# frieze, which follows the whole 7-vertex trapezoid footprint, and the
# light-well curb — and from long coplanar strips (the cornice crown, the belt
# course, the storefront band on four arc segments). Re-triangulating an annulus
# ngon emits slivers up to the full ring length; they pass every area-based
# degeneracy test, collapse their shared vertex normals to ~0, and only surface
# after the shipping swap as invalid_or_nonunit_loop_normal_count in the stage-2
# contract validator (the 350-brannan failure of 13 Aug 2026, and the same call
# 106 South Park made). On a 6.7k-triangle asset with no curved shells the step
# is worth a handful of triangles at most.
stats["limited_dissolve"] = "skipped: closed ring bands (parapet frieze, well curb) and long coplanar cornice/belt strips"
