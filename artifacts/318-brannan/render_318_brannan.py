"""Controlled review renders of the exported 350 Brannan Street GLB.
    ...
    blender -b --python render_318_brannan.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 318-brannan]
                                                    [--night]
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "318-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "318-brannan")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(168)  # from the SSE: the Brannan front AND the SW flank
        r = span * 3.6
        aer.location = Vector(
            (
...
def render_aerial(center, span, out, prefix):
    ...
    az = math.radians(168)  # from the SSE — the Brannan front AND the SW flank
    # in one frame, which is what the app's camera gets. Square-on from the SE
    # would flatten the two awning bands into a single stripe.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
