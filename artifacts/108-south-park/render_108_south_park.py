"""Controlled review renders of the exported 108-110 South Park GLB.

    blender -b --python render_108_south_park.py -- [--glb FILE] [--out DIR]
                                               [--prefix 108-south-park] [--night]
...
The building sits ~135 deg off the world axes, so the cameras are aligned to the
...
faces. Each view keeps the nearest compass filename:
    ...
    -south.png  shopfront elevation, faces 135.35 deg (South Park, the sign)
    -north.png  rear elevation, faces 315.35 deg (Taber Place, the carriage door)
    -east.png   party flank toward 104-106, faces 45.35 deg (blind)
    -west.png   party flank toward 112, faces 225.35 deg (blind above 6 m)

The building is 4.6x deeper than it is wide, so the elevations are framed to the
...
RES = (1200, 1000)
AER_RES = (1600, 1200)
TOP_RES = (1100, 1400)   # portrait: the building is a 29.8 m sliver
...
VIEWS = [
    ("south", FRONT_AZ),      # shopfront
    ("north", REAR_AZ),       # rear on Taber Place
    ("east", 45.35),          # flank toward 104-106
    ("west", 225.35),         # flank toward 112
]

# A three-quarter swung ~20 deg round from the facade normal toward the
# south-west flank. Square to the facade would hide the depth, which is a
# primary recognition cue; swinging the other way would show the north-east
# flank, which in the real city is buried behind an 11 m neighbour and is the
# one face a reviewer should NOT be judging.
AERIAL_AZ = 156.0
AERIAL_PITCH = 34.0
AERIAL_R = 2.85
AERIAL_LENS = 62.0
# The aim point is pushed 7 m from the bbox centre TOWARD the shopfront. Aiming
# at the centre of a 29.8 m sliver put the only designed elevation off the
# bottom edge of frame and filled the picture with roof (first review pass).
AERIAL_AIM_FWD = 7.0
...
def add_lights(height, night=False):
    ...
    if night:
        ...
        ob.rotation_euler = (math.radians(55), 0, math.radians(20))
    else:
        ...
        bpy.context.collection.objects.link(ob)
        # Keyed from the south-east so the shopfront, the gold sign and the
        # cornice all catch the light and the awnings throw their own shadow.
        ...
        ob2.rotation_euler = (math.radians(65), 0, math.radians(340))
        ...
        ob3.rotation_euler = (math.radians(78), 0, math.radians(260))
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "108-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "108-south-park")
    night = "--night" in argv
    ...
    fwd = Vector((math.sin(math.radians(FRONT_AZ)),
                  math.cos(math.radians(FRONT_AZ)), 0.0))
    aim_at = Vector((center.x + fwd.x * AERIAL_AIM_FWD,
                     center.y + fwd.y * AERIAL_AIM_FWD,
                     mn.z + 0.42 * height))

    if night:
        light_glow()
        aer = make_camera("cam_night")
        aer.data.lens = AERIAL_LENS
    ...
    # --- top view: the skylight line, the parapet ring and the cornice ------
    # Rolled to the building's axis so the sliver runs up the frame with the
    # shopfront end at the top. For a top-down camera image-up maps to world
    # (-sin rz, cos rz), so rz = -FRONT_AZ puts the front normal at the top.
    ...
    top.rotation_euler = (0, 0, math.radians(-FRONT_AZ))
    ...
    aer.data.type = "PERSP"
    aer.data.lens = AERIAL_LENS  # long lens, restrained perspective (bible §18)
    aer.location = orbit(center, AERIAL_AZ, AERIAL_PITCH, span * AERIAL_R)
    aim(aer, aim_at)
