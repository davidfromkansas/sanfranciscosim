"""Fresh-scene contract validation for 108-south-park.glb.

    blender -b --python validate_165_south_park.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3944817, 37.7816789],
        "front_heading_deg_true": 135.35,
        "rear_heading_deg_true": 315.35,
        "long_axis_heading_deg_true": 315.35,
        "target_height_m": 8.45,
    ...
    results["checks"] = {
        # 26.2 x 25.7 x 8.45 m: the XY box is the ~135 deg rotation of a
        # 6.43 x 29.75 m sliver plus the awnings' 0.80 m projection, not a 26 m
        # building. See REPORT.md.
        "meters_and_plausible_dimensions": (
            8.4 <= dims.z <= 8.5
            and 25.7 <= dims.x <= 26.7
            and 25.2 <= dims.y <= 26.2
        ),
        "crest_normalized_to_target": abs(mx.z - 8.45) <= 0.02,
