"""Fresh-scene contract validation for 4-embarcadero-center.glb.

    blender -b --python validate_226_ritch.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    # Deterministic visibility-ray normal test
    ray_hits = 0
    ray_flipped = 0
    ...
    results = {
        ...
        "anchor_lonlat": [-122.3961998, 37.7953001],
        "long_axis_bearing_deg_true": 81.09,
        "entrance_heading_deg_true": 351.09,
        "main_parapet_m": 173.70,
        "target_height_m": 179.00,
    ...
    results["checks"] = {
        # The tower plan is 63.46 x 37.34 m at an 81.09 deg bearing and the
        # podium plinth around it is 68 x 42 m, so the axis-aligned box is
        # ~73 x 52 m. That is the expected consequence of the real-world
        # orientation, not a scale error.
        "meters_and_plausible_dimensions": 178.8 <= dims.z <= 179.2
        and 70.0 <= dims.x <= 76.0
        and 49.0 <= dims.y <= 55.0,
        "crest_normalized_to_target": abs(mx.z - 179.00) <= 0.02,
