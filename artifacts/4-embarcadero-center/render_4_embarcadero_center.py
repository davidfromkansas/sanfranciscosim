"""Controlled review renders of the exported Four Embarcadero Center GLB.

    blender -b --python render_4_embarcadero_center.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 4-embarcadero-center]
                                                    [--night] [--only VIEW]
                                                    [--samples N]
...
(north = Blender +Y), which is how the asset is authored — the long axis bears
81.09 deg and the entrance faces NORTH onto Clay Street.

The EAST and WEST elevations are the money shots: they are the only views in
which the stepped chevron crown reads.
...
at the defaults (EEVEE, 128 samples), which is what the committed images are.
...
RES = (1000, 1500)   # the tower is 179 m tall and 63 m wide: portrait
AER_RES = (1200, 1400)
TOP_RES = (1200, 1200)
...
def fade_glow():
    """Preview the app's DAY state.

    A _Glow material is opaque by day and renders at its base colour — the
    emission is what the night pass adds.  Rendering it at 12% alpha (as some
    older rigs did) both lies about the day appearance and puts translucent
    haze over everything behind it, which on a 179 m tower is most of the
    tower.  Day = base colour, emission off.
    """
    for mat, bsdf in glow_materials():
        if not bsdf:
            continue
...
def render_top(center, mx, width, span, out, prefix):
    """Top view: the full roof, penthouse, terrace and mechanical layout."""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "4-embarcadero-center.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "4-embarcadero-center")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    if night:
        ...
        az = math.radians(42)  # same three-quarter as the day aerial: the crown
        # rings and the scattered lit windows read on the north face and the
        # east chevron at once.
        r = span * 4.4
        aer.location = Vector(
            (
    ...
    # 42 deg puts the camera north-east, near the bisector of the north face
    # (normal 351 deg) and the east end (81 deg): the blunt cliff, the stepped
    # chevron crown and the cooling-tower roof all read in one frame, which is
    # the whole argument for this building.
    az = math.radians(42)
    r = span * 4.4
    aer.location = Vector(
        (
