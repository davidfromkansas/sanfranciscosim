...
DO_DEGEN = "--no-degenerate" not in FLAGS
DO_INTERIOR = "--no-interior" not in FLAGS
DO_DISSOLVE = "--no-dissolve" not in FLAGS
DO_JOIN = "--no-join" not in FLAGS
...
# NOTE (sf3d): the shade_flat() guard that protects flat shading after a weld
# must NOT fire when the weld is skipped — calling it on a freshly imported
# mesh discards the glTF's custom split normals and costs thousands of
# exported vertices, which makes every no-weld variant look worse than it is.
if DO_WELD or DO_DEGEN:
    for o in mesh_objs():
        bm = bmesh.new()
        bm.from_mesh(o.data)
        if DO_WELD:
...
interior_removed = 0
for o in (mesh_objs() if DO_INTERIOR else []):
    bm = bmesh.new()
    bm.from_mesh(o.data)
    doomed = []
    for f in bm.faces:
...
# GLB-OPTIMIZE-PROMPT s.3.3 says to SKIP this on assets carrying large coplanar
# ring bands, because re-triangulating a merged annulus emits hairline slivers
# that only surface in the packed file's stored normals. This asset has 14 of
# them (parapet, trim_base, 12 fin caps), so the variant table measures it and
# the shipped build turns it off unless it pays for itself.
...
for o in (mesh_objs() if DO_JOIN else []):
