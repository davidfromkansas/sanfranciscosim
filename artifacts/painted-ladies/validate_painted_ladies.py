"""Fresh-scene validation of the final Painted Ladies GLB.

    blender -b --python validate_painted_ladies.py -- [--glb FILE] [--out FILE]
"""
...
def main():
    ...
    allowed = {
        "Toy_brick", "Toy_coral", "Toy_cream", "Toy_glass", "Toy_gold", "Toy_gold_Glow",
        "Toy_ink", "Toy_mint", "Toy_mustard", "Toy_red", "Toy_roofd",
        "Toy_rust", "Toy_sand", "Toy_sky", "Toy_stone", "Toy_trim",
        "Toy_verdigris",
    }
    mat_rows = []
    textured = []
    ...
    for mat in bpy.data.materials:
        ...
        if mat.name not in allowed or not mat.name.startswith("Toy_") or mat.name == "Toy_body":
            off_contract.append(mat.name)
        if mat.name.endswith("_Glow") and mat.name != "Toy_gold_Glow":
            glow_violations.append(mat.name)
        mat_rows.append({
            "name": mat.name,
            ...
            "exported_emission_strength": round(emission, 4),
            "contract_compliant": mat.name in allowed and mat.name != "Toy_body",
        })
    ...
    # Deterministic first-hit visibility rays test externally visible normals.
    ray_hits = ray_flipped = 0
    flipped_by_object = {}
    golden = math.pi * (3.0 - math.sqrt(5.0))
    targets = [
        Vector((center.x + dx * dims.x, center.y + dy * dims.y, mn.z + fz * dims.z))
        for dx, dy in ((0, 0), (-0.25, -0.12), (0.25, 0.12))
        for fz in (0.15, 0.52, 0.86)
    ]
    for target in targets:
        for i in range(1200):
            q = 1.0 - 2.0 * (i + 0.5) / 1200
            r = math.sqrt(max(0.0, 1.0 - q * q))
            a = golden * i
            outward = Vector((math.cos(a) * r, math.sin(a) * r, q))
            direction = -outward
            hit, _, normal, _, hit_obj, _ = bpy.context.scene.ray_cast(
                dg, target + outward * 1000, direction, distance=1400
            )
            if hit:
                ray_hits += 1
    ...
    results = {
        ...
        "invalid_or_nonunit_loop_normal_count": invalid_normals,
        ...
        "normal_orientation_method": "Reimported loop normals are finite/unit; 10,800 deterministic first-hit visibility rays verify outward visible faces with a 0.5% tolerance.",
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 20 <= dims.x <= 28 and 40 <= dims.y <= 48 and 12 <= dims.z <= 19,
    ...
    print(json.dumps({k: results[k] for k in (
        "overall", "triangle_count", "dimensions_m", "bbox_min_m", "bbox_max_m", "checks"
    )}, indent=2))
