"""Compose the six GLB review renders into painted-ladies-contact-sheet.png."""
...
def load(view):
    img = Image.open(HERE / f"painted-ladies-{view}.png").convert("RGB")
    w = CELL_W
...
def main():
    ...
    row_h = [LABEL_H + max(t.height for t in row) for row in rows]
    ...
    out = HERE / "painted-ladies-contact-sheet.png"
    sheet.save(out)
