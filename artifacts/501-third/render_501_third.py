"""Controlled review renders of the exported 501 Third Street GLB.

    blender -b --python render_501_third.py -- [--glb FILE] [--out DIR]
                                               [--prefix 501-third] [--night]
...
The hero view is the aerial from due north-east, where the 3rd Street front
reads against the rhombus footprint and the rooftop bulkhead; the top view
carries the roof composition.
"""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "501-third.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "501-third")
    night = "--night" in argv
    only = arg("--only", "")
    ...
    if night:
        ...
        aim(aer, Vector((center.x, center.y, mn.z + 5.0)))
    ...
    if only in ("", "elev"):
        ...
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), mn.z + 6.0))
            aim(cam, Vector((center.x, center.y, mn.z + 6.0)))
