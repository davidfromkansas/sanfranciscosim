"""Fresh-scene contract validation for 501-third.glb.

    blender -b --python validate_501_third.py -- [--glb FILE] [--out FILE]
...
Normals are checked two ways, because this asset is a union of ~86 closed
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(16.4 / dims.z, 6),
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": 16.3 <= dims.z <= 16.5
        and 34.0 <= dims.x <= 35.5
        and 34.0 <= dims.y <= 35.5,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 16.4) <= 0.01,
