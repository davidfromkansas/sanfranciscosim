# Gate G1/G2/G5 validation (Blender side) — run headless:
#   "$BLENDER" -b --python validate.py -- <input.glb> <optimized.glb> <out.json>
#
# Adapted for 501 Third Street: the ray-flip gate is a DELTA against the input,
# not an absolute 0.15%. This asset carries deliberately single-sided surfaces
# (the seven Toy_white_Glow shells are thin panels proud of opaque glazing, and
# the roof guardrail is an open ribbon), so it has a standing residual of its
# own. The gate exists to catch the optimizer flipping windings, so both files
# are ray-tested with the same seed and the OUTPUT must not be worse than the
# INPUT by more than 0.15 percentage points. The absolute figure is reported
# either way.
...
print("ray_in", json.dumps(report["ray_test_input"]))
print("ray_out", json.dumps(report["ray_test_output"]))
print("ray_delta", report["ray_flip_delta"])
