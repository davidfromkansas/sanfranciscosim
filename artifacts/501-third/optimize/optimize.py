...
#                                         [--no-weld] [--no-dissolve]
#
# Adapted for 501 Third Street.
#
# Steps (per GLB-OPTIMIZE-PROMPT §3):
...
#      --no-weld makes this measurable: on a flat-shaded box asset the weld can
#      cost bytes (the "coincident" verts ARE the flat-shading topology), while
#      on a beveled one it removes genuinely redundant bevel verts. 501 Third is
#      beveled throughout, so the four-variant table decides — see REPORT.md §3.
...
#   3. limited dissolve, 0.05 deg — DEFAULT OFF here (--no-dissolve is implied
#      unless --dissolve is passed). GLB-OPTIMIZE-PROMPT §3.3 says to skip this
#      step entirely on assets with large coplanar ring bands, and this asset
#      has two full-footprint annuli (parapet, parapet_cap, 288 tris each) plus
#      the rooftop guardrail ring. Re-triangulating an annulus ngon emits
#      metre-long slivers that only fail later, in the packed file.
#   4. (skipped) curve retess — no curved shells on this asset; every solid is
#      a beveled prism or box.
...
INPUT, OUTPUT, STATS = POS[0], POS[1], POS[2]
DO_WELD = "--no-weld" not in FLAGS
DO_DISSOLVE = "--dissolve" in FLAGS      # off by default: ring bands, see header
print(f"OPTIONS weld={DO_WELD} dissolve={DO_DISSOLVE}")
...
snap("weld+degenerate" if DO_WELD else "degenerate-only")
...
# --- 3. limited dissolve, coplanar only (OFF by default on this asset) ---
stats["dissolved"] = DO_DISSOLVE
for o in (mesh_objs() if DO_DISSOLVE else []):
...
snap("limited-dissolve" if DO_DISSOLVE else "dissolve-skipped")
