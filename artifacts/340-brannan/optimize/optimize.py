...
#   4. (skipped) curve retess — the only curves here are two 12-segment cooling
#      towers, already at the miniature style's low-seg floor
...
# --- 3. limited dissolve — DISABLED FOR THIS ASSET (per-asset adaptation) ---
#
# The generic script runs a 0.05 deg limited dissolve here. It stays off on this
# asset for the reason GLB-OPTIMIZE-PROMPT s.3 step 3 now documents, first
# measured on 350 Brannan across the alley: 340 Brannan has the same hazard and
# more of it — a parapet ring_band, a coping ring_band AND a raised parapet
# course, all following the footprint.
#
# What it did: the parapet and its coping are ring_band solids whose bottom and
# top faces are large, PERFECTLY coplanar annuli following the footprint. A
# strictly-coplanar dissolve is therefore free to merge each whole ring into one
# annulus ngon, and re-triangulating an annulus produces slivers — measured here
# as 7 triangles up to 24.35 m long with areas of 0.003-0.016 m2 (a width of
# ~0.24 mm). They are invisible, and they pass an area-based degeneracy test.
#
# Why that is not acceptable: a sliver's shared vertex sits between faces with
# opposing normals, so its averaged vertex normal collapses to ~0 (measured
# length 4.8e-05). Blender recomputes loop normals on import and hides this,
# but gltfpack re-emits the STORED normals, and the stage-2 contract validator
# then fails on `invalid_or_nonunit_loop_normal_count`. That is a real contract
# failure reached through an invisible defect.
#
# What it bought: 6,770 -> 6,740 triangles. 30 triangles, 0.4%. Not a trade.
#
# This is not the 0.5-deg transitive-chain hazard the prompt already documents;
# it is a distinct one that only appears on assets with large coplanar ring
# bands. The prompt has been updated with the finding.
...
# --- 6. prune unused data (GLB-OPTIMIZE-PROMPT s.6) ---
#
# The "340" numerals come from a Blender text object converted to a mesh, and
# the converter always emits a UV layer. Nothing in this asset is textured (the
# contract forbids it), so every UV is dead weight that gltfpack would still
# encode as a TEXCOORD_0 accessor on the whole joined Toy_white group. Same for
# any stray custom split normals on flat-shaded geometry.
uv_layers_removed = 0
for o in mesh_objs():
    while o.data.uv_layers:
        o.data.uv_layers.remove(o.data.uv_layers[0])
        uv_layers_removed += 1
stats["uv_layers_removed"] = uv_layers_removed
snap("prune-uvs")
