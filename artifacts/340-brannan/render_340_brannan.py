"""Controlled review renders of the exported 350 Brannan Street GLB.

    blender -b --python render_340_brannan.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 340-brannan]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the Brannan Street front faces SE (135.4 deg).
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "340-brannan.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "340-brannan")
    night = "--night" in argv
    only = arg("--only", None)
    ...
    # --- top view: parapet ring, penthouse, trellis, cooling towers, roof deck ----
...
def render_aerial(center, span, out, prefix):
    ...
    az = math.radians(170)  # from the S — the Brannan front AND the Jack London
    # Alley flank in one frame, which is what the app's camera gets. This
    # building's two finished faces meet at its SOUTH corner, so the hero aerial
    # comes from the south; anything east of ~140 deg puts a party wall in shot.
    r = span * 3.6
    aer.location = Vector(
        (
            center.x + r * math.cos(pitch) * math.sin(az),
