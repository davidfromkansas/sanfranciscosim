// Stage-5 local QA for 340-brannan, INTEGRATION-PROMPT.md Steps 5 and 6.
//
...
//
//   node artifacts/340-brannan/qa_local.mjs [--drill]
//
...
const LON = -122.3932324;
const LAT = 37.7812786;
const SLUG = '340-brannan';
const CAMERA = { distance: 240, yaw: 10, pitch: 26 }; // the registry preset
...
async function main() {
  ...
  const profile = await mkdtemp(path.join(os.tmpdir(), 'qa340-'));
  ...
  const stats = await until(
    ...
    DRILL
      ...
      : `(() => (window.SF.assets.placed.has('340Brannan') && window.SF.assets.stats().fading === 0)
  ...
  const placed = await evaluate(`(() => ({
    ...
    has340: window.SF.assets.placed.has('340Brannan'),
    place340: window.SF.assets.placed.get('340Brannan') ? JSON.parse(JSON.stringify({ log: window.SF.assets.placed.get('340Brannan').log, scale: window.SF.assets.placed.get('340Brannan').scale })) : null,
  }))()`);
  console.log('  rig:', JSON.stringify(placed.rig));
  console.log('  placed340:', placed.has340, JSON.stringify(placed.place340));
