"""Fresh-scene contract validation for 101-grove.glb.

    blender -b --python validate_101_grove.py -- [--glb FILE] [--out FILE]
...
Normals are checked two ways, because this asset is a union of ~610 closed
...
def main():
    ...
    results = {
        ...
        "loader_scale_factor": round(21.4 / dims.z, 6),
        ...
        "anchor_lonlat": [-122.4186747, 37.7781359],
        "long_axis_heading_deg_true": 80.6,
        "front_normal_heading_deg_true": 350.6,
        "entrance_bay_normal_heading_deg_true": 34.4,
        "object_details": sorted(object_rows, key=lambda x: x["name"]),
    }
    results["checks"] = {
        "meters_and_plausible_dimensions": 21.3 <= dims.z <= 21.5
        and 69.5 <= dims.x <= 71.5
        and 46.5 <= dims.y <= 48.5,
        "base_at_z_zero": abs(mn.z) <= 0.5,
        "crest_is_target_height": abs(mx.z - 21.4) <= 0.01,
