"""Controlled review renders of the exported 101 Grove Street GLB.

    blender -b --python render_101_grove.py -- [--glb FILE] [--out DIR]
                                               [--prefix 101-grove] [--night]
                                               [--only top|aerial|elev]
...
(north = Blender +Y), which is how the asset is authored — the Grove Street
front faces north and the entrance bay faces north-east.

The aerial is the hero image for this asset: it is framed off the north-east so
the chamfered corner bay, the Grove elevation and the Polk elevation all read in
one frame.

--night previews the app's dusk pass: _Glow materials get their emission turned
up under a dark moonlit world, producing <prefix>-night.png. Emission is driven
from Base Color, because glTF writes emissiveFactor = 0 for an authored strength
of 0 and a re-imported _Glow material would otherwise render pure white.
"""
...
RES = (1500, 640)   # elevations: this building is 21 m tall on a 70 m front
AER_RES = (1600, 1100)
TOP_RES = (1500, 1500)
...
# Square onto the corner bay's outward normal (34.4 deg true), which is the one
# viewpoint that shows the entrance, Grove and Polk together (style bible s.18).
AERIAL_AZ = 38.0
...
def light_glow():
    """Preview the app's night pass. Copy Base Color into Emission Color first:
    a re-imported GLB carries emissiveFactor = 0, so raising strength alone
    renders every glow surface as a white slab (see docs/asset-plans/README.md)."""
...
def place_aerial(name, center, mn, span):
    cam = make_camera(name)
    cam.data.type = "PERSP"
    cam.data.lens = 105.0  # long lens, restrained perspective (style bible s.18)
    pitch = math.radians(38)  # 30-50 deg downward
    az = math.radians(AERIAL_AZ)
    r = span * 3.9
    cam.location = Vector((center.x + r * math.cos(pitch) * math.sin(az),
                           center.y + r * math.cos(pitch) * math.cos(az),
                           mn.z + r * math.sin(pitch)))
    aim(cam, Vector((center.x, center.y, mn.z + 8.0)))
    return cam
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "101-grove.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "101-grove")
    night = "--night" in argv
    only = arg("--only", "")
    ...
    if night:
        ...
        render_to(os.path.join(out, f"{prefix}-night.png"),
                  place_aerial("cam_night", center, mn, span), AER_RES)
        return

    # --- beauty render from the app's high three-quarter aerial camera -----
    if only in ("", "aerial"):
        render_to(os.path.join(out, f"{prefix}-aerial.png"),
                  place_aerial("cam_aerial", center, mn, span), AER_RES)

    # --- top view ----------------------------------------------------------
    if only in ("", "top"):
        top = make_camera("cam_top")
        top.data.type = "ORTHO"
        top.data.ortho_scale = span * 1.16
    ...
    if only in ("", "elev"):
        ...
        for name, az in VIEWS:
            ...
            cam.location = Vector((center.x + dist * math.sin(a),
                                   center.y + dist * math.cos(a), eye))
            aim(cam, Vector((center.x, center.y, eye)))
