"""Controlled review renders of the exported Civic Center Courthouse GLB.

    blender -b --python render_orpheum_theatre.py -- [--glb FILE] [--out DIR]
                                                     [--prefix orpheum-theatre]
...
(north = Blender +Y, how the asset is authored). The Market Street show front
bears 45.9 deg, so it faces SOUTH-EAST: it is split between the "south" and
"east" elevations and is seen square only in the aerial and night views, which
stand off at 136 deg. That is the site, not a rig error.
"""
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "orpheum-theatre.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "orpheum-theatre")
    ...
    # The roof is the surface the app camera actually sees. The three-sun
    # elevation rig flattens it, so this one camera rakes the key down to 34 deg
    # and drops the fill: the tile eaves, the hipped auditorium roof, the plant
    # and the stage house get real shadows. Restored immediately afterwards so
    # the aerial and night views use the same rig as the elevations.
    top = make_camera("cam_top")
    top.data.type = "ORTHO"
    top.data.ortho_scale = span * 1.12
    top.location = Vector((center.x, center.y, mx.z + span))
    top.rotation_euler = (0, 0, math.radians(-45.9) + math.pi / 2)
    saved = {light.name: (light.energy, tuple(ob.rotation_euler))
             for light in bpy.data.lights
             for ob in bpy.data.objects if ob.data is light}
    for light in bpy.data.lights:
        for ob in bpy.data.objects:
            if ob.data is not light:
                continue
            if light.name == "key":
                light.energy = 2.4
                ob.rotation_euler = (math.radians(34), 0, math.radians(-52))
            else:
                light.energy = 0.18
    render_to(os.path.join(out, f"{prefix}-top.png"), top, TOP_RES)
    for light in bpy.data.lights:
        for ob in bpy.data.objects:
            if ob.data is light and light.name in saved:
                light.energy, ob.rotation_euler = saved[light.name]

    # --- beauty render from the app's high three-quarter aerial camera ------
    aer = make_camera("cam_aerial")
    aer.data.type = "PERSP"
    aer.data.lens = 82.0               # long lens, restrained perspective (s.18)
    pitch = math.radians(38)           # 30-50 deg downward
    az = math.radians(180)              # due south: Market front AND the sign face
    r = span * 3.15
    aer.location = Vector(
        (
        ...
        if mat.name.endswith("_Glow") and mat.use_nodes:
            ...
            if bsdf:
                ...
                bsdf.inputs["Emission Strength"].default_value = {
                    "Toy_white_Glow": 3.6,      # sign bulbs + marquee chase
                }.get(mat.name, 2.2)            # warm marquee soffit
    render_to(os.path.join(out, f"{prefix}-night.png"), aer, AER_RES)

    # night closeup: the sign and marquee from across Market
    ncam = make_camera("cam_night_corner")
    ncam.data.type = "PERSP"
    ncam.data.lens = 55.0
    pitch_n = math.radians(20)
    az_n = math.radians(180)
    rn = height * 4.4
    ...
    aim(ncam, Vector((center.x, center.y, mn.z + height * 0.45)))
    render_to(os.path.join(out, f"{prefix}-night-market.png"), ncam, (1300, 1100))
