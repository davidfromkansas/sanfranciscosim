"""Fresh-scene contract validation for orpheum-theatre.glb.

    blender -b --python validate_orpheum_theatre.py -- [--glb FILE] [--out FILE]
...
def main():
    ...
    results = {
        ...
        "heading_deg_cw_from_north": 45.9,
    ...
    results["checks"] = {
        "meters_and_plausible_dimensions": (
            65.0 <= dims.x <= 70.0 and 75.0 <= dims.y <= 80.0 and 27.0 <= dims.z <= 27.4
        ),  # world-aligned bbox of the 64.74 x 74.99 m footprint plus the 1.2 m
        # tile eave and the 4.2 m marquee/blade overhang on Market; z is the
        # stage-house roof normalised to the 27.2 m LiDAR hgt_max
