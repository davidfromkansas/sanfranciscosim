"""Compose the review renders into muni-bus-contact-sheet.png.

    python3 make_contact_sheet.py [--out DIR]

Pure PIL, no Blender. Labels every tile so the sheet is readable on its own in
a PR, and notes the shared ortho scale so nobody has to take "same framing" on
trust.
"""
...
TILES = [
    ...
    ("left", "LEFT"),
    ("right", "RIGHT"),
    ("top", "TOP — roof composition"),
    ("aerial", "AERIAL — 42° down, 100 mm"),
    ("night", "NIGHT — glow set ignited"),
    ("in-city-1.6x", "IN CITY @ 1.6× — the app's own scale"),
]
COLS = 2
PAD = 18
LABEL_H = 34
...
def main():
    ...
    d.text((PAD, 16), "muni-bus-40  ·  New Flyer Xcelsior XDE40  ·  29 SUNSET  ·  fleet 8632",
           fill=INK, font=font(30))
    d.text((PAD, 46), "elevations share ortho scale, framing, light rig and exposure",
    ...
    path = os.path.join(out, "muni-bus-contact-sheet.png")
    sheet.save(path)
