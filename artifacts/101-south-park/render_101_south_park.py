"""Controlled review renders of the exported 101 South Park GLB.

    blender -b --python render_101_south_park.py -- [--glb FILE] [--out DIR]
                                                    [--prefix 101-south-park]
                                                    [--night]
...
(north = Blender +Y), which is how the asset is authored — the South Park front faces NW (318.3 deg).
...
def light_glow():
    ...
    for mat, bsdf in glow_materials():
        if bsdf:
            # 6.0 blew every glow surface out to pure white and threw away the
            # warm oak the ground-floor row is supposed to read as.
...
def main():
    ...
    glb = arg("--glb", os.path.join(here, "101-south-park.glb"))
    out = arg("--out", here)
    prefix = arg("--prefix", "101-south-park")
    night = "--night" in argv
    only = arg("--only", "")
    ...
    if night:
        ...
        az = math.radians(5)  # three-quarter: front + Jack London Alley flank
        r = span * 3.4
        aer.location = Vector(
            (
    ...
    # Square onto the front (318 deg) hides both 29.6 m flanks edge-on, which is
    # most of this building. 5 deg sits between the front normal (318) and the
    # Jack London Alley flank normal (44) and shows both — a true three-quarter.
    az = math.radians(5)
    r = span * 3.4
    aer.location = Vector(
        (
