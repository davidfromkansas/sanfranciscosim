...
that could plausibly be a sea-level datum. 250 Van Ness is a fifth: OSM and the
LiDAR *mode* agree on ~10 m while Overture — the pipeline's own bake input —
carries 12.4 m, because both the LiDAR max (13.0 m) and the LiDAR min (2.5 m) on
that footprint are street-tree canopy, not building. A height read off a raster
statistic is only as good as the raster's edges.
